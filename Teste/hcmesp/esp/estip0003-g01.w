...
*******************************************************************************/
{include/i-prgvrs.i ESTIP0003G 2.12.00.004}
...
&Scoped-Define ENABLED-OBJECTS c_cdn_empresa rt-button c_cdn_estab ~
i_cdn_funcionario i_cdn_cargo_basic bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cdn_empresa c_cdn_estab ~
i_cdn_funcionario i_cdn_cargo_basic 
...
DEFINE VARIABLE i_cdn_cargo_basic AS INTEGER FORMAT ">>,>>9":U INITIAL 0 
     LABEL "Cargo B sico" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE i_cdn_funcionario AS INTEGER FORMAT "zzzzzzz9":U INITIAL 0 
     LABEL "Matr¡culo" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_cdn_empresa AT ROW 1 COL 30 COLON-ALIGNED WIDGET-ID 2
     c_cdn_estab AT ROW 2 COL 30 COLON-ALIGNED WIDGET-ID 4
     i_cdn_funcionario AT ROW 3 COL 30 COLON-ALIGNED WIDGET-ID 6
     i_cdn_cargo_basic AT ROW 4 COL 30 COLON-ALIGNED WIDGET-ID 8
     bt-ok AT ROW 5.71 COL 2.14
     bt-cancela AT ROW 5.71 COL 13
     bt-ajuda AT ROW 5.71 COL 58
     rt-button AT ROW 5.46 COL 1
...
DO:
FIND es_import_ppr NO-LOCK
    WHERE es_import_ppr.cdn_empresa EQ INPUT FRAME {&FRAME-NAME} c_cdn_empresa
      AND es_import_ppr.cdn_estab EQ INPUT FRAME {&FRAME-NAME} c_cdn_estab
      AND es_import_ppr.cdn_funcionario EQ INPUT FRAME {&FRAME-NAME} i_cdn_funcionario
      AND es_import_ppr.cdn_cargo_basic EQ INPUT FRAME {&FRAME-NAME} i_cdn_cargo_basic NO-ERROR.

IF NOT AVAILABLE es_import_ppr THEN DO:
    {utp/ut-table.i hresp es_import_ppr 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es_import_ppr).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_cdn_empresa c_cdn_estab i_cdn_funcionario i_cdn_cargo_basic 
      WITH FRAME d-vapara.
  ENABLE c_cdn_empresa rt-button c_cdn_estab i_cdn_funcionario 
         i_cdn_cargo_basic bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESTIP0003G" "2.12.00.004"}
