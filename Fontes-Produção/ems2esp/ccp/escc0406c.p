...
********************************************************************************/
{include/i-prgvrs.i ESCC0406C 2.00.00.044 } /*** 010044 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCC0406c MCC}
&ENDIF

/*******************************************************************************
**
**   ESCC0406C.P - relatorio de pedidos emitidos por fornecedor.
**
*******************************************************************************/
{cdp/cdcfgmat.i}
...
def var c-racha like item-fornec.cod-emitente.
...
form c-lb-fornec format "x(10)" ": "
     emitente.cod-emitente " - " emitente.nome-abrev skip(1)
     with stream-io no-box no-label width 132 frame f-racha.
    ...
    for each ordem-compra
        ...
        for each prazo-compra
            ...
            find emitente
                where emitente.cod-emitente = pedido-compr.cod-emitente
                no-lock no-error.
            if  pedido-compr.cod-emitente <> c-racha then do:
                page.
                disp c-lb-fornec
                     emitente.cod-emitente
                     emitente.nome-abrev
                     with stream-io frame f-racha.
                assign c-racha = integer(pedido-compr.cod-emitente).
            end.
            repeat:
...
/*                                          tt-param.i-moeda).                                                  */
            ...
            assign de-tot-liq    = de-tot-liq    + de-preco-merc
                   de-tot-ipi    = de-tot-ipi    + de-valor-ipi
                   de-tot-for    = de-tot-for    + de-preco-total
    ...
    if param-global.modulo-07 then
        if tt-param.l-despesas then do:
            ...
            if tt-param.l-despesas-inc then
                assign de-tot-for = de-tot-for + de-val-desp.                              
        end.    

    if  last-of(pedido-compr.cod-emitente) and de-tot-for <> 0 then do:
        ...
        put c-lb-tot-liq    to 54 format "x(13)" ": " de-tot-liq at 57 
            c-lb-tot-ipi    to 93 format "x(12)" ": " de-tot-ipi at 96 skip
            c-lb-tot-descto to 54 format "x(14)" ": " de-tot-descto at 59
            c-lb-tot-for    to 93 format "x(16)" ": " de-tot-for at 96 skip(1).
        assign de-tot-ger    = de-tot-ger + de-tot-for
    ...
    if  l-imprimiu then /*se h  ordem para o pedido*/
        assign l-imprimiu = no
               i-nr-pedido = i-nr-pedido + 1. /*totaliza pedidos*/.
end.

if  i-nr-pedido <> 0 and de-qt-tot-ped <> 0 then do:
    if  line-counter > 60 then do:
        page.
        disp c-lb-pedido  c-lb-dt-ped
             c-lb-nat     c-lb-fornec
