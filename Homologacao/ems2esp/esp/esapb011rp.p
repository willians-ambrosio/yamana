...
*                             ALTERA€ÇO
*******************************************************************************
* DATA ALTERA€ÇO: 03/07/2018
* AUTOR.........: WILLIANS M AMBROSIO - GRUPO DKP
* DESCRI€ÇO.....:  Regra para busca da conta contabil antecipada para atender a 
*...............   busca dos registros do esapb010 corretamente E INCLUSÇO DA
*...............   COLUNA AC CIDADE X ESTADO.                                    
*******************************************************************************/
{esp/esapb009-bf.i}
{include/i-prgvrs.i espab011rp 12.1.13.003}
{utp/ut-glob.i}
...
DEFINE TEMP-TABLE tt-titulos
    ...
    FIELD cidade-estado         AS CHARACTER
    FIELD vendor-cat-code       AS CHARACTER
    FIELD new-cat-code          AS CHARACTER.
...
DEFINE BUFFER bf-es-cat-code2 FOR es-cat-code.
...
ASSIGN c-arq-csv   = tt-param.arquivo
       ...
       c-arq-csv   = REPLACE(c-arq-csv,ENTRY(NUM-ENTRIES(c-arq-csv,"/"),c-arq-csv,"/"),"ESAPB011-" + REPLACE(STRING(TIME,"hh:mm"),":","") + ".csv").
...
PROCEDURE pi-cria-tt:
    ...
    do v_num_contador = 1 to num-entries(v_des_estab_select):
        ...
        FOR EACH estabelecimento NO-LOCK WHERE
            ...
            run pi-inicializar in h-acomp (input "Filtrando IMP, Aguarde...").
            RUN pi-filtra-tit-impl.

            run pi-inicializar in h-acomp (input "Filtrando BXA, Aguarde...").
            RUN pi-filtra-tit-bxa.
...
PROCEDURE pi-filtra-tit-bxa:
    ...
    FOR EACH tit_ap NO-LOCK WHERE
       ...
       first movto_tit_ap OF tit_ap where
             movto_tit_ap.ind_trans_ap = "BAIXA" NO-LOCK,
        each aprop_ctbl_ap OF movto_tit_ap where
             aprop_ctbl_ap.ind_natur_lancto_ctbl = "DB" no-lock:
       ...
       RUN pi-acompanhar IN h-acomp ('T¡tulo Verif: ' + STRING(bf-movto_tit_ap.num_id_tit_ap) + tit_ap.cod_estab + " -  Data: " + STRING( tit_ap.dat_transacao )).
       ...
       /* Begins 03/07/2018 - Regra para busca da conta contabil antecipada para atender a busca dos registros do esapb010 corretamente */
       ASSIGN c-conta-contabil = "".
...
/*                            es-cat-code.cat-code = es-cat-code-fornec.cat-code NO-LOCK NO-ERROR.  */
/*                                                                                                  */
...
/*                                                                                                  */
/*                  WHERE es-cat-code-conta.cod_cta_ctbl         = c-conta-contabil                 */
...
/*        END.                                                                                      */
       ...
       FOR EACH b-movto_tit_ap OF tit_ap WHERE
                ...
                b-movto_tit_ap.log_movto_estordo      = NO      AND 
                b-movto_tit_ap.log_bxa_contra_antecip = NO      NO-LOCK. 
           ...
           FIND FIRST compl_movto_pagto OF b-movto_tit_ap 
                     NO-LOCK NO-ERROR.    
           ...
           ASSIGN tt-titulos.cod-fornecedor        =   string(fornecedor.cdn_fornecedor) + "-" + fornecedor.nom_pessoa                         
                  ...
                  tt-titulos.val_pagto_tit_ap      =   IF ems5.espec_docto.ind_tip_espec_docto = "Antecipa‡Æo" THEN tit_ap.val_origin_tit_ap ELSE tit_ap.val_pagto_tit_ap                                       
           ...
           FIND FIRST emitente WHERE 
                      emitente.cod-emitente = fornecedor.cdn_fornecedor NO-LOCK NO-ERROR.
           IF AVAIL emitente THEN
           DO:                     
              ASSIGN tt-titulos.cidade-estado = emitente.cidade + " / " + emitente.estado.
       
              IF AVAIL es-cat-code-conta THEN
              DO:                         
                  FIND FIRST es-cat-code WHERE 
                             es-cat-code.cat-code = es-cat-code-conta.cat-code NO-LOCK NO-ERROR.
                  IF AVAIL es-cat-code THEN
                     ASSIGN tt-titulos.vendor-cat-code = es-cat-code-conta.cat-code + "-" + es-cat-code.desc-cat-code.
              END.
           END.
       
                    
       
           FIND FIRST ems5.portador OF b-movto_tit_ap NO-LOCK NO-ERROR.
           IF AVAILABLE(portador) THEN DO:
              CASE portador.cod_portador:
                  WHEN "1011" OR WHEN "1006" OR WHEN "1018" OR WHEN "1013" THEN DO:
                     ASSIGN  tt-titulos.portador = portador.cod_portador + "-" + portador.nom_abrev.
                  END.
                  OTHERWISE DO:
                     ASSIGN  tt-titulos.portador = "Dinheiro".
                  END.
              END CASE.
           END.
           ...
           IF AVAIL bf-es-cat-code THEN
           DO:                      
              ASSIGN tt-titulos.Object_Account       = bf-es-cat-code.cat-code + " - " + bf-es-cat-code.desc-cat-code.
       
              IF AVAIL es-cat-code-fornec THEN
              DO:                         
                  FIND FIRST es-cat-code-fornec-estab WHERE 
                             es-cat-code-fornec-estab.cdn_fornecedor = fornecedor.cdn_fornecedor     AND                                          
                             es-cat-code-fornec-estab.cod_estab      = tt-titulos.cod_Estab          NO-LOCK NO-ERROR.
                  IF AVAIL es-cat-code-fornec-estab THEN
                  DO:
                     FIND FIRST bf-es-cat-code2 WHERE 
                                bf-es-cat-code2.cat-code = es-cat-code-fornec-estab.cat-code-estab NO-LOCK NO-ERROR.
                     IF AVAIL bf-es-cat-code2 THEN
                        ASSIGN tt-titulos.new-cat-code = bf-es-cat-code2.cat-code + " - " + bf-es-cat-code2.desc-cat-code.
                  END.
              END.
           END.
           ELSE
...
PROCEDURE pi-gera-csv:
    ...
    IF tt-param.tipo-clas = 1 THEN DO:
        EXPORT STREAM s-csv DELIMITER ";"
            ...
            "Form of payment"
            "Vendor Category Code/State/City"
            "State/City"
            "New Cat Code".
    END.
    ELSE DO:
    ...
    FOR EACH tt-titulos
        ...
        IF tt-param.tipo-clas = 1 THEN DO:
            ...
            EXPORT STREAM s-csv DELIMITER ";"
                ...
                tt-titulos.portador     
                tt-titulos.vendor-cat-code
                tt-titulos.cidade-estado                
                tt-titulos.new-cat-code
                .
        END.
        ELSE DO:
...
------------------------------------------------------------------------------*/
    ...
    chworksheet:range("A1:AF1"):FONT:Bold = TRUE.
