...
*******************************************************************************/
{include/i-prgvrs.i ymfp0011C 1.02.10.000 } /*** 01010002 ***/
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ymfp0011C MFP}
&ENDIF
...
DEFINE VARIABLE h_ymqry005 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymvwr003c AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 9.54 COL 2.43 HELP
          "Salva e sai"
     bt-save AT ROW 9.54 COL 12.86 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 9.54 COL 23.43 HELP
          "Cancela"
     rt-button AT ROW 9.38 COL 1.29
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 62.86 BY 12.79 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica TIP Targets"
         HEIGHT             = 9.83
         WIDTH              = 63
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica TIP Targets */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica TIP Targets */
DO:
...
DO:
{include/salvafil.i h_ymvwr003c}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'object/sopesp/vwr/ymvwr003c.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymvwr003c ).
       RUN set-position IN h_ymvwr003c ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.29 , 62.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'object/sopesp/qry/ymqry005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_ymqry005 ).
       RUN set-position IN h_ymqry005 ( 1.25 , 45.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 6.00 ) */

       /* Links to SmartViewer h_ymvwr003c. */
       RUN add-link IN adm-broker-hdl ( h_ymqry005 , 'Record':U , h_ymvwr003c ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_ymvwr003c ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymvwr003c ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ymfp0011C" "1.02.10.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_ymvwr003c (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_ymqry005 (input v-row-table).
