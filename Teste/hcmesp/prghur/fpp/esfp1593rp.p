...
*******************************************************************************/
{include/i-prgvrs.i ESFP1593RP 12.1.13.000 } /*** 010135 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
{include/i-license-manager.i ESFP1593rp MFP}
&ENDIF
...
{include/i-epc200.i ESFP1593rp}

/* Temporary Table Definitions ---                                      */
{include/i_cdrel_hr.i}
{prghur/fpp/ESFP1593tt.i new shared}  /* Parametro */

define buffer b-tt-digita for tt-digita.

/* Defini‡Æo e Prepara‡Æo de Parƒmetros */
...
define new shared var v_han_acomp    as handle                                      no-undo.
...
define var v_cdn_categ_sal         like funcionario.cdn_categ_sal                   no-undo.
...
def var v_cdn_categ_sal2             like funcionario.cdn_categ_sal                 no-undo.
def var v_cod_unid_lotac2            like func_unid_lotac_plano.cod_unid_lotac      no-undo.
def var v_log_cabec                  as log                                         no-undo.
...
&if "{&cd_rel_hr}" >= "2.11" &then 
    form 
        skip(5)
        c-titulo                   no-label colon  60 skip(2)
        c-titulo-sel                        colon  20 skip(1)
        tt-param.i-es-ini                   colon  40 
        "               |<  >| "    
        tt-param.i-es-fim          no-label        skip   
        &if "{&emit_func}" <> "no" &THEN
        tt-param.i-fc-ini          format "zzzzzzz9"         colon  40  
        "            |<  >| " 
        tt-param.i-fc-fim no-label format "zzzzzzz9"         skip   
        &endif
        tt-param.v_num_faixa                colon  40 skip
        tt-param.v_cod_unid_lotac_ini       colon  40
        "         |<  >| " 
        tt-param.v_cod_unid_lotac_fim no-label        skip   
        ...
        {utp/ut-liter.i N¡vel_Unidade_Lota‡Æo MFP L}
        assign tt-param.v_num_faixa:label in frame f-sel = trim(return-value).
        {utp/ut-liter.i Sele‡Æo *}
        assign c-titulo-sel:label in frame f-sel = return-value.
        {utp/ut-liter.i Parƒmetros_de_EmissÆo * R}
        assign c-titulo = trim(return-value).
&else 
    form 
        tt-param.cod_rh_ccusto_ini  colon 40 space(1)
        "      |<  >| "
...
form header
    ...
    "Turno"                                     at 120  skip
    "----------- ---------------------------------------- -------- ----------- ---------- ---------- ------ --- ----------- -----" skip
     with stream-io attr-space side-labels /*page-top*/ no-box width 132 frame f-cab-dados-categ.
...
form
    v_cdn_categ_sal                    at 01    space(02)
    "-"                                         space(01)
    v_des_categ_sal                    no-label skip(1)
    with down width 132 no-box no-attr-space side-labels stream-io frame f-categ.
...
form v_num_fc                               at 001
     ...
     v_des_envel                            at 108
     func_turno_trab.cdn_turno_trab         at 121
     with no-label down width 132 no-box no-attr-space stream-io frame f-dados_categ.

form v_num_fc                               at 001
     funcionario.nom_pessoa_fisic           at 13
     func_unid_lotac_plano.cod_unid_lotac   at 54 FORMAT "x(11)"
     funcionario.dat_admis_func             at 70     
     funcionario.dat_livre_1                at 79     
     v_cdn_cargo_basic                      at 91     
     v_cdn_niv_cargo                        at 97     
     v_des_envel                            at 101    
     v_des_categ_sal                        at 113    
     func_turno_trab.cdn_turno_trab         at 130    
     with no-label down width 134 no-box no-attr-space stream-io frame f-dados_ccusto.
...
{utp/ut-liter.i Total_Categoria_-_Funcion rios * R}
assign v_des_tot_categ = trim(return-value) + ":".
...
run utp/ut-trfrrp.p (input frame f-cab-dados-categ:handle).
...
assign v_cdn_classif = classifica.

IF v_cdn_classif <> 99 THEN
DO:
    ...
    /*                 funcionario.dat_admis_transf_func   <= tt-param.v_dat_valid)               and  */
                  ...
                  ((funcionario.idi_tip_func = 1 and tt-param.l-func          = yes) or
                   (funcionario.idi_tip_func = 2 and tt-param.l-estag         = yes) or
                   (funcionario.idi_tip_func = 4 and tt-param.l-empreg        = yes) or
                   (funcionario.idi_tip_func = 5 and tt-param.l-prazo-determ  = yes) or
                   (funcionario.idi_tip_func = 6 and tt-param.l-tempo-parcial = yes) or
                   (funcionario.idi_tip_func = 7 and tt-param.l-aprendiz      = yes) or
                  ((funcionario.idi_tip_func = 3 and tt-param.l-aposent       = yes) and
                   (funcionario.dat_livre_1 >= tt-param.v-data-aposent))),
             ...
             for each tt-epc exclusive-lock where 
                      tt-epc.cod-event = "EPC_LISTA_AFASTADOS_FP1593RP":U:
                 delete tt-epc. 
             END.
    
             create tt-epc.
             assign tt-epc.cod-event         = "EPC_LISTA_AFASTADOS_FP1593RP":U
                    tt-epc.val-parameter     = string(funcionario.cdn_empresa)     + "|" +
                                               string(funcionario.cdn_estab)       + "|" +
                                               string(funcionario.cdn_funcionario) + "|" +
                                               string(tt-param.v_dat_valid).
             
             {include/i-epc201.i "EPC_LISTA_AFASTADOS_FP1593RP"} 
                 
             if return-value = "EPC_LISTA_AFASTADOS_FP1593RP_NOK":U then
                 next.
            /************************************************ FIM EPC ******************************************************/
             ...
             &ENDIF
              ...
              if last-of(funcionario.cdn_estab) and
                 v_tot_estab > 0 then do:
                  ...
                  put v_des_tot_est at 77 v_tot_estab to 116 format ">>>>>>>9".
                  put unformatted skip(1).
              end.
              if last-of(funcionario.cdn_empresa) and
                 v_tot_emp > 0 then do:
                  put unformatted skip(1).
                  put v_des_tot_emp at 75 v_tot_emp to 116 format ">>>>>>>9".
                  put unformatted skip(1).
                  page.
              end.
           end /* for funcionario */.
        ...
        if v_cdn_classif = 3 or
           ...
           /*Este preprocessor ‚ utilizado para filtrar os demais parametros de tela. Com isso a t‚cnica de lota‡Æo imprime corretamente as unidades pai do funcionario.*/
           &global-define outrosFiltros yes
           ...
           for each tt_lotac_funcionario no-lock,
               each funcionario of tt_lotac_funcionario no-lock  where
                  ...
                  ((funcionario.idi_tip_func = 1 and tt-param.l-func          = yes) or
                   ...
                   (funcionario.idi_tip_func = 7 and tt-param.l-aprendiz      = yes) or
                  ((funcionario.idi_tip_func = 3 and tt-param.l-aposent       = yes) and
                   (funcionario.dat_livre_1 >= tt-param.v-data-aposent))) and
                    funcionario.dat_admis_func         <= tt-param.v_dat_valid          and
                   (funcionario.dat_desligto_func       = ?                             or
                    funcionario.dat_desligto_func       > tt-param.v_dat_valid          or
                   (tt-param.log_lista_demit            = yes                           and
                   year(funcionario.dat_desligto_func)  = year(tt-param.v_dat_valid)    and
                   month(funcionario.dat_desligto_func) = month(tt-param.v_dat_valid))),
    /*             and                                                                         */
    /*               (funcionario.dat_admis_transf_func    = ?                             or  */
    /*                funcionario.dat_admis_transf_func   <= tt-param.v_dat_valid),            */
        ...
        if v_cdn_classif = 5 or
           ...
           view frame f-cab-estab.
           for each func_categ_sal no-lock where
                    func_categ_sal.cdn_empresa          = tt-param.v_cdn_empres_usuar         and
                    func_categ_sal.cdn_estab           >= tt-param.i-es-ini                   and
                    func_categ_sal.cdn_estab           <= tt-param.i-es-fim                   and
                    func_categ_sal.cdn_funcionario     >= tt-param.i-fc-ini                   and
                    func_categ_sal.cdn_funcionario     <= tt-param.i-fc-fim                   and
                    func_categ_sal.dat_inic_lotac_func <= tt-param.v_dat_valid                and
                    func_categ_sal.dat_fim_lotac_func  >= tt-param.v_dat_valid                and
                  ((func_categ_sal.cdn_categ_sal        = 1 and tt-param.log_mensal    = yes) or
                   (func_categ_sal.cdn_categ_sal        = 2 and tt-param.log_horista   = yes) or
    ...
    /*                funcionario.dat_admis_transf_func   <= tt-param.v_dat_valid),            */
              ...
              break by funcionario.cdn_empresa
                    by funcionario.cdn_estab
                    by func_categ_sal.cdn_categ_sal
                    by if v_cdn_classif = 5 
                       then string(funcionario.cdn_funcionario,"99999999")
                       else funcionario.nom_pessoa_fisic:
              ...
              assign v_num_fc        = if v_log_folha_educnal then string(funcionario.cdn_func_centrdor, "zzzzz9") + "/" + string(funcionario.cdn_tip_contrat_func, "99") + "-" + string(funcionario.num_digito_verfdor_func)
                     ...
                     v_cdn_categ_sal = func_categ_sal.cdn_categ_sal
                     v_des_categ_sal = {database/inpy/i01py118.i 04 func_categ_sal.cdn_categ_sal}.
              ...
              if first-of(func_categ_sal.cdn_categ_sal) then do:
                assign v_tot_categ = 0.
              end.
              ...
              if (v_cdn_cargo_basic > 0 and
                 ...
                 (v_cdn_cargo_basic_funcao > 0 and
                  ...
                  if v_cdn_categ_sal2 <> func_categ_sal.cdn_categ_sal or
                     line-counter < 2 or
                     line-counter > 63 then do:
                      assign v_cdn_categ_sal2 = func_categ_sal.cdn_categ_sal.
                      disp v_cdn_categ_sal
                           v_des_categ_sal
                           with stream-io frame f-categ.
                      view frame f-cab-dados-categ.
                  end.
              end.
              ...
              if v_cdn_cargo_basic > 0 and
                 tt-param.idi_tip_cargo_funcao <> 2 then do:
                  disp v_num_fc
                       ...
                       v_des_envel
                       func_turno_trab.cdn_turno_trab
                       with stream-io frame f-dados_categ.
                  down with frame f-dados_categ.
              end.
              /* Imprime dados da fun‡Æo */
              if v_cdn_cargo_basic_funcao > 0 and
                 tt-param.idi_tip_cargo_funcao <> 1 then do:
              ...
              if (v_cdn_cargo_basic > 0 and
                 ...
                 (v_cdn_cargo_basic_funcao > 0 and
                  ...
                  assign v_tot_emp   = v_tot_emp + 1
                         v_tot_estab = v_tot_estab + 1
                         v_tot_categ = v_tot_categ + 1.
    
              if last-of(func_categ_sal.cdn_categ_sal) and
                 v_tot_categ > 0 then do:
                put unformatted skip(1).
                put v_des_tot_categ at 75 v_tot_categ to 116 format ">>>>>>>9".
                put unformatted skip(1).
              end.
              if last-of(funcionario.cdn_estab) and
                 v_tot_estab > 0 then do:
...
if v_log_folha_educnal then
  assign tt-param.i-fc-ini = {prghur/dop/eng005.i &VAR="tt-param.i-fc-ini / 100"}
         tt-param.i-fc-fim = {prghur/dop/eng005.i &VAR="(tt-param.i-fc-fim - 99) / 100"}.
{utp/ut-liter.i Sim/NÆo MFP R}
assign tt-param.log_mensal:format       in frame f-param = trim(return-value)
       tt-param.log_horista:format      in frame f-param = trim(return-value)
       tt-param.log_semanal:format      in frame f-param = trim(return-value)
       tt-param.log_quinzenal:format    in frame f-param = trim(return-value)
       tt-param.log_tarefa:format       in frame f-param = trim(return-value)
       tt-param.log_diarista:format     in frame f-param = trim(return-value)
       tt-param.log_lista_demit:format  in frame f-param = trim(return-value).
...
if v_log_folha_educnal then do:
    disp v_des_selecao
         ...
         tt-param.i-fc-ini
         tt-param.i-fc-fim
         tt-param.v_num_faixa
         tt-param.v_cod_unid_lotac_ini
         tt-param.v_cod_unid_lotac_fim
         tt-param.cod_rh_ccusto_ini
         tt-param.cod_rh_ccusto_fim
         tt-param.cdn_tip_contrat_ini
         tt-param.cdn_tip_contrat_fim
         with frame f-selec_educ.
end.  
else do:
    &if "{&cd_rel_hr}" >= "2.11" &then
        disp c-titulo
    ...
    &else
        run pi_report_selec.
        disp tt-param.cod_rh_ccusto_ini
             tt-param.cod_rh_ccusto_fim
             with frame f-sel. 
    &endif
...
run pi_report_param.
   disp tt-param.log_mensal
        tt-param.log_horista
        tt-param.log_semanal
        tt-param.log_quinzenal
        tt-param.log_tarefa
        tt-param.log_diarista
        tt-param.log_lista_demit
        with frame f-param.
