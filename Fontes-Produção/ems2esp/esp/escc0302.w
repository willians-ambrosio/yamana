...
*******************************************************************************/
{include/i-prgvrs.i ESCC0302 2.06.00.002}
...
DEFINE VARIABLE da-dt-limite AS DATE FORMAT "99/99/9999":U 
     LABEL "Data Limite" 
     VIEW-AS FILL-IN 
     SIZE 11.57 BY .88 NO-UNDO.

DEFINE VARIABLE i-cod-mensagem AS INTEGER FORMAT ">>>9":U INITIAL 0 
     LABEL "Mensagem" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-pg-par
     l-narrativa-ord-compr AT ROW 1 COL 3.29
     l-narrativa-item-ord-compr AT ROW 2 COL 3.29
     l-descricao AT ROW 3 COL 3.29
     l-envia-email AT ROW 4 COL 3.29
     da-dt-limite AT ROW 5 COL 9.57 COLON-ALIGNED
     BUTTON-1 AT ROW 6.13 COL 21.29
     i-cod-mensagem AT ROW 6.21 COL 11.86 COLON-ALIGNED
     c-narrativa AT ROW 7.25 COL 2 NO-LABEL
     RECT-17 AT ROW 6 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 76.72 BY 10.46
...
ON END-ERROR OF w-relat /* Ficha de Cotacao */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Ficha de Cotacao */
DO:
...
ON CHOOSE OF BUTTON-1 IN FRAME f-pg-par /* Button 1 */
DO:
    ASSIGN i-cod-mensagem.

    FIND FIRST mensagem NO-LOCK
         WHERE mensagem.cod-mensage = i-cod-mensagem NO-ERROR.
    ASSIGN c-narrativa:SCREEN-VALUE = IF AVAIL mensagem THEN mensagem.texto-mensag ELSE "".
...
{utp/ut9000.i "ESCC0302" "2.06.00.002"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    assign dt-ini-emissao:screen-value in frame f-pg-sel = string(date(month(today),01,year(today)))
           ...
           da-dt-limite:screen-value   in frame f-pg-par = string(today).
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY l-narrativa-ord-compr l-narrativa-item-ord-compr l-descricao 
          l-envia-email da-dt-limite i-cod-mensagem c-narrativa 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-17 l-narrativa-ord-compr l-narrativa-item-ord-compr l-descricao 
         l-envia-email da-dt-limite BUTTON-1 i-cod-mensagem c-narrativa 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario                    = c-seg-usuario
           ...
           tt-param.narrativa                  = INPUT FRAME f-pg-par c-narrativa
           tt-param.dt-limite                  = INPUT FRAME f-pg-par da-dt-limite.
    ...
    {include/i-rprun.i esp\escc0302rp.p}
...
------------------------------------------------------------------------------*/
    ...
    for each ordem-compra no-lock
       where ordem-compra.numero-ordem >= input frame f-pg-sel i-ini-numero-ordem
         ...
         and ordem-compra.ep-codigo     = i-ep-codigo-usuario
         and ordem-compra.impr-ficha
        ...
        for each prazo-compra no-lock 
           where prazo-compra.numero-ordem = ordem-compra.numero-ordem:
            ...
            assign tt-digita.flag         = ""
                   ...
                   tt-digita.quantidade   = prazo-compra.quantidade
