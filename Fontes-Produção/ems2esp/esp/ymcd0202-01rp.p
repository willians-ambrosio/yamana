...
*******************************************************************************/
{include/i-prgvrs.i YMCD0202-01RP 11.5.12.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i YMCD0202-01RP 11.5.12.000}
&ENDIF

/* ***************************  Definitions  ************************** */
&global-define programa YMCD0202-01RP
...
FOR EACH es-fila-rep-item
    ...
    IF NOT AVAIL bitem THEN DO:
        ...
        OVERLAY(tt-item.char-2,212,1)  = SUBSTRING(es-fila-rep-item.char-2,212,1).
    ...
    ELSE DO:
        ...
        FIND bbitem WHERE 
             bbitem.it-codigo = bitem.it-codigo EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE(bbitem) THEN DO:
           ASSIGN bbitem.desc-item        = es-fila-rep-item.desc-item
                  bbitem.narrativa        = es-fila-rep-item.narrativa
           /* Begins Jan/2019 - Willians Ambrosio - DKP / Comentado o ponto para evitar que o fluxo seja atualizado nas outras empresas . 
                                Com isso o fluxo fica independente e por empresa */
