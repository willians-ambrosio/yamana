...
/****************************************************************************************** 
**         Programa: rep001-z01.w
**            Autor: Felipe Vieira
**       Fornecedor: DKP
**       Data: 25/09/2018
** Change/Chamado: 
**    Objetivo: Cadastro de aprovadores (Zoom)
**
...
******************************************************************************************/
{include/i-prgvrs.i REP001-Z01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i REP001-Z01 REP}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-ajuda bt-ok bt-cancela 
...
DEFINE VARIABLE h_b-table-customer AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 8.25 COL 14
     bt-ajuda AT ROW 13.71 COL 39.29
     bt-ok AT ROW 13.75 COL 2.72
     bt-cancela AT ROW 13.75 COL 13.72
     rt-button AT ROW 13.5 COL 1.72
...
ON END-ERROR OF w-pesquisa /* Pesquisa de aprovadores */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de aprovadores */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Aprovador' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 12.08 , 50.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'rep/REP001-B01.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-table-customer ).
       RUN set-position IN h_b-table-customer ( 2.79 , 2.72 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 48.14 ) */

       /* Links to SmartBrowser h_b-table-customer. */
       RUN add-link IN adm-broker-hdl ( h_b-table-customer , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-table-customer ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ENABLE rt-button bt-ajuda bt-ok bt-cancela 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "REP001-Z01" "1.00.00.000"}
