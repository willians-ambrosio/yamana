...
{include/i-prgvrs.i Q09AD264 2.00.00.011}  /*** 010011 ***/


&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i q09ad264 MUT}
&ENDIF
...
&Scoped-define INTERNAL-TABLES titulo

/* Definitions for QUERY Query-Main                                     */
&Scoped-define SELF-NAME Query-Main
&Scoped-define OPEN-QUERY-Query-Main if  gr-titulo <> ? and l-chamado-cons-cli = yes and l-mostra-titulo-cliente then do:     open query {&SELF-NAME} for each titulo           where rowid(titulo) = gr-titulo no-lock indexed-reposition. end. else do:     if l-mostra-faixa-titulo then         open query {&SELF-NAME} for each titulo              where titulo.ep-codigo   = i-emp-selecao                and titulo.dt-emissao >= da-cr0709-emis-ini                and titulo.dt-emissao <= da-cr0709-emis-fim no-lock indexed-reposition.     else         open query {&SELF-NAME} for each titulo              where titulo.ep-codigo  = i-emp-selecao no-lock indexed-reposition. end.
&Scoped-define TABLES-IN-QUERY-Query-Main titulo
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main titulo
...
<SOems2movmeY-OPTIONS>
</SOems2movmeY-OPTIONS>
<SOems2movmeY-RUN-CODE>
************************
* Set attributes related to SOems2movmeY-OPTIONS */
RUN set-attribute-list (
    'Soems2movmey-Options = ""':U).
/************************
</SOems2movmeY-RUN-CODE>
<FILTER-ATTRIBUTES>
************************
...
/* Query rebuild information for QUERY Query-Main
     _START_FREEFORM
if  gr-titulo <> ?
and l-chamado-cons-cli = yes
and l-mostra-titulo-cliente then do:
    open query {&SELF-NAME} for each titulo
          where rowid(titulo) = gr-titulo no-lock indexed-reposition.
end.
else do:
    if l-mostra-faixa-titulo then
        open query {&SELF-NAME} for each titulo
             where titulo.ep-codigo   = i-emp-selecao
               and titulo.dt-emissao >= da-cr0709-emis-ini
               and titulo.dt-emissao <= da-cr0709-emis-fim no-lock indexed-reposition.
    else
        open query {&SELF-NAME} for each titulo
             where titulo.ep-codigo  = i-emp-selecao no-lock indexed-reposition.
end.
     _END_FREEFORM
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _Where[1]         = "mgadm.titulo.ep-codigo = i-emp-selecao"
...
/*------------------------------------------------------------------------------
  Purpose:     Opens different cases of the query based on attributes
               such as the 'Key-Name', or 'Soems2movmey-Case'
  Parameters:  <none>
------------------------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
  ...
  def input param p-row-titulo as rowid no-undo.

  if  p-row-titulo <> ? then do:
      RUN dispatch IN THIS-PROCEDURE ('open-query':U).
      REPOSITION Query-Main TO ROWID p-row-titulo NO-ERROR.
  end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-retorna-titulo q-tables 
