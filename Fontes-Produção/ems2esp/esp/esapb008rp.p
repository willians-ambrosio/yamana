...
*******************************************************************************/
{esp/esapb009-bf.i}
{include/i-prgvrs.i espab008rp 12.1.13.001}
{utp/ut-glob.i}
...
DEFINE VARIABLE c-Object_Account    LIKE tt-titulos.Object_Account.
...
/*                            ASSIGN c-categoria-cat-code = string(es-cat-code.cat-code) + " - " + es-cat-code.desc-cat-code.                              */
/*                        END.                                                                                                                             */
/*                 END.                                                                                                                                    */
...
/*                                                                                                                                                         */
/*                         ASSIGN c-Object_Account        = IF AVAIL es-cat-code THEN es-cat-code.cat-code + " - " + es-cat-code.desc-cat-code ELSE "N/A". */
/*                                                                                                                                                         */
/*                 END.                                                                                                                                    */

                FIND FIRST es-cat-code-conta 
                     WHERE es-cat-code-conta.cod_cta_ctbl       = aprop_ctbl_ap.cod_cta_ctbl 
                     AND   es-cat-code-conta.cod_plano_cta_ctbl = aprop_ctbl_ap.cod_plano_cta_ctbl   
                     AND   es-cat-code-conta.cons-relatorio = YES 
                     NO-LOCK NO-ERROR.
                IF NOT AVAIL es-cat-code-conta THEN 
                   NEXT.

                FIND FIRST es-cat-code
                     WHERE es-cat-code.cat-code = es-cat-code-conta.cat-code 
                     NO-LOCK NO-ERROR. 
                ASSIGN c-Object_Account = IF AVAIL es-cat-code THEN es-cat-code.cat-code + " - " + es-cat-code.desc-cat-code ELSE "N/A".


                FIND FIRST es-cat-code-fornec 
                     WHERE es-cat-code-fornec.cdn_fornecedor = tit_ap.cdn_fornecedor 
                       AND es-cat-code-fornec.cons-relatorio = YES 
                     NO-LOCK NO-ERROR.
                IF NOT AVAIL es-cat-code-fornec THEN 
                   NEXT.

                 FIND FIRST es-cat-code
                      WHERE es-cat-code.cat-code = es-cat-code-fornec.cat-code 
                      NO-LOCK NO-ERROR. 
                 ASSIGN c-categoria-cat-code = "".
                ...
                FOR EACH b-movto_tit_ap OF tit_ap WHERE
                    ...
                    FIND FIRST compl_movto_pagto OF b-movto_tit_ap 
                        ...
                        ASSIGN tt-titulos.cod-fornecedor      = string(fornecedor.cdn_fornecedor) + "-" + fornecedor.nom_pessoa                         
                               ...
                               tt-titulos.Object_Account      = c-Object_Account.

                       FIND FIRST ems5.portador OF b-movto_tit_ap
                            NO-LOCK NO-ERROR.
                       IF AVAILABLE(portador) THEN DO:
                          CASE portador.cod_portador:
                              WHEN "1008" OR WHEN "1006" OR WHEN "1018" OR WHEN "1013" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    chworksheet:range("A1:AD1"):FONT:Bold = TRUE.

    /* L¢gico para Negritar as linhas de total */
    FOR EACH tt-linhas:
        chworksheet:range("A"+ string(tt-linhas.linha-negrito) + ":AD" + string(tt-linhas.linha-negrito)):FONT:Bold = TRUE.
        chworksheet:range("A"+ string(tt-linhas.linha-negrito) + ":AD" + string(tt-linhas.linha-negrito)):borders:item(3):linestyle = 1.
