...
&Scoped-Define ENABLED-OBJECTS rt-button c_classe bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_classe 
...
DEFINE VARIABLE c_classe AS CHARACTER FORMAT "x(4)":U 
     LABEL "CFA" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_classe AT ROW 1.83 COL 30 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND FIRST es-cfa-ge NO-LOCK
    WHERE es-cfa-ge.classe EQ INPUT FRAME {&FRAME-NAME} c_classe
      AND es-cfa-ge.ep-codigo = i-ep-codigo-usuario NO-ERROR.

IF NOT AVAILABLE es-cfa-ge THEN DO:
    {utp/ut-table.i mgesp es-cfa-ge 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-cfa-ge).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_classe 
      WITH FRAME d-vapara.
  ENABLE rt-button c_classe bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMOF0108-G01" "1.00.00.000"}
