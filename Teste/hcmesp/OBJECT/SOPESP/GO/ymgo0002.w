...
{include/i-prgvrs.i YMGO0002 1.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button v_cdn_empresa v_cod_grp_usuar ~
bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS v_cdn_empresa v_cod_grp_usuar 
...
DEFINE VARIABLE v_cdn_empresa AS INTEGER FORMAT "zz9" INITIAL 0 
     LABEL "Empresa" 
     VIEW-AS FILL-IN 
     SIZE 7.57 BY .88 NO-UNDO.

DEFINE VARIABLE v_cod_grp_usuar AS CHARACTER FORMAT "x(3)":U 
...
DEFINE FRAME d-vapara
     v_cdn_empresa AT ROW 1.75 COL 30 COLON-ALIGNED HELP
          "C¢digo da Empresa" WIDGET-ID 2
     v_cod_grp_usuar AT ROW 2.75 COL 30 COLON-ALIGNED
...
FIND FIRST grp_usuar_esp NO-LOCK WHERE 
           grp_usuar_esp.cdn_empresa   EQ INPUT FRAME {&FRAME-NAME} v_cdn_empresa AND
           grp_usuar_esp.cod_grp_usuar EQ INPUT FRAME {&FRAME-NAME} v_cod_grp_usuar NO-ERROR.
IF NOT AVAILABLE grp_usuar_esp THEN DO:
    ...
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(grp_usuar_esp).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY v_cdn_empresa v_cod_grp_usuar 
      WITH FRAME d-vapara.
  ENABLE rt-button v_cdn_empresa v_cod_grp_usuar bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMGO0002" "1.00.00.000"}
