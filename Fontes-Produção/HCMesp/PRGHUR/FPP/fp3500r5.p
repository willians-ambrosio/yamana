...
*******************************************************************************/
{include/i-prgvrs.i FP3500R5 1.02.00.048 } /*** 010048 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
{include/i-license-manager.i fp3500r5 MFP}
&ENDIF
...
if i-ord-aux = 3 or
  ...
  {prghur/fpp/fp9200.i6}
   for each tt_lotac_funcionario no-lock,
       ...
       DEFINE VARIABLE c-lbl-liter-est AS CHARACTER NO-UNDO.
       {utp/ut-liter.i "Est" *}
       ASSIGN c-lbl-liter-est = TRIM(RETURN-VALUE).
       DEFINE VARIABLE c-lbl-liter-mtr AS CHARACTER NO-UNDO.
       {utp/ut-liter.i "Mtr" *}
       ASSIGN c-lbl-liter-mtr = TRIM(RETURN-VALUE).
       run pi-acompanhar in v_han_acomp (input c-lbl-liter-est + ": " + STRING(funcionario.cdn_estab) + " - " + c-lbl-liter-mtr + ": " + STRING(funcionario.cdn_funcionario)). 
       ...
       for each movto_calcul_func of funcionario no-lock where
           ...
           if movto_calcul_func.idi_tip_fp = 3 and
              ...
              assign w-mvtocalc.fc-codigo   = movto_calcul_func.cdn_funcionario
                     ...
                     if w-mvtocalc.valor > 0 then
                        assign w-mvtocalc.ident = 1.
                     else
                        assign w-mvtocalc.ident = IF w-mvtocalc.ident = 3 THEN 3 ELSE 2
                               w-mvtocalc.valor = w-mvtocalc.valor * -1.
       ...
       END.
        ...
        for each w-mvtocalc 
            ...
            if first-of(w-mvtocalc.fc-codigo) then do:
                /* TQMQZW */
                FOR FIRST func_unid_lotac_plano
                   WHERE func_unid_lotac_plano.cdn_estab            = funcionario.cdn_estab        
                     and func_unid_lotac_plano.cdn_funcionario      = funcionario.cdn_funcionario  
                     and func_unid_lotac_plano.dat_inic_lotac_func <= tt-param.v_dat_valid                  
                     and func_unid_lotac_plano.dat_fim_lotac_func  >= tt-param.v_dat_valid                  NO-LOCK: END.
                FIND FIRST unid_lotac WHERE
                           unid_lotac.cod_unid_lotac = func_unid_lotac_plano.cod_unid_lotac NO-LOCK NO-ERROR.
                RUN pi-imprime-cab.
            END.

            IF v-cont-evento = 16 THEN DO:
                run setObject in h-FunctionLibrary (input "TextoEnvelope1", input "v_des_mensagem2", INPUT "CONTINUA...").
           ...
           if v_log_folha_educnal then
            ...
            REPEAT:
                ...
                for each func_ccusto no-lock where
                    ...
                    assign i-contador = i-contador + 1.
                    /* Inicio -- Projeto Internacional */
                    DEFINE VARIABLE c-lbl-liter-est-2 AS CHARACTER NO-UNDO.
                    {utp/ut-liter.i "Est" *}
                    ASSIGN c-lbl-liter-est-2 = TRIM(RETURN-VALUE).
                    DEFINE VARIABLE c-lbl-liter-mtr-2 AS CHARACTER NO-UNDO.
                    {utp/ut-liter.i "Mtr" *}
                    ASSIGN c-lbl-liter-mtr-2 = TRIM(RETURN-VALUE).
                    run pi-acompanhar in v_han_acomp (input c-lbl-liter-est-2 + ": " + STRING(funcionario.cdn_estab) + " - " + c-lbl-liter-mtr-2 + ": " + STRING(funcionario.cdn_funcionario)). 
                    ...
                    for each w-mvtocalc 
                        ...
                        ASSIGN v-cont-evento = v-cont-evento + 1.
                        IF tt-param.v_num_tip_aces_usuar = 8 THEN DO:
...
/* Inicio -- Projeto Internacional */
{utp/ut-liter.i "Finalizando_Arquivo_PDF" *}
run pi-acompanhar in v_han_acomp (input RETURN-VALUE + "."). 
...
PROCEDURE pi-imprime-cab:
    DO v-num-via = 1 TO 2:
        ...
        END.
        run setObject in h-FunctionLibrary (input "TextoEnvelope" + STRING(v-num-via,"9"), input "v_des_empresa", INPUT empresa.razao-social).
        ...
        END.
        run setObject in h-FunctionLibrary (input "TextoEnvelope" + STRING(v-num-via,"9"), input "v_cod_unid_lotac", INPUT func_unid_lotac_plano.cod_unid_lotac).
