...
*******************************************************************************/
{include/i-prgvrs.i ESTIP0001 2.12.00.004}
...
DEFINE VARIABLE h_estip0001-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_estip0001-b02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_estip0001-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_estip0001-v01 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/estip0001-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_estip0001-v01 ).
       RUN set-position IN h_estip0001-v01 ( 2.46 , 2.57 ) NO-ERROR.
       /* Size in UIB:  ( 3.25 , 87.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'N¡vel|Situa‡äes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.75 , 1.43 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.00 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/estip0001-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp/estip0001-z01.w,
                     ProgVaPara = esp/estip0001-g01.w,
                     ProgIncMod = esp/estip0001a.w,
                     Implantar = no':U ,
             OUTPUT h_estip0001-q01 ).
       RUN set-position IN h_estip0001-q01 ( 1.25 , 51.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 9.00 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_p-exihel ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_estip0001-v01. */
       RUN add-link IN adm-broker-hdl ( h_estip0001-q01 , 'Record':U , h_estip0001-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_estip0001-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_estip0001-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_estip0001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_estip0001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_estip0001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_estip0001-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_estip0001-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_estip0001-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/estip0001-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/estip0001b.w':U ,
             OUTPUT h_estip0001-b01 ).
       RUN set-position IN h_estip0001-b01 ( 7.50 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to BrowserCadastro2 h_estip0001-b01. */
       RUN add-link IN adm-broker-hdl ( h_estip0001-q01 , 'Record':U , h_estip0001-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_estip0001-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/estip0001-b02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp/estip0001c.w':U ,
             OUTPUT h_estip0001-b02 ).
       RUN set-position IN h_estip0001-b02 ( 7.50 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to BrowserCadastro2 h_estip0001-b02. */
       RUN add-link IN adm-broker-hdl ( h_estip0001-q01 , 'Record':U , h_estip0001-b02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_estip0001-b02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESTIP0001" "2.12.00.004"}

  {include/i-inifld.i}

  run pi-recebe-pai in h_estip0001-v01(input this-procedure).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-desabilita-campos w-cadpaifilho-ambos 
...
define input parameter p_row_controle as rowid no-undo.

/*find first es_controle no-lock where
     rowid(es_controle) = p_row_controle no-error.
if avail es_controle then do:

    find first es_movto_arq no-lock of es_controle no-error.
    if avail es_movto_arq and
             es_movto_arq.arq_calc then do:

        run set-buttons in h_p-cadpai('add-only').
        run set-buttons in h_estip0001-b02(false).
        run set-buttons in h_estip0001-b01(false).

    end.
    else do:

        run set-buttons in h_p-cadpai('initial').
        run set-buttons in h_estip0001-b02(true).
        run set-buttons in h_estip0001-b01(true).
