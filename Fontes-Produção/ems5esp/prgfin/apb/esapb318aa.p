...
def var c-versao-prg as char initial " 12.1.13.000":U no-undo.
...
{include/i-license-manager.i es_rpt_compl_impto_retid_ap APB}
&ENDIF
...
run pi_messages (input "show",
                 input 5009,
                 input substitute ("&1~&2~&3~&4~&5~&6~&7~&8~&9", 
                                    "es_rpt_compl_impto_retid_ap","~~EMSFIN", "~~{~&emsfin_version}", "~~5.01")) /*msg_5009*/.
&else
...
def temp-table tt_compl_impto_retid_ap no-undo
    ...
    field ttv_dat_trans                    as date format "99/99/9999" initial today label "Data Emiss∆o" column-label "Data Trans"
...
def frame f_rpt_s_1_header_period header
    "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" at 1
    "P†gina:" at 183
    (page-number (s_1) + v_rpt_s_1_page) at 190 format ">>>>>9" skip
    v_nom_enterprise at 1 format "x(40)"
    v_nom_report_title at 155 format "x(40)" skip
    "Per°odo: " at 1
    v_dat_inic_period at 10 format "99/99/9999"
    "A" at 21
    v_dat_fim_period at 23 format "99/99/9999"
    "-----------------------------------------------------------------------------------------------------------------------------------------------" at 34
    v_dat_execution at 178 format "99/99/9999" "- "
    v_hra_execution at 191 format "99:99" skip (1)
    with no-box no-labels width 195 page-top stream-io.
def frame f_rpt_s_1_header_unique header
    "-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" at 1
    'P†gina:' at 183
    (page-number (s_1) + v_rpt_s_1_page) at 190 format '>>>>>9' skip
    v_nom_enterprise at 1 format 'x(40)'
    v_nom_report_title at 156 format 'x(40)' skip
    '--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------' at 1
    v_dat_execution at 178 format '99/99/9999' '- '
    v_hra_execution at 191 format "99:99" skip (1)
    with no-box no-labels width 195 page-top stream-io.
def frame f_rpt_s_1_footer_last_page header
    "Èltima p†gina " at 1
    "-------------------------------------------------------------------------------------------------------------------------------------------------------------" at 15
    v_nom_prog_ext at 173 format "x(08)" "- "
    v_cod_release at 184 format "x(12)" skip
    with no-box no-labels width 195 page-bottom stream-io.
...
DO:
    ...
    /************************** Variable Definition End *************************/
        ...
        assign v_nom_prog     = v_nom_prog
                              + chr(10)
                              + "es_rpt_compl_impto_retid_ap":U.




    assign v_nom_prog_ext = "prgfin/apb/esapb318aa.py":U
           v_cod_release  = trim(" 12.1.13.000":U).
...
/* {include/i-ctrlrp5.i es_rpt_compl_impto_retid_ap} */
...
then do:
    run pi_version_extract ('es_rpt_compl_impto_retid_ap':U, 'prgfin/apb/esapb318aa.py':U, '12.1.13.000':U, 'pro':U).
end /* if */.
...
if  search("prgtec/men/men901za.r") = ? and search("prgtec/men/men901za.py") = ? then do:
    ...
    run prgtec/men/men901za.py (Input 'es_rpt_compl_impto_retid_ap') /*prg_fnc_verify_security*/.
...
then do:
    ...
    run pi_messages (input "show",
                     input 2014,
                     input substitute ("&1~~&2~~&3~~&4~~&5~~&6~~&7~~&8~~&9",
                                       'es_rpt_compl_impto_retid_ap')) /*msg_2014*/.
    return.
end /* if */.
...
then do:
    ...
    run pi_messages (input "show",
                     input 2012,
                     input substitute ("&1~~&2~~&3~~&4~~&5~~&6~~&7~~&8~~&9",
                                       'es_rpt_compl_impto_retid_ap')) /*msg_2012*/.
    return.
end /* if */.
...
if can-find(prog_dtsul
       where prog_dtsul.cod_prog_dtsul = 'es_rpt_compl_impto_retid_ap' 
         and prog_dtsul.log_gera_log_exec = yes) then do transaction:
    create log_exec_prog_dtsul.
    assign log_exec_prog_dtsul.cod_prog_dtsul           = 'es_rpt_compl_impto_retid_ap'
...
/* tech38629 - Alteraá∆o efetuada via filtro */
&if '{&emsbas_version}':U >= '5.05':U &then
run pi_permissoes in v_prog_filtro_pdf (input 'es_rpt_compl_impto_retid_ap':U).
&endif
/* tech38629 - Fim da alteraá∆o */
...
find prog_dtsul
    where prog_dtsul.cod_prog_dtsul = "es_rpt_compl_impto_retid_ap":U
    no-lock no-error.
...
/* Begin_Include: ix_p00_es_rpt_compl_impto_retid_ap */
...
FUNCTION GetDefinedFunction RETURNS LOGICAL (INPUT SPP AS CHARACTER):
    ...
    IF CAN-FIND (FIRST ems5.histor_exec_especial NO-LOCK
...
then do:
    ...
    if (ped_exec.cod_release_prog_dtsul <> trim(" 12.1.13.000":U)) then
        return "Vers‰es do programa diferente." /*1994*/ + " (" + "1994" + ")" + chr(10)
                                     + substitute("A vers∆o do programa (&3) que gerou o pedido de execuá∆o batch (&1) Ç diferente da vers∆o do programa que deveria executar o pedido batch (&2)." /*1994*/,ped_exec.cod_release_prog_dtsul,
                                                  trim(" 12.1.13.000":U),
                                                  "prgfin/apb/esapb318aa.py":U).
    assign v_nom_prog_ext     = caps("esapb318aa":U)
    ...
    /* Begin_Include: ix_p02_es_rpt_compl_impto_retid_ap */
    ...
    /* End_Include: ix_p02_es_rpt_compl_impto_retid_ap */
    ...
    then do:
        ...
        /* ix_p29_es_rpt_compl_impto_retid_ap */
        ...
        /* Begin_Include: ix_p30_es_rpt_compl_impto_retid_ap */
        ...
        /* End_Include: ix_p30_es_rpt_compl_impto_retid_ap */
...
/* tech38629 - Fim da alteraá∆o */
    ...
    /* Begin_Include: ix_p10_es_rpt_compl_impto_retid_ap */
    ...
    /* End_Include: ix_p10_es_rpt_compl_impto_retid_ap */
    ...
    repeat on error undo block1, retry block1:
        ...
        repeat on error undo super_block, retry super_block
            ...
            do transaction:
                ...
                /* Begin_Include: ix_p15_es_rpt_compl_impto_retid_ap */
                ...
                /* End_Include: ix_p15_es_rpt_compl_impto_retid_ap */
                ...
                /* ix_p20_es_rpt_compl_impto_retid_ap */
            end /* do param_block */.
...
&endif
                         do:
                            assign v_cod_dwb_file   = session:temp-directory + substring ("prgfin/apb/esapb318aa.py", 12, 6) + '.tmp'
...
&endif
                    assign v_nom_prog_ext  = caps(substring("prgfin/apb/esapb318aa.py",12,8))
                           v_cod_release   = trim(" 12.1.13.000":U)
                           v_dat_execution = today
                           v_hra_execution = replace(string(time,"hh:mm:ss" /*l_hh:mm:ss*/ ),":","").
                    run pi_es_rpt_compl_impto_retid_ap /*pi_es_rpt_compl_impto_retid_ap*/.
                end /* else */.
                if  dwb_rpt_param.log_dwb_print_parameters = yes
                then do:
                    if (page-number (s_1) > 0) then
...
&endif
        ...
        /* ix_p32_es_rpt_compl_impto_retid_ap */
        ...
        /* ix_p35_es_rpt_compl_impto_retid_ap */

    end /* repeat block1 */.
end /* repeat super_block */.


/* Begin_Include: ix_p40_es_rpt_compl_impto_retid_ap */
...
/*****************************************************************************
** Procedure Interna.....: pi_es_rpt_compl_impto_retid_ap
** Descricao.............: pi_es_rpt_compl_impto_retid_ap
** Criado por............: Uno
** Criado em.............: 27/05/1996 08:23:26
** Alterado por..........: log43291
** Alterado em...........: 12/03/2010 09:06:43
*****************************************************************************/
PROCEDURE pi_es_rpt_compl_impto_retid_ap:
    ...
    if  v_ind_classif_rpt_impto_retid_ap = ("Por Fornecedor Origem/Por Estab" /*l_por_fornecedor_origempor_estab*/ ) then do:
        ...
        for each tt_compl_impto_retid_ap
            ...
            then do:
                ...
                if (line-counter(s_1) + 3) > v_rpt_s_1_bottom then
                    page stream s_1.
                put stream s_1 unformatted
                    "Estab Esp SÇrie T°tulo           /P      Fornec Esp SÇrie T°tulo           /P Dat Emis Recolhim Dat Tran Rend T  Aliq Class      Rend Tribut Vl Impto Orig    Val Sdo Impto Impto IE Pa°s Recolher" SKIP
                    "----- --- ----- ---------------- -- ----------- --- ----- ---------------- -- -------- -------- -------- ------ ----- ----- ---------------- ------------- ---------------- ------------- --------" SKIP.
        ...
        put stream s_1 unformatted 
                ...
                tt_compl_impto_retid_ap.ttv_dat_trans at 97 format "99/99/99".
        put stream s_1 unformatted 
                tt_compl_impto_retid_ap.ttv_cod_indic_econ_rdt at 106 format "x(6)"
                tt_compl_impto_retid_ap.tta_val_aliq_impto to 117 format ">9.99"
                tt_compl_impto_retid_ap.tta_cod_classif_impto at 119 format "x(05)"
                tt_compl_impto_retid_ap.tta_val_rendto_tribut to 140 format ">,>>>,>>>,>>9.99"
                tt_compl_impto_retid_ap.ttv_val_origin_tit_ap_2 to 154 format ">,>>>,>>9.99"
                tt_compl_impto_retid_ap.ttv_val_imposto_rel to 171 format ">,>>>,>>>,>>9.99"
                tt_compl_impto_retid_ap.ttv_val_impto_indic_econ_pais to 185 format ">,>>>,>>9.99"
                tt_compl_impto_retid_ap.ttv_log_recolh at 187 format "Sim/N∆o" skip.
            ...
            then do:
                ...
                then do:
                    ...
                    put stream s_1 unformatted 
                        ...
                        "----------------" at 156
                        "-------------" at 173 skip
                        "Totais Classificaá∆o do Imposto:" at 86
                        v_val_tot_classif_impto[1] to 140 format ">,>>>,>>>,>>9.99"
                        v_val_tot_classif_impto[4] to 154 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                        v_val_tot_classif_impto[2] to 171 format ">,>>>,>>>,>>9.99"
                        v_val_tot_classif_impto[3] to 185 format ">,>>>,>>9.99" skip.
            ...
            then do:
                ...
                then do:
                    ...
                    put stream s_1 unformatted 
                        ...
                        "----------------" at 156
                        "-------------" at 173 skip
                        "Totais Fornecedor Favorecido:" at 89
                        v_val_tot_fornec_favorec[1] to 140 format ">,>>>,>>>,>>9.99"
                        v_val_tot_fornec_favorec[4] to 154 format ">,>>>,>>9.99".
        put stream s_1 unformatted                        
                        v_val_tot_fornec_favorec[2] to 171 format ">,>>>,>>>,>>9.99"
                        v_val_tot_fornec_favorec[3] to 185 format ">,>>>,>>9.99" skip.
                    assign v_val_tot_fornec_favorec[1] = 0
                           v_val_tot_fornec_favorec[2] = 0
                           v_val_tot_fornec_favorec[3] = 0                              
                           v_val_tot_fornec_favorec[4] = 0                              
            ...
            then do:
                ...
                then do:
                    ...
                    put stream s_1 unformatted 
                        ...
                        "----------------" at 156
                        "-------------" at 173 skip
                        "Totais Fornecedor Origem:" at 93 
                        v_val_tot_fornec_orig[1] to 140 format ">,>>>,>>>,>>9.99"
                        v_val_tot_fornec_orig[4] to 154 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                        v_val_tot_fornec_orig[2] to 171 format ">,>>>,>>>,>>9.99"
                        v_val_tot_fornec_orig[3] to 185 format ">,>>>,>>9.99" skip.
        ...
        &ENDIF skip.
                ...
                then do:
                    ...
                    if (line-counter(s_1) + 3) > v_rpt_s_1_bottom then
                        page stream s_1.
                    put stream s_1 unformatted
                        "Esp SÇrie T°tulo           /P      Fornec Esp SÇrie T°tulo           /P Dat Emis   Recolhim   Dat Trans  Rend Tri   Aliq Class      Rend Tribut Vl Impto Orig    Val Sdo Impto Impto IE Pa°s Recolher" SKIP
                        "--- ----- ---------------- -- ----------- --- ----- ---------------- -- ---------- ---------- ---------- -------- ------ ----- ---------------- ------------- ---------------- ------------- --------" SKIP.
                ...
                then do:
                    ...
                    if (line-counter(s_1) + 3) > v_rpt_s_1_bottom then
                        page stream s_1.
                    put stream s_1 unformatted 
                        "   Fornec Esp SÇrie T°tulo          /P    Fornec Esp SÇrie T°tulo          /P Dat Emis Recolhim Dat Tran Rend T   Aliq Class      Rend Tribut Vl Impto Orig    Val Sdo Impto Impto IE Pa°s Recolher" SKIP
                        "--------- --- ----- ------------------ --------- --- ----- ------------------ -------- -------- -------- ------ ------ ----- ---------------- ------------- ---------------- ------------- --------" SKIP.
                ...
                then do:
                    ...
                    if (line-counter(s_1) + 3) > v_rpt_s_1_bottom then
                        page stream s_1.
                    put stream s_1 unformatted 
                       "Esp SÇrie T°tulo           /P      Fornec Esp SÇrie T°tulo           /P Dat Emis   Recolhim   Dat Tran Rend Tri   Aliq Class      Rend Tribut Vl Impto Orig   Val Sdo Impto  Impto IE Pa°s Recolher" SKIP
                       "--- ----- ---------------- -- ----------- --- ----- ---------------- -- ---------- ---------- -------- -------- ------ ----- ---------------- ------------- ---------------  ------------- --------" SKIP.
                end /* if */.
        ...
        put stream s_1 unformatted 
                    ...
                    tt_compl_impto_retid_ap.ttv_dat_trans at 95 format "99/99/99"
                    tt_compl_impto_retid_ap.ttv_cod_indic_econ_rdt at 106 format "x(8)"
                    tt_compl_impto_retid_ap.tta_val_aliq_impto to 120 format ">9.99"
                    tt_compl_impto_retid_ap.tta_cod_classif_impto at 122 format "x(05)"
                    tt_compl_impto_retid_ap.tta_val_rendto_tribut to 143 format ">,>>>,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_val_origin_tit_ap_2 to 157 format ">,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_val_imposto_rel to 174 format ">,>>>,>>>,>>9.99".
        put stream s_1 unformatted 
                    tt_compl_impto_retid_ap.ttv_val_impto_indic_econ_pais to 188 format ">,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_log_recolh at 190 format "Sim/N∆o" 
                     skip.
            end /* if */.
            if  v_ind_classif_rpt_impto_retid_ap = "Por Classificaá∆o do Imposto" /*l_por_classificacao_do_imposto*/ 
            then do:
        ...
        put stream s_1 unformatted 
                    ...
                    tt_compl_impto_retid_ap.ttv_dat_trans at 97 format "99/99/99"
                    tt_compl_impto_retid_ap.ttv_cod_indic_econ_rdt at 106 format "x(6)"
                    tt_compl_impto_retid_ap.tta_val_aliq_impto to 118 format ">9.99"
                    tt_compl_impto_retid_ap.tta_cod_classif_impto at 120 format "x(05)"
                    tt_compl_impto_retid_ap.tta_val_rendto_tribut to 141 format ">,>>>,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_val_origin_tit_ap_2 to 155 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                    tt_compl_impto_retid_ap.ttv_val_imposto_rel to 172 format ">,>>>,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_val_impto_indic_econ_pais to 186 format ">,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_log_recolh at 188 format "Sim/N∆o" skip.
            end /* if */.
            if  v_ind_classif_rpt_impto_retid_ap = "Por Estab/Fornecedor Favorecido" /*l_por_estabfornec_favorec*/ 
            then do:
                if (line-counter(s_1) + 1) > v_rpt_s_1_bottom then
        ...
        put stream s_1 unformatted 
                    ...
                    tt_compl_impto_retid_ap.ttv_dat_recolhto at 84 format "99/99/9999"
                    tt_compl_impto_retid_ap.ttv_dat_trans at 95 format "99/99/99"
                    tt_compl_impto_retid_ap.ttv_cod_indic_econ_rdt at 104 format "x(8)"
                    tt_compl_impto_retid_ap.tta_val_aliq_impto to 118 format ">9.99"
                    tt_compl_impto_retid_ap.tta_cod_classif_impto at 120 format "x(05)"
                    tt_compl_impto_retid_ap.tta_val_rendto_tribut to 141 format ">,>>>,>>>,>>9.99"
                    tt_compl_impto_retid_ap.ttv_val_origin_tit_ap_2 to 155 format ">,>>>,>>9.99"
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 157
                            "-------------" at 174 skip
                            "Totais Fornecedor Origem:" at 94
                            v_val_tot_fornec_orig[1] to 141 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_orig[4] to 155 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_fornec_orig[2] to 172 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_orig[3] to 186 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_fornec_orig[1] = 0
                               v_val_tot_fornec_orig[2] = 0
                               v_val_tot_fornec_orig[3] = 0                              
                               v_val_tot_fornec_orig[4] = 0                              
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 159
                            "-------------" at 176 skip
                            "Totais Classificaá∆o do Imposto:" at 89
                            v_val_tot_classif_impto[1] to 143 format ">,>>>,>>>,>>9.99"
                            v_val_tot_classif_impto[4] to 157 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_classif_impto[2] to 174 format ">,>>>,>>>,>>9.99"
                            v_val_tot_classif_impto[3] to 188 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_classif_impto[1] = 0
                               v_val_tot_classif_impto[2] = 0
                               v_val_tot_classif_impto[3] = 0         
                               v_val_tot_classif_impto[4] = 0         
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 157
                            "-------------" at 174 skip
                            "Totais Fornecedor Favorecido:" at 90
                            v_val_tot_fornec_favorec[1] to 141 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_favorec[4] to 155 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_fornec_favorec[2] to 172 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_favorec[3] to 186 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_fornec_favorec[1] = 0
                               v_val_tot_fornec_favorec[2] = 0
                               v_val_tot_fornec_favorec[3] = 0                              
                               v_val_tot_fornec_favorec[4] = 0                              
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 159
                            "-------------" at 176 skip
                            "Totais Fornecedor Favorecido:" at 92
                            v_val_tot_fornec_favorec[1] to 143 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_favorec[4] to 157 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_fornec_favorec[2] to 174 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_favorec[3] to 188 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_fornec_favorec[1] = 0
                               v_val_tot_fornec_favorec[2] = 0
                               v_val_tot_fornec_favorec[3] = 0
                               v_val_tot_fornec_favorec[4] = 0                                                                                   
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 157
                            "-------------" at 174 skip
                            "Totais Classificaá∆o do Imposto:" at 87
                            v_val_tot_classif_impto[1] to 141 format ">,>>>,>>>,>>9.99"
                            v_val_tot_classif_impto[4] to 155 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_classif_impto[2] to 171 format ">,>>>,>>>,>>9.99"
                            v_val_tot_classif_impto[3] to 186 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_classif_impto[1] = 0
                               v_val_tot_classif_impto[2] = 0
                               v_val_tot_classif_impto[3] = 0                              
                               v_val_tot_classif_impto[4] = 0                              
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            v_val_tot_classif_impto[4] to 155 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_classif_impto[2] to 172 format ">,>>>,>>>,>>9.99"
                            v_val_tot_classif_impto[3] to 186 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_classif_impto[1] = 0
                               v_val_tot_classif_impto[2] = 0
                               v_val_tot_classif_impto[3] = 0                              
                ...
                then do:
                    ...
                    then do:
                        ...
                        put stream s_1 unformatted 
                            ...
                            "----------------" at 159
                            "-------------" at 176 skip
                            "Totais Fornecedor Origem:" at 96
                            v_val_tot_fornec_orig[1] to 143 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_orig[4] to 157 format ">,>>>,>>9.99".
        put stream s_1 unformatted 
                            v_val_tot_fornec_orig[2] to 174 format ">,>>>,>>>,>>9.99"
                            v_val_tot_fornec_orig[3] to 188 format ">,>>>,>>9.99" skip.
                        assign v_val_tot_fornec_orig[1] = 0
                               v_val_tot_fornec_orig[2] = 0
                               v_val_tot_fornec_orig[3] = 0                              
                               v_val_tot_fornec_orig[4] = 0                              
...
END PROCEDURE. /* pi_es_rpt_compl_impto_retid_ap */
/*****************************************************************************
...
PROCEDURE pi_cria_tt_compl_impto_retid_ap:
    ...
    if avail tit_ap then do:
        ...
        ASSIGN tt_compl_impto_retid_ap.ttv_dat_trans                = tit_ap.dat_transacao.
            ...
            if tit_ap.val_sdo_tit_ap = 0 then do:
                ...
                for each movto_tit_ap no-lock
                   where movto_tit_ap.cod_estab     = tit_ap.cod_estab
                    ...
                    then do:
                        assign tt_compl_impto_retid_ap.ttv_log_recolh = YES.
                        ASSIGN tt_compl_impto_retid_ap.ttv_dat_trans    = movto_tit_ap.dat_transacao.
                        leave.
                    end.
                end.
            end.
...
/*********************  End of es_rpt_compl_impto_retid_ap *********************/
