...
**       Data....: 30/03/12
**       Autor...: DATASUL S.A.
**       Objetivo: CATµLOGO DE ITENS
**       VersÆo..: 1.00.000 - adm
**       OBS.....: Este fonte foi gerado pelo Data Viewer 3.00
*******************************************************************************/

define variable c-prog-gerado as character no-undo initial "ITEM0127".

def new global shared var c-arquivo-log    as char  format "x(60)"no-undo.
def new global shared var c-prg-vrs as char no-undo.
def new global shared var c-prg-obj as char no-undo.
run grapi/gr2013.p (input c-prog-gerado, input "2.06.00.001").
...
def new shared var c-cod-estabel-ini like item.cod-estabel format "x(3)" initial "" no-undo.
def new shared var c-cod-estabel-fim like item.cod-estabel format "x(3)" initial "ZZZ" no-undo.
...
form item.it-codigo column-label "Item" format "x(12)" at 001
     item.desc-item column-label "Descri‡Æo" format "x(60)" at 018
     item.un column-label "Un" format "xx" at 079
     item.fm-codigo column-label "Fam¡lia" format "x(8)" at 082
     item.codigo-refer column-label "C¢digo Complementar" format "x(20)" at 091
     item.inform-compl column-label "Inf Compl" format "x(16)" at 112
     item.cod-localiz column-label "Localiz" format "x(10)" at 129
     with down width 142 no-box stream-io frame f-relat-09-132.
...
assign c-programa     = "ITEM0127"
       c-versao       = "2.06"
       c-revisao      = ".00.000"
       c-titulo-relat = "CATµLOGO DE ITENS"
       c-sistema      = "EMS".
...
form header
    fill("-", 142) format "x(132)" skip
    c-empresa c-titulo-relat at 50
    "Folha:" at 122 page-number(str-rp) at 128 format ">>>>9" skip
    fill("-", 112) format "x(110)" today format "99/99/9999"
    "-" string(time, "HH:MM:SS") skip(1)
    with stream-io width 142 no-labels no-box page-top frame f-cabec.
form header
    fill("-", 142) format "x(132)" skip
    c-empresa c-titulo-relat at 50
    "Folha:" at 122 page-number(str-rp) at 128 format ">>>>9" skip
    ...
    "-" string(time, "HH:MM:SS") skip(1)
    with stream-io width 142 no-labels no-box page-top frame f-cabper.
run grapi/gr2004.p.
form header
    c-rodape format "x(142)"
    with stream-io width 142 no-labels no-box page-bottom frame f-rodape.
...
run pi-inicializar in h-acomp(input "C¢digo").
...
for each item no-lock
         where item.cod-estabel >= c-cod-estabel-ini and 
               item.cod-estabel <= c-cod-estabel-fim and
...
if  tt-param.parametro then do:
   ...
   disp stream str-rp 
      c-fm-codigo-ini colon 28 "|< >|"   at 52 c-fm-codigo-fim no-label
      c-it-codigo-ini colon 28 "|< >|"   at 52 c-it-codigo-fim no-label
      c-cod-estabel-ini colon 28 "|< >|"   at 52 c-cod-estabel-fim no-label
