...
DEFINE INPUT PARAMETER c-diretorio      AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER c-diretorio-dest AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER c-empresa        AS CHAR NO-UNDO.
...
IF CAN-FIND (FIRST tt-erros-leitura-xml )THEN DO:
    ...
    OUTPUT TO value(session:TEMP-DIRECTORY + "erros-leitura.csv").
    ...
    MESSAGE "Foi criado um arquivo de erro em: " SESSION:TEMP-DIRECTORY + "erros-leitura.csv"
    ...
    FOR EACH tt-nfe-item,
        ...
        IF AVAIL  tt-item-doc-est-used THEN DO:
            ...
            ASSIGN tt-planilha.cod-estabel  = docum-est.cod-estabel
                   ...
                   tt-planilha.it-codigo    = IF avail(es-de-para-it-padr) THEN es-de-para-it-padr.it-codigo-padr ELSE tt-item-doc-est-used.it-codigo   
        ...
        ELSE DO:
            ...
            IF AVAIL tt-item-doc-est-used THEN DO:
                ...
                FIND FIRST es-de-para-it-padr no-lock
                     WHERE es-de-para-it-padr.ep-codigo      = c-empresa
                       AND es-de-para-it-padr.it-codigo-orig = tt-item-doc-est-used.it-codigo NO-ERROR.
