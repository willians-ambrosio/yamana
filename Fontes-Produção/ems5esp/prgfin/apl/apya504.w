...
*******************************************************************************/
{include/i-prgvrs.i APYA504 12.1.17.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-adm 
...
DEFINE VARIABLE h_apya504-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_apya504-v01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadsim AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-adm 
     IMAGE-UP FILE "prgfin/image/list_adm.png":U
     LABEL "Button 6" 
     SIZE 4 BY 1.13 TOOLTIP "Relaciona Administradora ao produto financeiro.".
...
DEFINE FRAME f-cad
     bt-adm AT ROW 1.38 COL 57.43 WIDGET-ID 2
     rt-button AT ROW 1.21 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1.14 ROW 1
         SIZE 90 BY 8.46 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Produto Financeiro Intercompany"
         HEIGHT             = 8.58
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90.29
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Produto Financeiro Intercompany */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Produto Financeiro Intercompany */
DO:
...
&Scoped-define SELF-NAME bt-adm
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-adm w-cadsim
ON CHOOSE OF bt-adm IN FRAME f-cad /* Button 6 */
DO:
  RUN pi-retorna-rowid IN h_apya504-q01 (OUTPUT v-row-parent). 


  RUN prgfin\apl\apya504a.w (INPUT v-row-parent).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya504-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_apya504-v01 ).
       RUN set-position IN h_apya504-v01 ( 2.79 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 6.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya504-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = prgfin/apl/apya504-z01.w,
                     ProgVaPara = prgfin/apl/apya504-g01.w,
                     Implantar = no':U ,
             OUTPUT h_apya504-q01 ).
       RUN set-position IN h_apya504-q01 ( 5.25 , 76.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_apya504-v01. */
       RUN add-link IN adm-broker-hdl ( h_apya504-q01 , 'Record':U , h_apya504-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_apya504-v01 ).

       /* Links to SmartQuery h_apya504-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_apya504-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_apya504-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_apya504-q01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-adm:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsim ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_apya504-v01 ,
             bt-adm:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "APYA504" "12.1.17.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  RUN enable-copia IN h_p-cadsim (NO).
