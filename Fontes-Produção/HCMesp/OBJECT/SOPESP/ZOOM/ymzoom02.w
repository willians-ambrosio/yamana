...
{include/i-prgvrs.i YMZOOM02 1.00.00.000}
...
DEFINE VARIABLE h_ymbrw002c AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 14.38 COL 1.57
     bt-ok AT ROW 15.71 COL 3
     bt-cancela AT ROW 15.71 COL 14
     bt-ajuda AT ROW 15.71 COL 79
     rt-button AT ROW 15.46 COL 2
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 13.08 , 88.57 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'object/sopesp/brw/ymbrw002c.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymbrw002c ).
       RUN set-position IN h_ymbrw002c ( 2.46 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.25 , 80.14 ) */

       /* Links to SmartBrowser h_ymbrw002c. */
       RUN add-link IN adm-broker-hdl ( h_ymbrw002c , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-implantar:HANDLE IN FRAME f-zoom , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymbrw002c ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMZOOM02" "1.00.00.000"}
...
-------------------------------------------------------------*/
  ...
  RUN "object\sopesp\brw\ymbrw002c.w *RTB-SmObj* ".
