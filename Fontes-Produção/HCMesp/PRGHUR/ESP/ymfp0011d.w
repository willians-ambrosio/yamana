...
*******************************************************************************/
{include/i-prgvrs.i ymfp0011D 1.02.10.000 } /*** 01010002 ***/
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ymfp0011D MFP}
&ENDIF
...
DEFINE FRAME f-cad
     bt-ok AT ROW 4.92 COL 3.14 HELP
          "Salva e sai"
     bt-save AT ROW 4.92 COL 14.14 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 4.92 COL 25.14 HELP
          "Cancela"
     bt-ajuda AT ROW 4.92 COL 78.14
     rt-button AT ROW 4.75 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-incmdp ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Métricas PLR"
         HEIGHT             = 5.5
         WIDTH              = 90
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 114.29
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_ymvwr003 ).
       RUN set-position IN h_ymvwr003 ( 1.33 , 1.43 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_ymqry006 ).
       RUN set-position IN h_ymqry006 ( 1.00 , 69.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.75 , 5.86 ) */

       /* Links to SmartViewer h_ymvwr003. */
       RUN add-link IN adm-broker-hdl ( h_ymqry006 , 'Record':U , h_ymvwr003 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_ymvwr003 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ymfp0011D" "1.02.10.000"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_ymqry006 (INPUT v-row-table). 
