...
{include/i-prgvrs.i AP0804F-B05 2.00.00.003}  /*** 010003 ***/
/*------------------------------------------------------------------------
...
&glob ORIGINALNAME 'adbrw\b05ad180.w'
...
def var i-lancamento  as integer initial 2 no-undo.

def buffer b-mov-ap  for mov-ap.
def buffer b2-mov-ap for mov-ap.
def buffer b-mov-ap-aux for mov-ap.

def temp-table tt-mov-ap like mov-ap
    field c-transacao         as char format "X(5)"
    field c-lancamento        like mov-ap.lancamento
    field gr-tt-doc           as rowid.

def var de-saldo-cmi      LIKE tit-ap.vl-saldo no-undo.
def var de-saldo-vl-pres  LIKE tit-ap.vl-saldo no-undo.
...
&Scoped-define FIELDS-IN-QUERY-br-table entry(tt-mov-ap.transacao, c-lista-trans) @ c-transacao tt-mov-ap.dt-transacao tt-mov-ap.dt-vencimen entry(fn-lancamento(), c-lista-lanc) @ c-lancamento tt-mov-ap.referencia tt-mov-ap.vl-desc-cmi tt-mov-ap.vl-juros-cmi tt-mov-ap.vl-ant-cmi tt-mov-ap.valor-presente tt-mov-ap.acerto-cmi tt-mov-ap.ganho-perda fn-saldo-cmi() @ de-saldo-cmi fn-saldo-vl-pres() @ de-saldo-vl-pres   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table   
&Scoped-define FIELD-PAIRS-IN-QUERY-br-table
&Scoped-define SELF-NAME br-table
&Scoped-define OPEN-QUERY-br-table if l-modulo-fc = yes then    run pi-criar-tt-mov-ap.  OPEN QUERY {&SELF-NAME} FOR EACH tt-mov-ap      WHERE tt-mov-ap.transacao <> 8      AND   l-modulo-fc = yes NO-LOCK     ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fn-lancamento B-table-Win 
FUNCTION fn-lancamento RETURNS INTEGER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-table B-table-Win _FREEFORM
  QUERY br-table NO-LOCK DISPLAY
      entry(tt-mov-ap.transacao, c-lista-trans) @ c-transacao
      tt-mov-ap.dt-transacao
      tt-mov-ap.dt-vencimen
      entry(fn-lancamento(), c-lista-lanc) @ c-lancamento
      tt-mov-ap.referencia
      tt-mov-ap.vl-desc-cmi    FORMAT "->>>>>>>,>>9.99"
      tt-mov-ap.vl-juros-cmi   FORMAT "->>>>>>>,>>9.99"
      tt-mov-ap.vl-ant-cmi     FORMAT "->>>>>>>,>>9.99"
      tt-mov-ap.valor-presente FORMAT "->>>>>>>,>>9.99"
      tt-mov-ap.acerto-cmi     FORMAT "->>>,>>>,>>>,>>9.99"
      tt-mov-ap.ganho-perda
      fn-saldo-cmi() @ de-saldo-cmi
      fn-saldo-vl-pres() @ de-saldo-vl-pres
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 86.29 BY 8.67.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-Main
     br-table AT ROW 1 COL 1.57
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-mov-ap
     WHERE tt-mov-ap.transacao <> 8
     AND   l-modulo-fc = yes NO-LOCK
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "mov-ap.transacao <> 8 
and l-modulo-fc = yes"
...
{utp/ut-liter.i Saldo_CMCAC}
assign de-saldo-cmi:label in browse {&browse-name} = return-value.

{utp/ut-liter.i Saldo_Vl_Presente}
assign de-saldo-vl-pres:label in browse {&browse-name} = return-value.

{utp/ut-field.i mgadm mov-ap transacao  1}
assign c-transacao:label in browse br-table = return-value.

{utp/ut-field.i mgadm mov-ap lancamento 1}
...
------------------------------------------------------------------------------*/
    ...
    if  avail tit-ap then do: 
        ...
        if  tit-ap.tipo <> 2 then do:
            ...
            assign da-fim-per  = tit-ap.dt-emissao
                   da-fim-per1 = da-fim-per.
                            
            if  tt-mov-ap.transacao = 9 then do:
                if  tt-mov-ap.dt-trans > da-fim-per then 
                    assign da-fim-per = tt-mov-ap.dt-trans.
            end.
                
            if  da-fim-per <> tit-ap.dt-emissao then 
                assign da-fim-per1 = if  empresa.cmi-prim-ult = 2 then
                                         da-fim-per
                                     else
                                         da-fim-per + 1.
                               
            if  tit-ap.dt-vecto-orig <> ? then 
                assign da-vencimen =  tit-ap.dt-vecto-orig.
            else
                assign da-vencimen = tit-ap.dt-vencimen.
                            
            if  da-fim-per > da-vencimen then
                assign de-cotacao-anbid = 1.
            else 
                assign de-cotacao-anbid = exp((if  tit-ap.taxa-emis-anbid = 0 then 1 
                                               else tit-ap.taxa-emis-anbid),
                                               (da-vencimen - da-fim-per)).
                        
            if  de-cotacao-anbid = 0
            or  de-cotacao-anbid = ? then 
                assign de-cotacao-anbid = 1.
                        
            if  da-fim-per1 <> tit-ap.dt-emissao then do:
                find param-fasb where param-fasb.ep-codigo = tit-ap.ep-codigo no-lock no-error.
                {cdp/cd9600.i "param-fasb.moeda-cmi"
                              "da-fim-per1"
                              "de-cotacao"} 
            end.                                  
            else
               assign de-cotacao = tit-ap.taxa-emis-cmi.
                            
            if  de-cotacao = 0
            or  de-cotacao = ? then 
                assign de-cotacao = 1.
                    
            assign de-saldo-vl-pres = tit-ap.valor-saldo / de-cotacao-anbid
                   de-saldo-cmi     = de-saldo-vl-pres   / de-cotacao.
        end.
        
        if  de-saldo-cmi < 0 then
            assign de-saldo-cmi = 0.
        
        if  de-saldo-vl-pres < 0 then
    ...
    for each  mov-ap
            ...
            for each  b-mov-ap-aux use-index ch-antecip
                ...
                if  not avail tt-mov-ap then do:
                    ...
                    assign tt-mov-ap.transacao         = b-mov-ap-aux.transacao
                           ...
                           tt-mov-ap.referencia        = b-mov-ap-aux.referencia
                           tt-mov-ap.acerto-cmi        = b-mov-ap-aux.acerto-cmi
                           tt-mov-ap.ganho-perda       = b-mov-ap-aux.ganho-perda
                           tt-mov-ap.vl-desc-cmi       = b-mov-ap-aux.vl-desc-cmi
                           tt-mov-ap.vl-juros-cmi      = b-mov-ap-aux.vl-juros-cmi
                           tt-mov-ap.vl-ant-cmi        = b-mov-ap-aux.vl-ant-cmi
                           tt-mov-ap.valor-presente    = b-mov-ap-aux.valor-presente
        ...
        if  not avail tt-mov-ap then do:           
            ...
            assign tt-mov-ap.transacao         = mov-ap.transacao
                   ...
                   tt-mov-ap.referencia        = mov-ap.referencia
                   tt-mov-ap.acerto-cmi        = mov-ap.acerto-cmi
                   tt-mov-ap.ganho-perda       = mov-ap.ganho-perda
                   tt-mov-ap.vl-desc-cmi       = mov-ap.vl-desc-cmi
                   tt-mov-ap.vl-juros-cmi      = mov-ap.vl-juros-cmi
                   tt-mov-ap.vl-ant-cmi        = mov-ap.vl-ant-cmi
                   tt-mov-ap.valor-presente    = mov-ap.valor-presente
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fn-lancamento B-table-Win 
FUNCTION fn-lancamento RETURNS INTEGER
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
    if tt-mov-ap.transacao = 2 then
       assign i-lancamento = 2.
    else
       assign i-lancamento = tt-mov-ap.lancamento.
  RETURN i-lancamento.   /* Function return value. */
