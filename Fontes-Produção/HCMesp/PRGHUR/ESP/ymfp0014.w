...
*******************************************************************************/
{include/i-prgvrs.i YMFP0014 1.02.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ymfp0014 mfp}
&ENDIF
...
DEFINE VARIABLE v_arquivo_importacao AS CHARACTER 
     VIEW-AS EDITOR MAX-CHARS 256
     SIZE 44.72 BY .88
     BGCOLOR 15  NO-UNDO.

DEFINE VARIABLE text-entrada AS CHARACTER FORMAT "X(256)":U INITIAL "Arquivo Importa‡Æo" 
      VIEW-AS TEXT 
...
DEFINE VARIABLE v_importa AS INTEGER INITIAL 1 
...
DEFINE VARIABLE v_fim_cdn_estab AS CHARACTER FORMAT "x(5)" INITIAL "ZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 9.14 BY .88 NO-UNDO.

DEFINE VARIABLE v_fim_cdn_funcionario AS INTEGER FORMAT "zzzzzzz9" INITIAL 99999999 
     VIEW-AS FILL-IN 
     SIZE 10.29 BY .88 NO-UNDO.

DEFINE VARIABLE v_ini_cdn_estab AS CHARACTER FORMAT "x(5)" 
     LABEL "Estabelecimento" 
     VIEW-AS FILL-IN 
     SIZE 9.14 BY .88 NO-UNDO.

DEFINE VARIABLE v_ini_cdn_funcionario AS INTEGER FORMAT "zzzzzzz9" INITIAL 0 
     LABEL "Matr¡cula" 
     VIEW-AS FILL-IN 
     SIZE 10.29 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     v_ini_cdn_estab AT ROW 4.75 COL 19 COLON-ALIGNED HELP
          "C¢digo N£mero Estabelecimento" WIDGET-ID 2
     v_fim_cdn_estab AT ROW 4.75 COL 51 HELP
          "C¢digo N£mero Estabelecimento" NO-LABEL WIDGET-ID 10
     v_ini_cdn_funcionario AT ROW 5.75 COL 19 COLON-ALIGNED HELP
          "N£mero da matr¡cula do funcion rio" WIDGET-ID 4
     v_fim_cdn_funcionario AT ROW 5.75 COL 51 HELP
          "N£mero da matr¡cula do funcion rio" NO-LABEL WIDGET-ID 12
     IMAGE-1 AT ROW 4.75 COL 31.86 WIDGET-ID 14
     IMAGE-2 AT ROW 4.75 COL 47.29 WIDGET-ID 16
     IMAGE-21 AT ROW 5.75 COL 31.86 WIDGET-ID 6
     IMAGE-22 AT ROW 5.75 COL 47.29 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
...
DEFINE FRAME f-pg-par
     ...
     v_importa AT ROW 4 COL 18 NO-LABEL WIDGET-ID 14
     v_arquivo_importacao AT ROW 8.79 COL 15.29 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL WIDGET-ID 34
     bt-arquivo-entrada AT ROW 8.79 COL 59.86 HELP
          "Escolha do nome do arquivo" WIDGET-ID 32
     text-entrada AT ROW 7.75 COL 16.14 NO-LABEL WIDGET-ID 38
     "Importar" VIEW-AS TEXT
          SIZE 9 BY .67 AT ROW 3.17 COL 18 WIDGET-ID 12
     RECT-8 AT ROW 3.5 COL 14 WIDGET-ID 10
     RECT-11 AT ROW 8.08 COL 14 WIDGET-ID 36
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
/* SETTINGS FOR FRAME f-pg-sel
                                                                        */
/* SETTINGS FOR FILL-IN v_fim_cdn_estab IN FRAME f-pg-sel
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN v_fim_cdn_funcionario IN FRAME f-pg-sel
   ALIGN-L                                                              */
...
DO:
    ...
    assign c-arq-conv = replace(input frame {&frame-name} v_arquivo_importacao, "/", "~\").
    ...
    SYSTEM-DIALOG GET-FILE c-arq-conv
       ...
       UPDATE l-ok.
    if  l-ok = yes then do:
        assign v_arquivo_importacao = replace(c-arq-conv, "~\", "/").
        display v_arquivo_importacao with frame {&frame-name}.
    end.  
END.
...
&Scoped-define SELF-NAME v_importa
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v_importa w-relat
ON VALUE-CHANGED OF v_importa IN FRAME f-pg-par
DO:
    assign v_arquivo_importacao = fn_nome_arquivo().

    disp v_arquivo_importacao with frame f-pg-par.  
END.
...
{utp/ut9000.i "YMFP0014" "1.02.00.000"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY v_ini_cdn_estab v_fim_cdn_estab v_ini_cdn_funcionario 
          v_fim_cdn_funcionario 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-21 IMAGE-22 v_ini_cdn_estab v_fim_cdn_estab 
         v_ini_cdn_funcionario v_fim_cdn_funcionario 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY v_cdn_param_calc_ppr v_des_param v_num_mes_refer_calc_efetd 
          v_num_ano_refer_calc_efetd v_importa v_arquivo_importacao 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-8 RECT-11 v_cdn_param_calc_ppr v_num_mes_refer_calc_efetd 
         v_num_ano_refer_calc_efetd v_importa v_arquivo_importacao 
         bt-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    if input frame {&frame-name} v_arquivo_importacao = "" or input frame {&frame-name} v_arquivo_importacao = ? then do:

        run utp/ut-msgs.p (input "show":U, input 265, 
                           input text-entrada:input-value in frame {&frame-name}).
        
        apply "MOUSE-SELECT-CLICK":U to im-pg-par in frame f-relat.
        apply "ENTRY":U to v_arquivo_importacao in frame {&frame-name}.
        return error.
    ...
    assign tt-param.tt_cdn_param_calc_ppr       = input frame {&frame-name} v_cdn_param_calc_ppr      
           ...
           tt-param.tt_ind_tip_importa          = input frame {&frame-name} v_importa         
           tt-param.tt_des_ind_importa          = entry((tt-param.tt_ind_tip_importa - 1) * 2 + 1, 
                                                        v_importa:radio-buttons in frame {&frame-name})                            
           tt-param.tt_arquivo_importa          = input frame {&frame-name} v_arquivo_importacao.         
    ...
    {include/i-rprun.i prghur/esp/ymfp0014rp.p}
