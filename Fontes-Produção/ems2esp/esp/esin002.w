...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "ESIN002"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    /*Altera‡Æo - 08/09/2006 - tech1007 - Altera‡Æo para exibir o nome do programa que executou o programa que ser  exibido no extrato de versÆo
        ...
        PUT "ESIN002" AT 1 "2.00.00.000" AT 69 STRING(TODAY,'99/99/99') AT 84 STRING(TIME,'HH:MM:SS':U) AT 94 SKIP.
        /*FO 1329.898 - tech1139 - 01/08/2006 */
...
DEFINE VARIABLE dt-trans-fim AS DATE FORMAT "99/99/9999":U INITIAL ? 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE dt-trans-ini AS DATE FORMAT "99/99/9999":U INITIAL ? 
     LABEL "Data Movimenta‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE i-num-ordem-fim AS INTEGER FORMAT ">>9":U INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE i-num-ordem-ini AS INTEGER FORMAT ">>9":U INITIAL 0 
     LABEL "Ordem" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88
     FONT 1 NO-UNDO.

DEFINE VARIABLE i-num-projeto-fim AS INTEGER FORMAT ">>>>9":U INITIAL 99999 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.

DEFINE VARIABLE i-num-projeto-ini AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Projeto" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 NO-UNDO.
...
DEFINE FRAME fPage2
     i-num-ordem-ini AT ROW 1.5 COL 19.71
     i-num-ordem-fim AT ROW 1.5 COL 51 NO-LABEL
     i-num-projeto-ini AT ROW 2.5 COL 23 COLON-ALIGNED
     i-num-projeto-fim AT ROW 2.5 COL 49 COLON-ALIGNED NO-LABEL
     dt-trans-ini AT ROW 3.5 COL 23 COLON-ALIGNED
     dt-trans-fim AT ROW 3.5 COL 49 COLON-ALIGNED NO-LABEL
     IMAGE-1 AT ROW 1.5 COL 35.86
     IMAGE-2 AT ROW 1.5 COL 47
     IMAGE-3 AT ROW 2.5 COL 36
     IMAGE-4 AT ROW 2.5 COL 47.14
     IMAGE-5 AT ROW 3.5 COL 36
     IMAGE-6 AT ROW 3.5 COL 47.14
...
/* _UIB-CODE-BLOCK-END */
    ...
    /*--- Desabilita campos contidos no {&page2Fields} ---*/
        ...
        DO WITH FRAME fPage2:
            DISABLE i-num-ordem-ini   i-num-ordem-fim                               i-num-projeto-ini i-num-projeto-fim                               dt-trans-ini      dt-trans-fim.
        END.
    ...
    /*--- Desabilita widgets contidos no {&page6Widgets} ---*/
        ...
        DO WITH FRAME fPage6:
            DISABLE rsDestiny                               btConfigImpr                               btFile                               rsExecution                               l-habilitaRtf                               btModelRtf.
        END.
...
------------------------------------------------------------------------------*/
        ...
        DISPLAY text-destino text-modo text-rtf text-ModelRtf WITH FRAME fPage6.
...
/* _UIB-CODE-BLOCK-END */
    ...
    /*--- Habilita campos contidos no {&page2Fields} ---*/
        ...
        ENABLE i-num-ordem-ini   i-num-ordem-fim                               i-num-projeto-ini i-num-projeto-fim                               dt-trans-ini      dt-trans-fim WITH FRAME fPage2.
        DISPLAY i-num-ordem-ini   i-num-ordem-fim                               i-num-projeto-ini i-num-projeto-fim                               dt-trans-ini      dt-trans-fim WITH FRAME fPage2.
    ...
    /*--- Habilita campos contidos no {&page6Fields} ---*/
        ...
        ENABLE cFile cModelRTF WITH FRAME fPage6.
        DISPLAY cFile cModelRTF WITH FRAME fPage6.
    ...
    /*--- Habilita widgets contidos no {&page6Widgets} ---*/
        ...
        ENABLE rsDestiny                               btConfigImpr                               btFile                               rsExecution                               l-habilitaRtf                               btModelRtf WITH FRAME fPage6.
        DISPLAY rsDestiny                               btConfigImpr                               btFile                               rsExecution                               l-habilitaRtf                               btModelRtf WITH FRAME fPage6.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN c-programa-mg97 = CAPS("ESIN002":U)
...
/* SETTINGS FOR FRAME fPage2
                                                                        */
/* SETTINGS FOR FILL-IN i-num-ordem-fim IN FRAME fPage2
   ALIGN-L                                                              */
/* SETTINGS FOR FILL-IN i-num-ordem-ini IN FRAME fPage2
   ALIGN-L                                                              */
...
DO:
    ...
    assign cModelRTF = replace(input frame fPage6 cModelRTF, "/", "~\").
    SYSTEM-DIALOG GET-FILE cFile
       ...
       UPDATE l-ok.
    if  l-ok = yes then
        assign cModelRTF:screen-value in frame fPage6  = replace(cFile, "~\", "/"). 
...
assign wReport:virtual-width-chars  = wReport:width-chars
    ...
    if  avail usuar_mestre then 
        assign cFile = if  length(usuar_mestre.nom_subdir_spool) <> 0 then 
                           caps(replace(usuar_mestre.nom_dir_spool, "~\":U, "~/":U) + "~/":U + replace(usuar_mestre.nom_subdir_spool, "~\":U, "~/":U) + "~/":U + "ESIN002":U + "~.lst":U)
                       else caps(replace(usuar_mestre.nom_dir_spool, "~\":U, "~/":U) + "~/":U + "ESIN002":U + "~.lst":U)
               c-arq-old = cFile.
    else 
        assign cFile     = caps("spool~/":U + "ESIN002":U + "~.lst":U)
               c-arq-old = cFile.
...
/* Alterado em 14/06/2005 - tech1139 - Altera‡äes FO 1161.541*/
...
/* Alterado em 14/06/2005 - tech1139 - Altera‡äes FO 1161.541*/
    ...
    ON "~\":U OF cModelRtf IN FRAME fPage6 DO:
        APPLY "/":U TO cModelRtf IN FRAME fPage6.
        RETURN NO-APPLY.
    END.
    ...
    ON "VALUE-CHANGED":U OF rsDestiny IN FRAME fPage6 DO:
        CASE rsDestiny:SCREEN-VALUE IN FRAME fPage6:
            WHEN "1":U THEN DO:
                ...
                IF l-habilitaRtf:SCREEN-VALUE IN FRAME fPage6 <> "No" THEN DO:
                   ASSIGN l-rtf = YES
                          c-modelo-aux = cModelRtf:SCREEN-VALUE IN FRAME fPage6.
                END.
            ...
            WHEN "2":U THEN DO:
                ASSIGN cFile = IF rsExecution = 1 
                    ...
                    ASSIGN l-habilitaRtf:sensitive  = YES.               
                
                
                /* Alterado em 14/06/2005 - tech1139 - Altera‡äes FO 1161.541*/
                
                IF l-rtf = YES THEN DO:
                   ASSIGN l-habilitaRtf:SCREEN-VALUE IN FRAME fPage6 = "Yes"
                          l-rtf = NO
                          cModelRtf = c-modelo-aux
                          cModelRtf:SCREEN-VALUE IN FRAME fPage6 = c-modelo-aux.
                END.
...
/*Procedure criada para controlar quando o RTF foi ativado*/
    IF l-habilitaRtf:SCREEN-VALUE IN FRAME fPage6 <> "No" THEN DO:
        ASSIGN cModelRtf:sensitive  = YES
               btModelRtf:SENSITIVE  = YES.
        /* Alterado em 14/06/2005 - tech1139 - Altera‡äes FO 1161.541*/
        IF c-modelo-default <> "" THEN DO:
            IF  cModelRtf= "" THEN DO:
                ASSIGN cModelRtf = c-modelo-default
                       cModelRtf:SCREEN-VALUE IN FRAME fPage6 = c-modelo-default.
            END.
        END.
        /* Alterado em 14/06/2005 - tech1139 - Altera‡äes FO 1161.541*/
    END.
    ELSE DO:
        ASSIGN cModelRtf:sensitive  = NO
               cModelRtf:SCREEN-VALUE IN FRAME fPage6 = ""
               cModelRtf = ""
               btModelRtf:SENSITIVE  = NO.
    END.
END.
/*RUN pi-habilitaRtf.*/
...
assign dt-trans-ini = today
       dt-trans-fim = today.

disp dt-trans-ini
     dt-trans-fim with frame fPage2.
...
/* end of rpexa */
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.num-ordem-ini   = input frame fPage2 i-num-ordem-ini
           tt-param.num-ordem-fim   = input frame fPage2 i-num-ordem-fim
           tt-param.num-projeto-ini = input frame fPage2 i-num-projeto-ini
           tt-param.num-projeto-fim = input frame fPage2 i-num-projeto-fim
...
if rsExecution:screen-value in frame fPage6 = "2":U then do:
  ...
  run esp/esin002rp.p (input raw-param,
           input table tt-raw-digita).
end.
...
/* Alterado 14/06/2005 - tech1139 - Alterado para corrigir o erro 142 gerado quando se utiliza a op»’o RTF com o cursor no editor
