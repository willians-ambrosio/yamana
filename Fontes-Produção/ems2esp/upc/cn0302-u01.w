...
&Scoped-Define ENABLED-OBJECTS rt-buttom RECT-19 RECT-20 RECT-22 rs-exporta ~
...
DEFINE VARIABLE rs-exporta-2 AS CHARACTER INITIAL "A" 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Registro Atual", "A",
...
------------------------------------------------------------------------------*/
  DISPLAY rs-exporta rs-exporta-2 cInputFile 
      WITH FRAME D-Dialog.
  ENABLE rt-buttom RECT-19 RECT-20 RECT-22 rs-exporta rs-exporta-2 btInputFile 
         cInputFile bt-ok bt-cancela bt-ajuda 
      WITH FRAME D-Dialog.
...
DEFINE VARIABLE i-cont          AS INTEGER     NO-UNDO.
    ...
    IF NOT VALID-HANDLE(h-acomp ) THEN
    RUN utp/ut-acomp.p PERSISTENT SET h-acomp.
    {utp/ut-liter.i Processando arquivo...  *}
    ...
    /*DO i-aba = 1 TO 3:*/
        ...
        REPEAT:
            ...
            IF AVAIL tt-excel THEN
                   ASSIGN tt-excel.perc-rateioConta = tt-excel.perc-rateioConta + ROUND(DECIMAL(ChWorkSheet:RANGE("R" + STRING(i-linha)):VALUE) * 100, 2).
            ELSE
            DO:

                CREATE tt-excel.
                ASSIGN 
                    tt-excel.nr-contrato      = INTEGER(ChWorkSheet:RANGE("A"       + STRING(i-linha)):VALUE)
                    ...
                    tt-excel.perc-rateioConta = ROUND(DECIMAL(ChWorkSheet:RANGE("R" + STRING(i-linha)):VALUE) * 100, 2).
            END.
        END.
    END.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-excel.
        FIND FIRST contrato-for NO-LOCK
             WHERE contrato-for.nr-contrato = tt-excel.nr-contrato NO-ERROR.
        IF AVAIL contrato-for THEN
        DO:
            FOR EACH medicao-contrat OF contrato-for 
                WHERE medicao-contrat.numero-ordem    = tt-excel.numero-ordem
                AND   medicao-contrat.numero-ordem    = tt-excel.numero-ordem
                AND   medicao-contrat.num-seq-medicao = tt-excel.num-seq-medicao
                NO-LOCK:
                run cdp/cdapi181.p ( INPUT c-seg-usuario,
                                     INPUT 3,
                                     INPUT 2,
                ...
                END.
                .
                FIND FIRST matriz-rat-med EXCLUSIVE-LOCK
                     ...
                     AND   matriz-rat-med.num-seq-medicao = medicao-contrat.num-seq-medicao
                     AND   matriz-rat-med.ct-codigo       = tt-excel.ct-codigo     
                     AND   matriz-rat-med.sc-codigo       = tt-excel.sc-codigo     
                     AND   matriz-rat-med.cod-unid-negoc  = tt-excel.cod-unid-negoc NO-ERROR.
                IF NOT AVAIL matriz-rat-med THEN DO:
                    CREATE matriz-rat-med.
                    ASSIGN matriz-rat-med.nr-contrato     = medicao-contrat.nr-contrato 
                           matriz-rat-med.num-seq-item    = medicao-contrat.num-seq-item 
                           matriz-rat-med.numero-ordem    = medicao-contrat.numero-ordem 
                           matriz-rat-med.num-seq-event   = medicao-contrat.num-seq-event 
                           matriz-rat-med.num-seq-medicao = medicao-contrat.num-seq-medicao.
                END.
                ASSIGN matriz-rat-med.ct-codigo      = tt-excel.ct-codigo     
                       matriz-rat-med.sc-codigo      = tt-excel.sc-codigo     
                       matriz-rat-med.cod-unid-negoc = tt-excel.cod-unid-negoc
                       matriz-rat-med.perc-rateio    = tt-excel.perc-rateioConta.
            END.
        END.
    END.
END PROCEDURE.
