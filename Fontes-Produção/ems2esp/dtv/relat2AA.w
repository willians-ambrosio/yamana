...
/*------------------------------------------------------------------------
File.............: relat2AA.w
Description......: Relat¢rio Grupo x Programa x Empresa x Usu rio
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 18/10/10 - 10:16 - 30100979
OBS..............: Este fonte foi gerado pelo Data Viewer
------------------------------------------------------------------------*/

define variable c-prog-gerado as character no-undo initial "RELAT2AA".
...
define temp-table tt-param
    ...
    field c-cod_grp_usuar-ini like grp_usuar.cod_grp_usuar
    field c-cod_grp_usuar-fim like grp_usuar.cod_grp_usuar
...
def new shared var c-cod_grp_usuar-ini like grp_usuar.cod_grp_usuar format "x(3)" initial "" no-undo.
def new shared var c-cod_grp_usuar-fim like grp_usuar.cod_grp_usuar format "x(3)" initial "ZZZ" no-undo.
...
DEFINE FRAME f-pg-sel
   c-cod_grp_usuar-ini label "Grupo Usu rios"
     at row 1 col 19 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   c-cod_grp_usuar-fim no-label
     at row 1 col 50 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   c-cod_prog_dtsul-ini label "Programa"
     at row 2 col 19 colon-aligned
     view-as fill-in 
     size 20 by .88
     font 1

   c-cod_prog_dtsul-fim no-label
     at row 2 col 50 colon-aligned
     view-as fill-in 
     size 20 by .88
     font 1

   c-cod_usuario-ini label "Usu rio"
     at row 3 col 19 colon-aligned
     view-as fill-in 
     size 13 by .88
     font 1

   c-cod_usuario-fim no-label
     at row 3 col 50 colon-aligned
     view-as fill-in 
     size 13 by .88
     font 1

   IMAGE-1 AT ROW 01.00 COL 43
   IMAGE-2 AT ROW 01.00 COL 48
   IMAGE-3 AT ROW 02.00 COL 43
   IMAGE-4 AT ROW 02.00 COL 48
   IMAGE-5 AT ROW 03.00 COL 43
   IMAGE-6 AT ROW 03.00 COL 48
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Relat¢rio Grupo x Programa x Empresa x Usu rio - relat2AA"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "relat2AA".
...
PROCEDURE enable_UI :
   ...
   DISPLAY  
   c-cod_grp_usuar-ini c-cod_grp_usuar-fim
   c-cod_prog_dtsul-ini c-cod_prog_dtsul-fim
   c-cod_usuario-ini c-cod_usuario-fim
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "relat2AA" + "." + v-cod-extens-arq.

    assign tt-param.c-cod_grp_usuar-ini = input frame f-pg-sel c-cod_grp_usuar-ini
           tt-param.c-cod_grp_usuar-fim = input frame f-pg-sel c-cod_grp_usuar-fim
           tt-param.c-cod_grp_usuar-fim = input frame f-pg-sel c-cod_grp_usuar-fim
           tt-param.c-cod_prog_dtsul-ini = input frame f-pg-sel c-cod_prog_dtsul-ini
           tt-param.c-cod_prog_dtsul-fim = input frame f-pg-sel c-cod_prog_dtsul-fim
           tt-param.c-cod_usuario-ini = input frame f-pg-sel c-cod_usuario-ini
           tt-param.c-cod_usuario-fim = input frame f-pg-sel c-cod_usuario-fim
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "dtv/relat227.p".
