...
DEFINE VARIABLE h_ymof0101a-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0101a-v01-4 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 9.96 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 9.96 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 9.96 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 9.96 COL 70.72
     rt-button AT ROW 9.79 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 82.43 BY 17 WIDGET-ID 100.
...
DO:
{include/okfil.i h_ymof0101a-v01-4}
END.
...
DO:
{include/salvafil.i h_ymof0101a-v01-4}
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 8.21 , 81.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0101a-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymof0101a-v01-4 ).
       RUN set-position IN h_ymof0101a-v01-4 ( 3.13 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.25 , 72.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0101a-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_ymof0101a-q01 ).
       RUN set-position IN h_ymof0101a-q01 ( 1.00 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 7.72 ) */

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_ymof0101a-v01-4. */
       RUN add-link IN adm-broker-hdl ( h_ymof0101a-q01 , 'Record':U , h_ymof0101a-v01-4 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_ymof0101a-v01-4 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-ok:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymof0101a-v01-4 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ymof0101a" "1.00.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_ymof0101a-v01-4 (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_ymof0101a-q01 (input v-row-table).
