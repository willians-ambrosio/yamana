...
/*****************************************************************************
** Programa..............: esjd002
** Descricao.............: Cria Mapa e Ajusta Crit‚rio de Distribui‡Æo
** Versao................: 1.00.00.000
** Procedimento..........: 
** Nome Externo..........: 
** Criado por............: Bruno Bertulli (DSC)
** Criado em.............: 01/07/2014
*****************************************************************************/
...
IMAGE-16 RECT-12 rs-saida tg-cria-mapas fi-cod_cta_ctbl-ini ~
fi-cod_cta_ctbl-fim bt_executar rs-mapa fi-arquivo bt_fechar bt_cancelar ~
Btn_Help 
&Scoped-Define DISPLAYED-OBJECTS rs-saida tg-cria-mapas fi-cod_cta_ctbl-ini ~
fi-cod_cta_ctbl-fim rs-mapa fi-fim-validade fi-arquivo 
...
DEFINE VARIABLE fi-cod_cta_ctbl-fim AS CHARACTER FORMAT "x(20)" INITIAL "79999999" 
     VIEW-AS FILL-IN 
     SIZE 15.43 BY .88.

DEFINE VARIABLE fi-cod_cta_ctbl-ini AS CHARACTER FORMAT "x(20)" INITIAL "30000000" 
     LABEL "Conta Cont bil" 
     VIEW-AS FILL-IN 
     SIZE 15.43 BY .88.

DEFINE VARIABLE fi-fim-validade AS DATE FORMAT "99/99/9999":U 
     LABEL "Fim de Validade do Crit‚rio de Distribui‡Æo" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE rs-mapa AS INTEGER INITIAL 1 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "LISTA Mapa do Crit‚rio de Distribui‡Æo", 1,
"ALTERA Mapa do Crit‚rio de Distribui‡Æo", 2
...
DEFINE VARIABLE tg-cria-mapas AS LOGICAL INITIAL yes 
     LABEL "Criar Mapas para Contas Ativas e Anal¡ticas Exclusivas" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f_main
     bt-busca-arq AT ROW 13.79 COL 74
     rs-saida AT ROW 1.25 COL 30 NO-LABEL
     tg-cria-mapas AT ROW 3 COL 20
     ...
     rs-mapa AT ROW 5.25 COL 20 NO-LABEL
     fi-fim-validade AT ROW 7.46 COL 41.57 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = ".: Cria Mapa e Ajusta Crit‚rio de Distribui‡Æo - ESJD002 :."
         HEIGHT             = 16
         WIDTH              = 80
         MAX-HEIGHT         = 42.42
         MAX-WIDTH          = 274.29
...
/* SETTINGS FOR FILL-IN fi-fim-validade IN FRAME f_main
   NO-ENABLE                                                            */
...
ON END-ERROR OF C-Win /* .: Cria Mapa e Ajusta Crit‚rio de Distribui‡Æo - ESJD002 :. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* .: Cria Mapa e Ajusta Crit‚rio de Distribui‡Æo - ESJD002 :. */
DO:
...
DO:
    DEFINE VARIABLE l-aplica AS LOGICAL     NO-UNDO.

    IF INPUT FRAME {&FRAME-NAME} fi-cod_cta_ctbl-ini < '30000000' OR 
       INPUT FRAME {&FRAME-NAME} fi-cod_cta_ctbl-fim > '79999999' THEN DO:
        MESSAGE 'A faixa de conta informada deve estar entre 30000000 ate 79999999.' SKIP
                'Informe uma faixa v lida!'
    ...
    IF INPUT FRAME {&FRAME-NAME} rs-mapa = 2 THEN DO:
        MESSAGE "Deseja realmente alterar os Crist‚rios de Distribui‡Æo ?"
            VIEW-AS ALERT-BOX INFO BUTTONS YES-NO UPDATE l-aplica.
        IF NOT l-aplica THEN UNDO, RETURN NO-APPLY.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY rs-saida tg-cria-mapas fi-cod_cta_ctbl-ini fi-cod_cta_ctbl-fim rs-mapa 
          fi-fim-validade fi-arquivo 
      WITH FRAME f_main IN WINDOW C-Win.
  ENABLE bt-busca-arq rt_rgf rt_cxcf IMAGE-15 IMAGE-16 RECT-12 rs-saida 
         tg-cria-mapas fi-cod_cta_ctbl-ini fi-cod_cta_ctbl-fim bt_executar 
         rs-mapa fi-arquivo bt_fechar bt_cancelar Btn_Help 
      WITH FRAME f_main IN WINDOW C-Win.
...
IF NOT AVAIL dwb_rpt_param THEN DO:
    ...
    ASSIGN dwb_rpt_param.cod_dwb_program = "esjd002rp"
           dwb_rpt_param.cod_dwb_user    = v_cod_usuar_corren.
END.

ASSIGN dwb_rpt_param.cod_dwb_parameters = STRING (INPUT FRAME {&FRAME-NAME} tg-cria-mapas)
       dwb_rpt_param.cod_dwb_parameters = dwb_rpt_param.cod_dwb_parameters + ';' + STRING (INPUT FRAME {&FRAME-NAME} fi-cod_cta_ctbl-ini)
       dwb_rpt_param.cod_dwb_parameters = dwb_rpt_param.cod_dwb_parameters + ';' + STRING (INPUT FRAME {&FRAME-NAME} fi-cod_cta_ctbl-fim)
       dwb_rpt_param.cod_dwb_parameters = dwb_rpt_param.cod_dwb_parameters + ';' + STRING (INPUT FRAME {&FRAME-NAME} rs-mapa)
       dwb_rpt_param.cod_dwb_parameters = dwb_rpt_param.cod_dwb_parameters + ';' + STRING (INPUT FRAME {&FRAME-NAME} fi-fim-validade)
...
IF INTEGER (INPUT FRAME {&FRAME-NAME} rs-saida) = 2 THEN DO: /* Batch */
    run prgtec/btb/btb911za.p (Input "esjd002rp",
                               Input "5.06.00.001",
                               Input 40,
...
*/
    do with frame {&frame-name}:
        ...
        assign fi-arquivo     :screen-value = session:temp-directory + "esjd002-" + string(time) + ".csv"
               fi-fim-validade:screen-value = STRING(DATE('01/' + STRING(MONTH(TODAY)) + '/' + STRING(YEAR(TODAY)))).
    end.
END PROCEDURE.
