...
&IF DEFINED(EXCLUDE-pi-cria-cab-excel) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-cria-cab-excel Procedure 
...
DEFINE VARIABLE i-coluna-ini  AS INTEGER  INIT 6   NO-UNDO.
...
DEFINE VARIABLE i-maior-col AS INTEGER     NO-UNDO.
...
/*         chWorksheet:range("c" + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.it-codigo. */
        ...
        tt-excel.celula[i-coluna] = tt-es-mov-ext-cfa.pergunta-literal . /*chWorksheet:range( c-coluna + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.pergunta-literal.*/
        ASSIGN i-coluna = i-coluna + 1.
        c-coluna = ENTRY(i-coluna,c-coluna1,",").
        tt-excel.celula[i-coluna] = tt-es-mov-ext-cfa.resposta-literal. /*chWorksheet:range( c-coluna + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.resposta-literal.*/
        ASSIGN i-coluna = i-coluna + 1.
...
/*         c-coluna = ENTRY(i-coluna,c-coluna1,","). */
        tt-excel.celula[i-coluna] = tt-es-mov-ext-cfa.pergunta-literal. /* chWorksheet:cells( i-linha,i-coluna):VALUE = tt-es-mov-ext-cfa.pergunta-literal.*/
        ASSIGN i-coluna = i-coluna + 1.
/*         c-coluna = ENTRY(i-coluna,c-coluna1,","). */
         tt-excel.celula[i-coluna] = tt-es-mov-ext-cfa.resposta-literal.  /*chWorksheet:cells(i-linha,i-coluna):VALUE = tt-es-mov-ext-cfa.resposta-literal.*/
        ASSIGN i-coluna = i-coluna + 1.
    END.
...
DEFINE VARIABLE i-col-csv AS INTEGER     NO-UNDO.
...
FOR EACH tt-excel:
    ...
    DO WHILE i-col-csv <= 50:
        ...
        run pi-acompanhar in h-acomp (input "Arquivo ===> " +  STRING(i-col-csv)).
