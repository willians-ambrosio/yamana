...
define variable c-prog-gerado as character no-undo initial "ITEM01AA".
...
DEFINE FRAME f-pg-sel
   ...
   c-it-codigo-fim no-label
     at row 2 col 49 colon-aligned
     view-as fill-in 
     size 17 by .88
     font 1

   c-cod-estabel-ini label "Est Pad"
   ...
   c-cod-estabel-fim no-label
     at row 3 col 49 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   IMAGE-1 AT ROW 01.00 COL 42
   IMAGE-2 AT ROW 01.00 COL 47
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "CATµLOGO DE ITENS - ITEM01AA"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "ITEM01AA".
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "ITEM01AA" + "." + v-cod-extens-arq.

    assign tt-param.c-fm-codigo-ini = input frame f-pg-sel c-fm-codigo-ini
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "DTV/ITEM0127.p".
