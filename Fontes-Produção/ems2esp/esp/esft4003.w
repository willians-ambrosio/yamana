...
{include/i-prgvrs.i ESFT4003 "TOTVS"}
...
DEFINE VARIABLE cod-estab-de AS CHARACTER FORMAT "X(256)":U 
     LABEL "Estab. DE" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE cod-serie AS CHARACTER FORMAT "X(256)":U 
     LABEL "Serie NF" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE nat-operacao AS CHARACTER FORMAT "X(256)":U 
     LABEL "Nat. Operacao" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE qtd-itens-nf AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 100 
     LABEL "Qtd. Itens NF" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.
...
DEFINE FRAME f-import
     ...
     im-pg-par AT ROW 1.5 COL 2.57
     im-pg-log AT ROW 1.5 COL 18.29
...
DEFINE FRAME f-pg-par
     bt-arquivo-entrada AT ROW 3.25 COL 55.14 HELP
          "Escolha do nome do arquivo"
     c-arquivo-entrada AT ROW 3.29 COL 15.29 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     cod-estab-de AT ROW 5 COL 30 COLON-ALIGNED WIDGET-ID 4
     cod-serie AT ROW 6 COL 30 COLON-ALIGNED WIDGET-ID 6
     cod-emitente AT ROW 7 COL 30 COLON-ALIGNED WIDGET-ID 8
     nat-operacao AT ROW 8 COL 30 COLON-ALIGNED WIDGET-ID 10
     qtd-itens-nf AT ROW 9 COL 30 COLON-ALIGNED WIDGET-ID 14
     text-entrada AT ROW 2.25 COL 16.14 NO-LABEL
     RECT-8 AT ROW 2.58 COL 14
     RECT-10 AT ROW 4.75 COL 14 WIDGET-ID 12
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 77.57 BY 10.38 WIDGET-ID 100.
...
/* SETTINGS FOR IMAGE im-pg-lay IN FRAME f-import
   NO-ENABLE                                                            */
ASSIGN 
       im-pg-lay:HIDDEN IN FRAME f-import           = TRUE.
...
ON END-ERROR OF C-Win /* Importa FT4003 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Importa FT4003 */
DO:
...
DO:
    {include/i-imarq.i c-arquivo-entrada f-pg-par "'*.csv' '*.csv*','TODOS' '*.*'  " }
END.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-arquivo-entrada cod-estab-de cod-serie cod-emitente nat-operacao 
          qtd-itens-nf 
      WITH FRAME f-pg-par IN WINDOW C-Win.
  ENABLE RECT-8 RECT-10 bt-arquivo-entrada c-arquivo-entrada cod-estab-de 
         cod-serie cod-emitente nat-operacao qtd-itens-nf 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
do  on error undo, return error
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.cod-estab-de    = input frame f-pg-par cod-estab-de
           tt-param.cod-serie       = input frame f-pg-par cod-serie   
           tt-param.cod-emitente    = input frame f-pg-par cod-emitente
           tt-param.nat-operacao    = input frame f-pg-par nat-operacao
           tt-param.qtd-itens-nf    = input frame f-pg-par qtd-itens-nf.

    /*if  tt-param.destino = 1 then
        assign tt-param.arq-destino = "".
    else
    if  tt-param.destino = 2 then 
        assign tt-param.arq-destino = "c:\temp\nat-log.txt"  /*input frame f-pg-log c-arquivo-destino*/.
    else
        assign tt-param.arq-destino = session:temp-directory +  "\" + OS-GETENV("username") + "\" +  "ymof0121rp-log.txt" .


        ASSIGN tt-param.arquivo = session:temp-directory +  "\" + OS-GETENV("username") + "\" +  "ymof0121rp-log.txt" . */
    ...
    {include/i-imrun.i esp/esft4003rp.p}
