...
{include/i-prgvrs.i esin004RP 2.00.00.015}  /*** 010015 ***/
{include/i_fnctrad.i}
/******************************************************************************
**
...
DEF VAR tot-valor     LIKE  t-controle-1.valor.
DEF VAR tot-valor-ord LIKE   t-controle-1.valor.
...
/*************** Imprime altera‡äes **************************/
    /*************** Cabe»alho ***********************************/ 
 chWorkSheet:Range("a1"):Value =    empresa.nome    
                                    +  "   -   " 
                                    + proj-inv.descricao.
...
FOR EACH t-controle-1 NO-LOCK
  ...
  ASSIGN tot-valor = tot-valor + t-controle-1.valor. 
   ...
   /*DISp t-controle-1.ep-codigo
    ...
    PUT  STREAM st UNFORMATTED 
     ...
     ASSIGN chExcelApplication:range( "A" + STRING(i-linha) ):value =  t-controle-1.num-ord-magnus   /* t-controle-1.cod-est-exec      */
            chExcelApplication:range( "B" + STRING(i-linha) ):value =  t-controle-1.num-pedido       /* t-controle-1.num-projeto       */
            chExcelApplication:range( "c" + STRING(i-linha) ):value =  t-controle-1.cod-emit         /* t-controle-1.nr-contrato       */
            chExcelApplication:range( "d" + STRING(i-linha) ):value =  c-nome                        /* t-controle-1.nr-contrato       */
            chExcelApplication:range( "e" + STRING(i-linha) ):value =  tot-valor /*t-controle-1.valor*/ /*ent-comp*/         /* t-controle-1.num-pedido        */
            chExcelApplication:Range( "e" + STRING(i-linha)):NumberFormat="###.###.##0" 
            chExcelApplication:range( "f" + STRING(i-linha) ):value =  t-controle-1.dt-trans.
  ...
  IF LAST-OF(t-controle-1.num-ordem /*-magnus*/ ) THEN DO:
     ASSIGN chExcelApplication:range( "d" + STRING(i-linha) ):value = "           Total: " 
            chExcelApplication:range( "e" + STRING(i-linha) ):value =  tot-valor-ord 
            chExcelApplication:Range( "e" + STRING(i-linha)):NumberFormat="###.###.##0".
