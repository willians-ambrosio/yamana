...
**         Objetivo: Cadastro simples da tabela es_param_webserver (Parametro webserver)
**
...
{include/i-prgvrs.i VAN001-W01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i VAN001-W01 VAN}
&ENDIF
...
DEFINE VARIABLE h_van001-b03 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_van001-q01-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_VAN001-V01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 97 BY 21.75 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo de Parƒmetro webserver"
         HEIGHT             = 21.75
         WIDTH              = 97
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 119
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo de Parƒmetro webserver */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo de Parƒmetro webserver */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-cadsim ).
       RUN set-position IN h_p-cadsim ( 1.33 , 31.29 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 80.72 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'VAN/VAN001-V01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_VAN001-V01 ).
       RUN set-position IN h_VAN001-V01 ( 2.79 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 8.50 , 96.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'van/van001-b03.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_van001-b03 ).
       RUN set-position IN h_van001-b03 ( 11.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.50 , 96.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'van/van001-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = VAN\VAN001-Z01.w,
                     ProgVaPara = VAN\VAN001-G01.w,
                     ProgIncMod = ,
                     Implantar = NO':U ,
             OUTPUT h_van001-q01-2 ).
       RUN set-position IN h_van001-q01-2 ( 3.46 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_VAN001-V01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_VAN001-V01 ).
       RUN add-link IN adm-broker-hdl ( h_van001-q01-2 , 'Record':U , h_VAN001-V01 ).

       /* Links to SmartBrowser h_van001-b03. */
       RUN add-link IN adm-broker-hdl ( h_van001-q01-2 , 'Record':U , h_van001-b03 ).
       RUN add-link IN adm-broker-hdl ( h_VAN001-V01 , 'hab':U , h_van001-b03 ).

       /* Links to SmartQuery h_van001-q01-2. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_van001-q01-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_van001-q01-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_van001-q01-2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_VAN001-V01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_van001-b03 ,
             h_VAN001-V01 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "VAN001-W01" "1.00.00.000"}
