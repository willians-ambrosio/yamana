...
*******************************************************************************/
{include/i-prgvrs.i ESUT0012 2.06.00.001}
...
DEFINE VARIABLE text-tipo AS CHARACTER FORMAT "X(256)":U INITIAL " Tipo Sa°da" 
      VIEW-AS TEXT 
     SIZE 11.14 BY .63 NO-UNDO.

DEFINE VARIABLE rs-tipo-saida AS INTEGER 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Mensal", 1,
"Anual", 2,
...
DEFINE VARIABLE fi-periodo-fim AS CHARACTER FORMAT "9999/99":U INITIAL "299912" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi-periodo-ini AS CHARACTER FORMAT "9999/99":U INITIAL "190001" 
     LABEL "Per°odo" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     c-cod-estabel-ini AT ROW 1.5 COL 20 COLON-ALIGNED HELP
          "C¢digo do estabelecimento" WIDGET-ID 4
     c-cod-estabel-fim AT ROW 1.5 COL 49 COLON-ALIGNED HELP
          "C¢digo do estabelecimento" NO-LABEL WIDGET-ID 6
     c-it-codigo-ini AT ROW 2.5 COL 12 COLON-ALIGNED HELP
          "C¢digo do Item" WIDGET-ID 2
     c-it-codigo-fim AT ROW 2.5 COL 49 COLON-ALIGNED HELP
          "C¢digo do Item" NO-LABEL WIDGET-ID 8
     fi-periodo-ini AT ROW 3.5 COL 18.86 COLON-ALIGNED WIDGET-ID 20
     fi-periodo-fim AT ROW 3.5 COL 49 COLON-ALIGNED NO-LABEL WIDGET-ID 22
     IMAGE-1 AT ROW 1.5 COL 31.86
     IMAGE-2 AT ROW 1.5 COL 47.29
     IMAGE-3 AT ROW 2.5 COL 31.86 WIDGET-ID 10
     IMAGE-4 AT ROW 2.5 COL 47.29 WIDGET-ID 12
     IMAGE-5 AT ROW 3.5 COL 31.86 WIDGET-ID 14
     IMAGE-6 AT ROW 3.5 COL 47.29 WIDGET-ID 16
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
...
DEFINE FRAME f-pg-par
     rs-tipo-saida AT ROW 1.75 COL 6 NO-LABEL WIDGET-ID 2
     text-tipo AT ROW 1.04 COL 3.86 NO-LABEL WIDGET-ID 8
     RECT-8 AT ROW 1.33 COL 2.14 WIDGET-ID 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio de Saldos de Itens"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-relat /* Relat¢rio de Saldos de Itens */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Relat¢rio de Saldos de Itens */
DO:
...
{utp/ut9000.i "ESUT0012" "2.06.00.001"}
...
------------------------------------------------------------------------------*/
  ...
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 c-cod-estabel-ini 
         c-cod-estabel-fim c-it-codigo-ini c-it-codigo-fim fi-periodo-ini 
         fi-periodo-fim 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY rs-tipo-saida 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-8 rs-tipo-saida 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF INPUT FRAME f-pg-sel c-cod-estabel-ini > INPUT FRAME f-pg-sel c-cod-estabel-fim THEN DO:
       apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
            
       run utp/ut-msgs.p (input "show":U, input 142, input "Estabelecimento~~Estabelecimento").
    ...
    IF INPUT FRAME f-pg-sel c-it-codigo-ini > INPUT FRAME f-pg-sel c-it-codigo-fim THEN DO:
       apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
            
       run utp/ut-msgs.p (input "show":U, input 142, input "Item~~Item").
            
       apply "ENTRY":U to c-it-codigo-ini in FRAME f-pg-sel.
            
       return error.
    END.   


    IF INTEGER (SUBSTRING (INPUT FRAME f-pg-sel fi-periodo-ini,5,2)) < 1 OR INTEGER (SUBSTRING (INPUT FRAME f-pg-sel fi-periodo-ini,5,2)) > 12 THEN DO:
       apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
            
       run utp/ut-msgs.p (input "show":U, input 17006, input "Per°odo inv†lido").
            
       apply "ENTRY":U to fi-periodo-ini in FRAME f-pg-sel.
            
       return error.
    END.   


    IF INTEGER (SUBSTRING (INPUT FRAME f-pg-sel fi-periodo-fim,5,2)) < 1 OR INTEGER (SUBSTRING (INPUT FRAME f-pg-sel fi-periodo-fim,5,2)) > 12 THEN DO:
       apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
            
       run utp/ut-msgs.p (input "show":U, input 17006, input "Per°odo inv†lido").
            
       apply "ENTRY":U to fi-periodo-fim in FRAME f-pg-sel.
            
       return error.
    END. 

    IF INTEGER (SUBSTRING (INPUT FRAME f-pg-sel fi-periodo-ini,1,4)) < 1999 THEN DO:
       apply "MOUSE-SELECT-CLICK":U to im-pg-sel in frame f-relat.
   ...
   ASSIGN tt-param.cod-estabel-ini = INPUT FRAME f-pg-sel  c-cod-estabel-ini
          ...
          tt-param.periodo-fim     = INPUT FRAME f-pg-sel  fi-periodo-fim
          tt-param.rs-tipo-saida   = INPUT FRAME f-pg-par  rs-tipo-saida.
    ...
    {include/i-rprun.i esp/esut0012rp.p}
