...
DEFINE VARIABLE h_ymof0112a-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0112a-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 7.17 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 7.17 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 7.17 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 7.17 COL 70.72
     rt-button AT ROW 7 COL 1.57
...
DO:
{include/okfil.i h_ymof0112a-v01}
END.
...
DO:
{include/salvafil.i h_ymof0112a-v01}
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 5.46 , 79.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112a-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymof0112a-v01 ).
       RUN set-position IN h_ymof0112a-v01 ( 3.00 , 2.57 ) NO-ERROR.
       /* Size in UIB:  ( 3.25 , 75.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112a-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_ymof0112a-q01 ).
       RUN set-position IN h_ymof0112a-q01 ( 1.25 , 51.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 7.72 ) */

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_ymof0112a-v01. */
       RUN add-link IN adm-broker-hdl ( h_ymof0112a-q01 , 'Record':U , h_ymof0112a-v01 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'tableio':U , h_ymof0112a-v01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             bt-ok:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymof0112a-v01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  run pi-atualiza-parent in h_ymof0112a-v01 (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_ymof0112a-q01 (input v-row-table).
