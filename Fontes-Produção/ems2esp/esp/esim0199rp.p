...
** Altera‡oe:
**
** Data         Autor          Altera‡Æo
** -----------  -------------- ----------------------------------------------------
** 03/12/2013   Jurandir       Cria‡Æo da Rotina
**    02/2019   Fl via Acioli  Ajustes do Programa
** 
** 
*******************************************************************************/
...
Def Temp-table tt-mov                   No-undo                                                        
    ...
    Field qt-merc                       As Dec /* S¢ mostra se for Nota MÆe */
    Field un                            As Char
...
/*     Field sequencia                     Like item-doc-est.sequencia */
    ...
    Field dt-solicitacao-nf             As Char /* MMIC: Se Nota MÆe ou Filha, mostra a dt-efetiva do ponto de controle 7 (IM0055); Se Nota Complementar, mostra a dt-efetiva do ponto de controle 8; */
                                                /* JMC:  Se Nota MÆe ou Filha, mostra a dt-efetiva do ponto de controle 5 (IM0055); Se Nota Complementar, mostra a dt-efetiva do ponto de controle 6; */
                                                /* O campo s¢ vir  preenchido com a data solicitada quando os CFOPs forem: */
                                                /* 3949 Nota Filha / MÆe / Normal                                          */
                                                /* 3127 MÆe / Normal / Complementar                                        */
                                                /* 3101 MÆe / Normal / Complementar                                        */
                                                /* 3551 MÆe / Normal / Complementar                                        */
                                                /* 3556 MÆe / Normal / Complementar                                        */
    ...
    Field vl-frete-rodoviario-a         As Dec /* RE0701 - Valor Total da aba Totais II se o CFOP for 1352 ou 2352 - Somente para Notas Complementares */
...
/*         "Sequˆncia" */
        ...
        "Tx Siscomex"
        "AFRMM"
        "Servi‡os despachante"
        "Demurrage"
        "S D A"
        "Armazenagem porto"
        "II" /* Imposto de Importa‡Æo */
        "IPI"
        "Cofins"
        "PIS"
        "S D A Antecipada"
        ...
        "Remo‡Æo de DTA"
        "Multas DI"
        "Frete rodo at‚ EADI" /* Frete rodovi rio at‚ EADI */
        "Frete ferro at‚ EADI" /* Frete ferrovi rio at‚ EADI */
        "Frete rodo at‚ unid." /* Frete rodovi rio at‚ unidade */
        "ICMS"
        "Aluguel de ctn" /* Aluguel de Container */
        "Frete ferro at‚ Santos" /* Frete ferrovi rio at‚ Santos */
        "Compl. Armazenagem Porto"
        "Servi‡o de entrega"
        "Frete ferro at‚ Santos"
        ...
        "Compl. Frete Intern."
        "Tx Portu rias na origem"
        "Inspe‡Æo da carga no destino"
        "Inspe‡Æo da carga no embarque"
        "THC"
        "Coordena‡Æo de descarga"
        "GestÆo do processo"
        "Compl. Capatazia"
        "IPI - Filha"
        "Cofins - Filha"
...
For Each embarque-imp No-lock 
   Where embarque-imp.cod-estabel             >= tt-param.cod-estabel-ini
     ...
     Or   (Replace(Replace(embarque-imp.declaracao-import,"/",""),"-","") >= tt-param.nr-di-ini
     And   Replace(Replace(embarque-imp.declaracao-import,"/",""),"-","") <= tt-param.nr-di-fim))
...
/*                tt-mov.sequencia                    = item-doc-est.sequencia */
               ...
               tt-mov.qt-merc                      = /* item-doc-est.quantidade */ If tt-mov.tp-docto = "Nota MÆe" Then ordem-compra.qt-solic Else 0 /* S¢ deve mostrar quantidade para a Nota MÆe */
...
/*                tt-mov.vl-frete-rodoviario-a        = fnValDespesa(00) */ /* Somente para Notas Complementares */
        ...
        /* O campo dever  vir preenchido com a data solicitada quando os CFOPs forem: */
        /* 3949 Nota Filha / MÆe / Normal                                             */
        /* 3127 MÆe / Normal / Complementar                                           */
        /* 3101 MÆe / Normal / Complementar                                           */
        /* 3551 MÆe / Normal / Complementar                                           */
        /* 3556 MÆe / Normal / Complementar                                           */
        If tt-mov.tp-docto = "Nota Filha" Then Do:
            If Substring(tt-mov.nat-operacao,1,4) <> "3949" Then
                Assign tt-mov.dt-solicitacao-nf = "".
        End.
        Else Do:
            If  Substring(tt-mov.nat-operacao,1,4) <> "3127"
            And Substring(tt-mov.nat-operacao,1,4) <> "3101"
            And Substring(tt-mov.nat-operacao,1,4) <> "3551"
            And Substring(tt-mov.nat-operacao,1,4) <> "3556"
            And Substring(tt-mov.nat-operacao,1,4) <> "3949" /* Inclu¡do em 25/03/2019 */ Then
                Assign tt-mov.dt-solicitacao-nf     = "".
        End.
...
/*                        tt-mov.sequencia                    = b-docum-est.sequencia */
                       ...
                       tt-mov.nat-operacao                 = /*docum-est.nat-operacao*/ rat-docum.nat-oper /* Natureza que aparece no IM0545G */
                ...
                And Substring(tt-mov.nat-operacao,1,4) <> "3101"
                And Substring(tt-mov.nat-operacao,1,4) <> "3551"
                And Substring(tt-mov.nat-operacao,1,4) <> "3556" Then
                    Assign tt-mov.dt-solicitacao-nf     = "".

                /* Se for uma nota complementar com CFOP (natureza de opera‡Æo do documento por‚m sem as duas últimas letras)
                   1352 ou 2352, indica que ‚ uma nota de frete */
                If Substring(tt-mov.nat-operacao,1,4) = "1352"
                Or Substring(tt-mov.nat-operacao,1,4) = "2352" Then
...
------------------------------------------------------------------------------*/
    ...
    Def Var dataDICot  As Date No-undo.
    ...
    If dataDI >= 04/10/2019 Then Do:

        Repeat:
            dataDICot = dataDICot - 1.
    
            If  Weekday(dataDICot) <> 1 And Weekday(dataDICot) <> 7 Then Do:
                /* Verifica se ‚ dia £til - Programa bas_calend_glob */
                Find First dia_calend_glob Where dia_calend_glob.cod_calend   = "Fiscal"
                                           And   dia_calend_glob.dat_calend   = dataDICot
                                           And   dia_calend_glob.log_dia_util = Yes No-lock No-error.
                If Avail dia_calend_glob Then
                    Assign cont = cont + 1.
                Else Leave. /* Se nÆo encontrar registro na tabela dia_calend_glob, sai do bloco, para nÆo entrar em loop */
            End.
    
            /* A taxa de cƒmbio para venda da moeda estrangeira, divulgada pelo Sistema de Informa‡äes Banco Central - SISBACEN, por meio da transa‡Æo "PTAX800, op‡Æo 05 - Cota‡äes para
               Contabilidade", no fechamento do dia £til imediatamente anterior …quele em que houver sido disponibilizada no SISCOMEX.
    
            ********** Not¡cia SISCOMEX **********
            Data: 10/04/2019
            Assunto: Taxa de Cƒmbio para efeito de c lculo dos tributos incidentes na importa‡Æo
            Conteúdo da Not¡cia: Informamos que, conforme Portaria MF nº 6, de 25 de janeiro de 1999, a taxa de cƒmbio para efeito de c lculo dos tributos incidentes na importa‡Æo ‚ fixada
            com base na cota‡Æo di ria para venda da respectiva moeda e produz efeitos no dia subsequente. Exemplificando, para as DI registradas no dia 10/04/2019, a taxa a ser observada
            ‚ a do dia 09/04/2019.
            */
            If cont = 1 Then Leave. /* A partir do dia 10/04/2019, considera a taxa da DI de 1 dia £til anterior … Data da DI */
    ...
    Else Do:
        Repeat:
            ...
            ********** Not¡cia SISCOMEX **********
            ...
            ‚ a do dia 09/04/2019.
            */
            If cont = 2 Then Leave. /* At‚ o dia 09/04/2019, considera a taxa da DI de 2 dias £teis anterior … Data da DI */
        End.
    ...
    Find First cotacao No-lock
         Where cotacao.mo-codigo    = ordem-compra.mo-codigo
           And cotacao.ano-periodo  = String(Year (dataDICot),"9999")
                                    + String(Month(dataDICot),"99")
        No-error.
    If Avail cotacao Then Do:
        If  cotacao.cotacao[Day(dataDICot)] = 0 Then
            Assign taxa = 1.
        Else
            Assign taxa = cotacao.cotacao[Day(dataDICot)].
    End.

    Find First ems2cadme.moeda Where moeda.mo-codigo = ordem-compra.mo-codigo No-lock No-error.

    Assign tt-mov.dt-cotacao = dataDICot
...
/*          Then Return "Nota MÆe".                                                         */
/*          Else Do:                                                                        */
/*              Find First b-tt-mov Where b-tt-mov.nr-pedido = tt-mov.nr-pedido             */
/*                                  And   b-tt-mov.tp-docto  = "Nota MÆe" No-lock No-error. */
