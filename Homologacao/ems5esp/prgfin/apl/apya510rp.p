...
** 11/06/2019   Daniela Campos  DKP                        Tratamento para baixa de EmprÇstimo sem parcelas - pagamento  
** Williams, gentileza incluir aqui o histÛrico da sua alteraÁ„o, bem como no corpo do programa onde ela foi feita. obrigada.
...
DEF TEMP-TABLE tt-dados
    ...
    FIELD valor_oper_principal AS DECIMAL
    /* Movto emprÇstimos */
    ...
    FOR EACH operac_financ NO-LOCK 
        ...
        FIRST banco OF operac_financ NO-LOCK:
          ...
          FOR EACH movto_operac_financ NO-LOCK
             WHERE movto_operac_financ.num_id_operac_financ = operac_financ.num_id_operac_financ
              ...
              IF movto_operac_financ.dat_transacao > tt-param.data_movimento_ini THEN DO:
				  ...
				  /*** 11/06/2019 - DPC - Tratamento para pagamento de ACC - n∆o possui parcela - para todos os outros casos ser† */
                  /*** Tratado via pagamento de parcela ***/  
                  IF movto_operac_financ.ind_tip_trans_apl = "Pagamento" THEN DO:
                      /* Ir· considerar o movimento de pagamento apenas se n„o existir parcela de pagamento - para nío duplicar o valor */
                      IF NOT CAN-FIND(FIRST parc_operac_financ OF operac_financ NO-LOCK 
                                WHERE parc_operac_financ.ind_sit_parc_pagto = "quitada"
                                  AND parc_operac_financ.ind_tip_parc_emprest = "Pagamento"
                                  AND parc_operac_financ.dat_pagto > tt-param.data_movimento_ini) THEN 
                            ASSIGN v_val_movto_apl_pagamento[1] = v_val_movto_apl_pagamento[1] + d-valor-real
                                   v_val_movto_apl_pagamento[2] = v_val_movto_apl_pagamento[2] + d-valor-dolar.
                  END.
              END.
          ...
          ASSIGN tt-dados.tipo_produto             = produt_financ.cod_tip_produt_financ
                 ...
                 tt-dados.valor_oper_principal     = v_val_sdo_princ_operac_financ
...
PROCEDURE pi-impressao:
   ...
   FOR EACH tt-dados:
      ...
      IF tt-dados.me_sl_principal  = 0 AND
       ...
       FIND FIRST tt_resumo_apl  where
                  tt_resumo_apl.tta_cod_operac_financ = tt-dados.cod_operac_financ  NO-LOCK NO-ERROR.
      ...
      ASSIGN excelappl:cells(i-linha,i-col)      = tt-dados.tipo_produto        /* A */
             ...
             excelappl:cells(i-linha,27) = tt-dados.valor_oper_principal           
             excelappl:cells(i-linha,28) = if avail tt_resumo_apl then tt_resumo_apl.ttv_val_sdo_princ else 0
             excelappl:cells(i-linha,29) = if avail tt_resumo_apl then (tt_resumo_apl.ttv_val_operac_financ - tt_resumo_apl.ttv_val_sdo_princ) else 0
             excelappl:cells(i-linha,30) = 0       
             /* Valores controlados em reais */
             excelappl:cells(i-linha,32)  = tt-dados.me_sl_principal
             excelappl:cells(i-linha,33)  = tt-dados.me_novos_emp           
             excelappl:cells(i-linha,34)  = tt-dados.me_juros_prov          
             excelappl:cells(i-linha,35)  = tt-dados.me_dif_cambial         
             excelappl:cells(i-linha,36)  = tt-dados.me_prov_irrf           
             excelappl:cells(i-linha,37)  = tt-dados.me_pg_principal        
             excelappl:cells(i-linha,38)  = tt-dados.me_pg_juro             
             excelappl:cells(i-linha,39)  = tt-dados.me_sl_final
 
             /* Valores controlados em d¢lar */
             excelappl:cells(i-linha,41)  = tt-dados.mv_sl_emp 
             excelappl:cells(i-linha,42)  = tt-dados.mv_novo_emp                          
             excelappl:cells(i-linha,43)  = tt-dados.mv_juros_prov   
             excelappl:cells(i-linha,44)  = tt-dados.mv_dif_cambial
             excelappl:cells(i-linha,45)  = tt-dados.mv_prv_irrf     
             excelappl:cells(i-linha,46)  = tt-dados.mv_pg_principal        
             excelappl:cells(i-linha,47)  = tt-dados.mv_pg_juros           
             excelappl:cells(i-linha,48)  = tt-dados.mv_sl_final     

             excelappl:cells(i-linha,50)  = tt-dados.cc_principal       
             excelappl:cells(i-linha,51)  = tt-dados.cc_desc            
             excelappl:cells(i-linha,52)  = tt-dados.cc_var_cambial     
             excelappl:cells(i-linha,53)  = tt-dados.cc_desc_var_camb   
             excelappl:cells(i-linha,54)  = tt-dados.cc_juros           
             excelappl:cells(i-linha,55)  = tt-dados.cc_desc_juros

             excelappl:cells(i-linha,56)  = tt-dados.me_sl_juros
             excelappl:cells(i-linha,57)  = tt-dados.me_sl_ir
             excelappl:cells(i-linha,58)  = 0.
...
PROCEDURE pi-excel-abrir :
   ASSIGN c-modelo = SEARCH('prgfin\apl\apya510.xlsx').
...
PROCEDURE pi-filtra-apl:
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
EMPTY TEMP-TABLE tt_log_erros_apl_emp.
EMPTY TEMP-TABLE tt_resumo_apl.

run prgfin/apl/apl318za.py (Input 1,
                            Input v_cod_empres_usuar,
                            Input "REAL",
                            Input tt-param.data_movimento_fim,
                            Input 0,
                            Input "",
                            Input "ZZZZZZZZ",
                            Input "",
                            ...
                            Input "",
                            output v_val_tot_operac_swap).
                            
                            
/* FOR EACH tt_resumo_apl WHERE                                         */
/*          tt_resumo_apl.tta_cod_operac_financ = "MMR-80144":          */
/*                                                                      */
...
/*                                                                      */
/*     MESSAGE v_val_sdo_princ SKIP v_val_sdo_juros VIEW-AS ALERT-BOX.  */
/* END.                                                                 */
/*                                                                      */
