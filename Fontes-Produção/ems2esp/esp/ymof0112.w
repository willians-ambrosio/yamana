...
DEFINE VARIABLE h_ymof0112-b01-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0112-b02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0112-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0112-q01-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymof0112-v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 88.86 BY 17.38 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "CRITêRIOS ESTORNO CRêDITO CONSUMO"
         HEIGHT             = 17.38
         WIDTH              = 88.86
         MAX-HEIGHT         = 21.21
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadpaifilho-ambos /* CRITêRIOS ESTORNO CRêDITO CONSUMO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* CRITêRIOS ESTORNO CRêDITO CONSUMO */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.17 , 70.29 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymof0112-v01 ).
       RUN set-position IN h_ymof0112-v01 ( 2.50 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Itens Estorno|Centro  Custo' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 4.25 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 14.13 , 88.86 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = ,
                     ProgVaPara = ,
                     ProgIncMod = esp\ymof0112c.w,
                     Implantar = no':U ,
             OUTPUT h_ymof0112-q01 ).
       RUN set-position IN h_ymof0112-q01 ( 1.13 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 7.72 ) */

       /* Links to SmartViewer h_ymof0112-v01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_ymof0112-v01 ).
       RUN add-link IN adm-broker-hdl ( h_ymof0112-q01 , 'Record':U , h_ymof0112-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_ymof0112-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_ymof0112-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_ymof0112-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymof0112-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_ymof0112-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp\ymof0112a.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_ymof0112-b01-2 ).
       RUN set-position IN h_ymof0112-b01-2 ( 5.50 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 10.58 , 84.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_ymof0112-q01-4 ).
       RUN set-position IN h_ymof0112-q01-4 ( 1.13 , 51.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 7.72 ) */

       /* Links to BrowserCadastro2 h_ymof0112-b01-2. */
       RUN add-link IN adm-broker-hdl ( h_ymof0112-q01 , 'Record':U , h_ymof0112-b01-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymof0112-b01-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/ymof0112-b02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp\ymof0112b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_ymof0112-b02 ).
       RUN set-position IN h_ymof0112-b02 ( 5.50 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 12.46 , 86.00 ) */

       /* Links to BrowserCadastro2 h_ymof0112-b02. */
       RUN add-link IN adm-broker-hdl ( h_ymof0112-q01 , 'Record':U , h_ymof0112-b02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymof0112-b02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMOF0112 CRITêRIOS ESTORNO CRêDITO CONSUMO" "1.00.00.000"}
