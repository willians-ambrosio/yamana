...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getNaturezaNotaFiscal DBOProgram 
PROCEDURE getNaturezaNotaFiscal:
/*:T------------------------------------------------------------------------------
  Purpose:     Valida»„es pertinentes ao DBO
  Parameters:  recebe o tipo de valida»’o (Create, Delete, Update)
  Notes:       
------------------------------------------------------------------------------*/
    /* Defini»’o dos parümetros de Entrada/Saðda */
    def input  param p-c-cod-estabel  as char no-undo.
    def input  param p-c-nome-abrev   as char no-undo.
    def output param p-c-nat-operacao as char no-undo.

    DEFINE VARIABLE l-proc-ok-aux AS LOGICAL     NO-UNDO.

    run localizaEstabelec(input  p-c-cod-estabel,
    ...
    FIND FIRST emitente where 
               emitente.nome-abrev = trim(p-c-nome-abrev) NO-LOCK NO-ERROR.
    if  not avail emitente then
        FIND FIRST emitente where
                   emitente.cgc = p-c-nome-abrev NO-LOCK NO-ERROR.
    if  not avail emitente then
        FIND FIRST emitente where
                   emitente.cod-emitente = int(p-c-nome-abrev) NO-LOCK NO-ERROR.
    
    /* Leitura do local de entrega do emitente */
    if  avail emitente
    and emitente.cod-entrega <> "":U then 
        find loc-entr
    ...
    if  avail emitente then do:
        ...
        for first distrib-emit-estab
            where distrib-emit-estab.cod-estabel  = p-c-cod-estabel
              and distrib-emit-estab.cod-emitente = emitente.cod-emitente no-lock:
        end. 
    
        assign p-c-nat-operacao = if  avail estabelec 
                                  and (   emitente.natureza = 3
                                       or emitente.estado   = estabelec.estado) 
                                       or (    avail loc-entr
                                           and loc-entr.estado = estabelec.estado)
                                       or (    not avail loc-entr
                                           and emitente.estado = estabelec.estado)
                                  then if avail distrib-emit-estab 
                                           then distrib-emit-estab.nat-operacao 
                                           else emitente.nat-operacao
                                  else if avail distrib-emit-estab 
                                           then distrib-emit-estab.nat-oper-ext
                                           else emitente.nat-ope-ext.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE localizaEstabelec DBOProgram 
PROCEDURE localizaEstabelec:
    /* Defini‡Æo dos parƒmetros de Entrada/Sa¡da */
    def input  param p-c-cod-estabel     like estabelec.cod-estabel no-undo.
    def output param p-l-procedimento-ok as   log                   no-undo.

    for first estabelec
        fields (estabelec.cod-estabel   estabelec.estado 
                estabelec.pais
                estabelec.cidade
		estabelec.char-1) no-lock
        where estabelec.cod-estabel = p-c-cod-estabel:
    end.

    assign p-l-procedimento-ok = yes. 
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
