...
DEFINE VARIABLE cargo-fim AS INTEGER FORMAT ">>>>9":U INITIAL 99999 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.

DEFINE VARIABLE cargo-ini AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Cargo" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     ...
     cargo-ini AT ROW 5.25 COL 16 COLON-ALIGNED
     cargo-fim AT ROW 5.25 COL 45 COLON-ALIGNED NO-LABEL
     v_cdn_avpes_padr_ini AT ROW 6.25 COL 16 COLON-ALIGNED HELP
          "Codigo da Avaliacao"
     v_cdn_avpes_padr_fim AT ROW 6.25 COL 45 COLON-ALIGNED HELP
          "Codigo da Avaliacao" NO-LABEL
     rs-tipo AT ROW 7.25 COL 24 NO-LABEL
     v_cdn_estab_ini AT ROW 8.5 COL 16 COLON-ALIGNED
     v_cdn_estab_fim AT ROW 8.5 COL 46 COLON-ALIGNED NO-LABEL
     v_cdn_funcionario_ini AT ROW 9.5 COL 16 COLON-ALIGNED
     v_cdn_funcionario_fim AT ROW 9.5 COL 46 COLON-ALIGNED NO-LABEL
     ...
     IMAGE-69 AT ROW 6.25 COL 34
     IMAGE-70 AT ROW 6.25 COL 41
     RECT-29 AT ROW 1 COL 1
     RECT-31 AT ROW 10.5 COL 1
     RECT-33 AT ROW 8.25 COL 1
     "  Avaliado" VIEW-AS TEXT
          SIZE 9.14 BY .67 AT ROW 8 COL 7
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "<Avaliacao por Competencia - Funcionario>"
         HEIGHT             = 14.46
         WIDTH              = 72
         MAX-HEIGHT         = 31.29
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF C-Win /* <Avalia»’o por Compet¼ncia - Funcionÿrio> */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* <Avalia»’o por Compet¼ncia - Funcionÿrio> */
DO:
...
ON F5 OF v_cdn_avpes_padr_ini IN FRAME f-pg-sel /* C½digo Avalia»’o */
DO:
  /*{include/zoomvar.i &prog-zoom=object/sopm/zoom/z01pm132.w
                     &campo=v_cdn_avpes_padr
                     &campozoom=cdn_avpes_padr
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v_cdn_avpes_padr_ini C-Win
ON LEAVE OF v_cdn_avpes_padr_ini IN FRAME f-pg-sel /* C½digo Avalia»’o */
DO:
   assign v_nom_orig_inform = "DSP".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v_cdn_avpes_padr_ini C-Win
ON MOUSE-SELECT-DBLCLICK OF v_cdn_avpes_padr_ini IN FRAME f-pg-sel /* C½digo Avalia»’o */
DO:
  apply 'F5' to self.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL v_cdn_avpes_padr_ini C-Win
ON TAB OF v_cdn_avpes_padr_ini IN FRAME f-pg-sel /* C½digo Avalia»’o */
DO:
  apply 'leave' to self.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-empresa fi-cc-ini fi-cc-fim unid-lotac-ini unid-lotac-fim fi-dt-ini 
          fi-dt-fim cargo-ini cargo-fim v_cdn_avpes_padr_ini 
          v_cdn_avpes_padr_fim rs-tipo v_cdn_estab_ini v_cdn_estab_fim 
  ...
  ENABLE fi-empresa fi-cc-ini fi-cc-fim unid-lotac-ini unid-lotac-fim fi-dt-ini 
         ...
         v_cdn_funcionario_ini v_cdn_funcionario_fim v_cdn_estab_avaldor_ini 
         v_cdn_estab_avaldor_fim v_cdn_avaliador_ini v_cdn_avaliador_fim 
         IMAGE-16 IMAGE-18 IMAGE-19 IMAGE-20 IMAGE-47 IMAGE-48 IMAGE-59 
         IMAGE-60 IMAGE-61 IMAGE-62 IMAGE-63 IMAGE-64 IMAGE-65 IMAGE-66 
         IMAGE-67 IMAGE-68 IMAGE-69 IMAGE-70 RECT-29 RECT-31 RECT-33 
      WITH FRAME f-pg-sel IN WINDOW C-Win.
        ...
        for each avpes_emitid where avpes_emitid.cdn_avpes_padr              >= v_cdn_avpes_padr_ini
          ...
          if avpes_emitid.cdn_funcionario <> 0 then do: 
             for each funcionario where
                ...
                AND   (funcionario.cdn_cargo_basic) >= cargo-ini
                and   (funcionario.cdn_cargo_basic) <= cargo-fim
...
IF rs-tipo = 1  THEN DO: /* Grupo de Assunto */
    ...
    /*************** Imprime altera»„es **************************/
    /*************** Cabe¯alho ***********************************/
    chWorkSheet:Range("a1"):Value = "Avaliacao por Competencia - Grupo de Assunto     " + 
    ...
    chWorkSheet:Range("c3"):Value = "Lotacao ".  
    chWorkSheet:Range("d3"):Value = "Descricao Lotacao    ".  
    chWorkSheet:Range("e3"):Value = "Centro Custo".
    chWorkSheet:Range("f3"):Value = "Matricula   ".  
    chWorkSheet:Range("g3"):Value = "Funcionario ".
    chWorkSheet:Range("h3"):Value = "Grupo de Assunto".
    chWorkSheet:Range("i3"):Value = "Media Grupo ".       
    chWorkSheet:Range("j3"):Value = "   %  ". 
      ...
      BREAK BY tt-dados.cdn_empresa
        ...
        IF FIRST-OF (tt-dados.cdn_funcionario) THEN DO:
           ASSIGN chExcelApplication:range( "A" + STRING(i-linha) ):value = tt-dados.cdn_empresa .
                  ...
                  chExcelApplication:range( "f" + STRING(i-linha) ):value = tt-dados.cdn_funcionario.
                  chExcelApplication:range( "g" + STRING(i-linha) ):value =  tt-dados.nom_pessoa_fisic.
        END.
        ...
        IF LAST-OF (tt-dados.des_grp_avpes) THEN DO:  /* des_grp_avpes */
           ASSIGN chExcelApplication:range( "h" + STRING(i-linha) ):value = tt-dados.des_grp_avpes.
                  chExcelApplication:range( "i" + STRING(i-linha) ):value = tot-grupo / i-nr-grupo.
        ...
        IF LAST-OF (tt-dados.cdn_funcionario) THEN DO: /* cod_unid_lotac */
           ...
           chExcelApplication:range( "h" + STRING(i-linha) ):value = "* Media Funcionario => " .
           chExcelApplication:range( "i" + STRING(i-linha) ):value = d-tt-perc .
           chExcelApplication:Range( "i" + STRING(i-linha)):NumberFormat="##0,00" .
           chExcelApplication:range( "j" + STRING(i-linha) ):value = (d-tt-perc / i-tot-perc) * 100.
        ...
        IF LAST-OF(tt-dados.cdn_empresa) THEN DO:
           ...
           ASSIGN i-linha = i-linha + 2.
           chExcelApplication:range( "h" + STRING(i-linha) ):value = "*** MEDIA TOTAL - GRUPO ***".
           ASSIGN i-linha = i-linha + 2.
           ...
           FOR EACH tt-des_grp_avpes 
               ...
               IF LAST-OF( tt-des_grp_avpes.des_grp_avpes) THEN DO:
                  chExcelApplication:range( "h" + STRING(i-linha) ):value = tt-des_grp_avpes.des_grp_avpes.
                  chExcelApplication:range( "i" + STRING(i-linha) ):value = (i-media / i-tt-des-gr).
                  chExcelApplication:Range( "i" + STRING(i-linha)):NumberFormat="##0,00" .
           ...
           ASSIGN i-linha = i-linha + 2.
           chExcelApplication:range( "h" + STRING(i-linha) ):value = "* Media Geral => " .
           chExcelApplication:range( "i" + STRING(i-linha) ):value = tot-ger-emp / tot-nr-emp.
           chExcelApplication:Range( "i" + STRING(i-linha)):NumberFormat="##0,00" .
...
IF rs-tipo = 2  THEN DO: /* Pergunta */
    ...
    /*************** Imprime altera»„es **************************/
    /*************** Cabe¯alho ***********************************/
    chWorkSheet:Range("a1"):Value = "Avaliacao por Competencia - Pergunta "        + 
                                    "     Tipo:  " + String(v_cdn_avpes_padr_ini)  + 
                                    "   a    "     + String(v_cdn_avpes_padr_fim).
    chWorkSheet:Range("a1"):Font:Size = 12.
    chWorkSheet:Range("a1"):Font:Bold = TRUE.
    ...
    chWorkSheet:Range("g3"):Value = "Funcionario ".   
    chWorkSheet:Range("h3"):Value = "Grupo de Assunto".
    chWorkSheet:Range("i3"):Value = "Pergunta            ".    
    chWorkSheet:Range("j3"):Value = "Media Pergunta".    
      ...
      BREAK BY tt-dados.cdn_empresa
        ...
        IF FIRST-OF (tt-dados.des_grp_avpes) THEN DO:
           ASSIGN chExcelApplication:range( "A" + STRING(i-linha) ):value = tt-dados.cdn_empresa .
                  ...
                  chExcelApplication:range( "F" + STRING(i-linha) ):value = tt-dados.cdn_funcionario. 
                  chExcelApplication:range( "G" + STRING(i-linha) ):value = tt-dados.nom_pessoa_fisic . 
                  chExcelApplication:range( "h" + STRING(i-linha) ):value = tt-dados.des_grp_avpes.
                  chExcelApplication:range( "i" + STRING(i-linha) ):value = tt-dados.des_impres_item_avpes.
        END.
        ...
        IF /* LAST-OF (tt-dados.cdn_emp)     AND 
           ...
           ASSIGN /* chExcelApplication:range( "A" + STRING(i-linha) ):value = tt-dados.cdn_empresa .
                  ...
                  chExcelApplication:range( "i" + STRING(i-linha) ):value = tt-dados.des_impres_item_avpes. 
                  chExcelApplication:range( "j" + STRING(i-linha) ):value = tot-pergunta / i-nr-pergunta. 
                  chExcelApplication:Range( "j" + STRING(i-linha)):NumberFormat="##0,00".
        ...
        */
        ...
        IF LAST-OF(tt-dados.cdn_empresa) THEN DO:
           ...
           ASSIGN i-linha = i-linha + 2.
           chExcelApplication:range( "i" + STRING(i-linha) ):value = "*** MEDIA EMPRESA - PERGUNTA ***".
           ASSIGN i-linha = i-linha + 2.
           ...
           FOR EACH tt-des_impres_item_avpes 
               ...
               IF LAST-OF(tt-des_impres_item_avpes.cdn_item_avpes ) THEN DO:
                  chExcelApplication:range( "i" + STRING(i-linha) ):value = tt-des_impres_item_avpes.des_impres_item_avpes.
                  chExcelApplication:range( "j" + STRING(i-linha) ):value = i-media / i-tt-des-gr.
                  chExcelApplication:Range( "j" + STRING(i-linha)):NumberFormat="##0,00" .
               ...
               for each avpes_anexo where avpes_anexo.cdn_avpes_padr = avpes_emitid.cdn_avpes_padr no-lock
                   ...
                   if avail avpes_reg_mestre then do:        
                      ...
                      if v_cdn_campo_impres = 2 then 
                         run pi-respostas. /* Esta procedure mostra as respostas das avalia»„es, n’o foi criada um 
