...
/*------------------------------------------------------------------------
File.............: RELT01AB.w
Description......: PEDIDOS POR COMPRADOR - CT
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 03/01/08 - 15:34 - 01575407
OBS..............: Este fonte foi gerado pelo Data Viewer
------------------------------------------------------------------------*/

define variable c-prog-gerado as character no-undo initial "RELT01AB".
...
DEFINE VARIABLE i-tipo AS INTEGER LABEL "Tipo" INITIAL 4 
...
DEFINE VARIABLE rs-formato-arquivo AS INTEGER INITIAL 2 
VIEW-AS RADIO-SET HORIZONTAL 
RADIO-BUTTONS 
"Formato XLSX", 1,
"Formato CSV", 2
...
DEFINE FRAME f-pg-imp
    ...
    "Modo de Execu‡Æo" NO-LABEL
     ...
     rs-formato-arquivo AT ROW 8.92 COL 3 NO-LABEL
     rs-formato AT ROW 9.92 COL 3 HELP
...
DEFINE FRAME f-pg-sel
   da-dat-ordem-ini label "Data Ordem"
     at row 1 col 20 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dat-ordem-fim no-label
     at row 1 col 43 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   i-cod-emitente-ini label "Fornecedor"
   ...
   c-cod-comprado-ini label "Comprador"
     at row 3 col 20 colon-aligned
     view-as fill-in 
     size 13 by .88
     font 1

   c-cod-comprado-fim no-label
   ...
   c-cod-estabel-fim no-label
    ...
    IMAGE-3 AT ROW 02.00 COL 36
    IMAGE-5 AT ROW 03.00 COL 36
    IMAGE-7 AT ROW 04.00 COL 36
    IMAGE-2 AT ROW 01.00 COL 41
    IMAGE-4 AT ROW 02.00 COL 41
    IMAGE-6 AT ROW 03.00 COL 41
    IMAGE-8 AT ROW 04.00 COL 41
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "PEDIDOS POR COMPRADOR - CT - RELT01AB"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
END.
        ...
        ASSIGN c-arquivo:VISIBLE      IN FRAME f-pg-imp = NO
...
assign v-cod-prog-gerado = "RELT01AB".
...
PROCEDURE enable_UI :
   ...
   DISPLAY  
   da-dat-ordem-ini da-dat-ordem-fim
   i-cod-emitente-ini i-cod-emitente-fim
   c-cod-comprado-ini c-cod-comprado-fim
   ...
   DISPLAY rs-destino c-arquivo rs-execucao tb-parametro rs-formato rs-formato-arquivo
   WITH FRAME f-pg-imp IN WINDOW C-Win.
   ENABLE RECT-7 rs-destino bt-arquivo bt-config-impr c-arquivo RECT-9 rect-10 rs-execucao tb-parametro rs-formato rs-formato-arquivo
   WITH FRAME f-pg-imp IN WINDOW C-Win.
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   assign tt-param.usuario              = c-seg-usuario
          ...
          tt-param.formato-arquivo      = if input frame f-pg-imp rs-formato-arquivo   = "1" then 1 else 2
   ...
   if  tt-param.destino = 2 then
    ...
    assign tt-param.da-dat-ordem-ini = input frame f-pg-sel da-dat-ordem-ini
           tt-param.da-dat-ordem-fim = input frame f-pg-sel da-dat-ordem-fim
           tt-param.da-dat-ordem-fim = input frame f-pg-sel da-dat-ordem-fim
           ...
           tt-param.c-cod-comprado-ini = input frame f-pg-sel c-cod-comprado-ini
           tt-param.c-cod-comprado-fim = input frame f-pg-sel c-cod-comprado-fim
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "DTV/RELT0128.p".
