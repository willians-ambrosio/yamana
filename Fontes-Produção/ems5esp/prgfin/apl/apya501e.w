...
*******************************************************************************/
{include/i-prgvrs.i APYA501E 12.1.17.000}
...
DEFINE VARIABLE h_apya501e-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_apya501e-b02 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_apya501e-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_apya501e-v01 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-filho ASSIGN
         HIDDEN             = YES
         TITLE              = "Manutená∆o Documentos Operaá∆o"
         HEIGHT             = 16.75
         WIDTH              = 90
         MAX-HEIGHT         = 21.21
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadpaifilho-filho /* Manutená∆o Documentos Operaá∆o */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Manutená∆o Documentos Operaá∆o */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya501e-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_apya501e-v01 ).
       RUN set-position IN h_apya501e-v01 ( 2.75 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 5.50 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'ROF|Cambio' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.17 , 90.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya501e-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = prgfin/apl/apya501e-z01.w,
                     ProgVaPara = prgfin/apl/apya501e-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_apya501e-q01 ).
       RUN set-position IN h_apya501e-q01 ( 1.00 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_apya501e-v01. */
       RUN add-link IN adm-broker-hdl ( h_apya501e-q01 , 'Record':U , h_apya501e-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_apya501e-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_apya501e-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_apya501e-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_apya501e-q01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_apya501e-v01 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_apya501e-v01 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya501e-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = prgfin/apl/apya501e1.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_apya501e-b01 ).
       RUN set-position IN h_apya501e-b01 ( 9.83 , 3.72 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 86.00 ) */

       /* Links to BrowserCadastro2 h_apya501e-b01. */
       RUN add-link IN adm-broker-hdl ( h_apya501e-q01 , 'Record':U , h_apya501e-b01 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_apya501e-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prgfin/apl/apya501e-b02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = prgfin/apl/apya501e2.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_apya501e-b02 ).
       RUN set-position IN h_apya501e-b02 ( 9.83 , 3.72 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 86.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to BrowserCadastro2 h_apya501e-b02. */
       RUN add-link IN adm-broker-hdl ( h_apya501e-b01 , 'Record':U , h_apya501e-b02 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_apya501e-b02 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
-------------------------------------------------------------*/
   FIND es_operac_financ NO-LOCK WHERE
       ROWID(es_operac_financ) = gr-es_operac_financ NO-ERROR.
  
  FIND es_produt_financ NO-LOCK WHERE
       es_produt_financ.cod_produt_financ  = es_operac_financ.cod_produt_financ NO-ERROR.
  IF NOT AVAIL es_produt_financ THEN RETURN.
  ...
  THEN DO:
      MESSAGE "Informar contrato ROF "
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.

  IF es_produt_financ.log_drawdown AND es_operac_financ.arquivo = "" THEN DO:

      MESSAGE "Contrato DrawDown n∆o informado!"
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
  ...
  THEN DO:
      ...
      MESSAGE "Contrato n∆o possui ROF, DrawDown ou CÉmbio!"
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      RETURN NO-APPLY.
  END.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "APYA501E" "12.1.17.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  IF gr-es_operac_financ <> ? THEN
     RUN pi-reposiciona-query in h_apya501e-q01 (input gr-es_operac_financ).
