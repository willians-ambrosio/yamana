...
**       Data....: 17/01/11
**       Autor...: DATASUL S.A.
**       Objetivo: Exporta Cotaá∆o de Moeda
**       Vers∆o..: 1.00.000 - adm
*******************************************************************************/

define variable c-prog-gerado as character no-undo initial "ESI00727".
...
form ems2cadme.cotacao.mo-codigo column-label "Mo" format ">9" at 001
     ems2cadme.moeda.descricao column-label "Desc" format "x(12)" at 004
     ems2cadme.cotacao.ano-periodo column-label "Per°odo" format "9999/99" at 017
     ems2cadme.cotacao.cotacao[1] column-label "Cotaá∆o" format ">>>,>>9.99999999" at 025
     with down width 132 no-box stream-io frame f-relat-09-132.

/****************** Definiáao de Forms do Relat¢rio 80 Colunas ***************************************/ 
...
assign c-programa     = "esi00727"
       c-versao       = "2.00"
       c-revisao      = ".00.000"
       c-titulo-relat = "Exporta Cotaá∆o de Moeda"
       c-sistema      = "".
...
FIND FIRST es_cotacao NO-ERROR.
    IF AVAIL es_cotacao THEN DO:
        RUN esp/esidelete.i 'es_cotacao'.
    END.


for each ems2cadme.cotacao WHERE ano-periodo = string(YEAR(TODAY)) 
                                   + string(MONTH(TODAY),'99') NO-LOCK:
    FOR each ems2cadme.moeda no-lock
          where ems2cadme.moeda.mo-codigo = ems2cadme.cotacao.mo-codigo:

    assign v-num-reg-lidos = v-num-reg-lidos + 1.
    run pi-acompanhar in h-acomp(input string(v-num-reg-lidos)).

    /************************ EXPORTA INFORMAÄÂES PARA ILHA ************************************/
                                                                
            CREATE es_cotacao.

            ASSIGN es_cotacao.mo_codigo                       = cotacao.mo-codigo
                   es_cotacao.descricao                       = moeda.descricao
                   es_cotacao.ano_periodo                     = ano-periodo
                   es_cotacao.cotacao                         = cotacao.cotacao[DAY(TODAY)]
                   es_cotacao.data                            = TODAY
                   es_cotacao.id                              = string(cotacao.mo-codigo) + ano-periodo + STRING(TODAY).
  ...
  /*  /***  C‡DIGO PARA SA÷DA EM 80 COLUNAS ***/
    ...
    if  tt-param.formato = 1 then do:
        ...
        display stream str-rp ems2cadme.cotacao.mo-codigo
            ems2cadme.moeda.descricao
            ems2cadme.cotacao.ano-periodo
            ems2cadme.cotacao.cotacao[DAY(TODAY)]
