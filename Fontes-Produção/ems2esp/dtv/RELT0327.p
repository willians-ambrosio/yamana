...
**       Data....: 11/04/12
**       Autor...: DATASUL S.A.
**       Objetivo: TOTAL CONSUMIDO POR FAMILIA
**       VersÆo..: 1.00.000 - adm
**       OBS.....: Este fonte foi gerado pelo Data Viewer 3.00
*******************************************************************************/

define variable c-prog-gerado as character no-undo initial "RELT0327".
...
def var Tipodoc as character.
def var TotalMensal as decimal label "Total Mensal".
...
form item.fm-codigo column-label "Fam¡lia" format "x(8)" at 001
     movto-estoq.dt-trans column-label "Transa‡Æo" format "99/99/9999" at 010
     movto-estoq.it-codigo column-label "Item" format "x(16)" at 021
     movto-estoq.quantidade column-label "Qtde" format "->>>>,>>>,>>9.9999" at 038
     movto-estoq.un column-label "Un" format "xx" at 057
     TotalMensal column-label "Total Mensal" format ">>>>,>>>,>>9.99" at 060
     Tipodoc column-label "Tipo Doc" format "x(3)" at 076
     movto-estoq.nro-docto column-label "Documento" format "x(16)" at 085
     movto-estoq.conta-contabil column-label "Conta Cont bil" format "x(17)" at 102
...
assign c-programa     = "RELT0327"
       c-versao       = "2.06"
       c-revisao      = ".00.000"
       c-titulo-relat = "TOTAL CONSUMIDO POR FAMILIA"
       c-sistema      = "EMS".
...
run pi-inicializar in h-acomp(input "Progresso").
...
for each movto-estoq no-lock
         where movto-estoq.dt-trans >= da-dt-trans-ini and 
               movto-estoq.dt-trans <= da-dt-trans-fim,
    each item no-lock
         where item.it-codigo = movto-estoq.it-codigo and
               item.fm-codigo >= c-fm-codigo-ini and 
               item.fm-codigo <= c-fm-codigo-fim and
               (movto-estoq.esp-docto  >= 5 and
               movto-estoq.esp-docto  <= 7 or
               movto-estoq.esp-docto  >= 27 and
               movto-estoq.esp-docto  <= 33)
    break by item.it-codigo:

    assign v-num-reg-lidos = v-num-reg-lidos + 1.
    run pi-acompanhar in h-acomp(input string(movto-estoq.it-codigo)).

    assign TotalMensal = movto-estoq.valor-ggf-m[1] + movto-estoq.valor-mat-m[1] .
    assign Tipodoc = IF movto-estoq.esp-docto  = 5 THEN "DEV"ELSE IF movto-estoq.esp-docto = 6 THEN "DIV"ELSE IF movto-estoq.esp-docto = 7 THEN "DRM"ELSE IF movto-estoq.esp-docto = 27 THEN "RDD"ELSE IF movto-estoq.esp-docto = 28 THEN "REQ"ELSE IF movto-estoq.esp-docto = 30 THEN "RM"ELSE IF movto-estoq.esp-docto = 31 THEN "RRQ"ELSE "OUTRO".
...
if  tt-param.parametro then do:
   ...
   disp stream str-rp 
      ...
      c-fm-codigo-ini colon 19 "|< >|"   at 43 c-fm-codigo-fim no-label
