...
&SCOPED-DEFINE forEach        FIND usuar_financ_estab_apb NO-LOCK                                                  ~
                               ...
                               ASSIGN tt-titulo.cdn_fornec  = log_concur_titulo.cdn_fornec                         ~
                                      ...
                                      tt-titulo.dt_repros   = log_concur_titulo.dt_repros                          ~
                                      tt-titulo.valor       = LOG_concur_titulo.valor.                             ~
                                                                                                                   ~
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-11 tt-titulo.log_reprocess tt-titulo.cod_empresa tt-titulo.cod_estab tt-titulo.cod_esp tt-titulo.cdn_fornec tt-titulo.titulo tt-titulo.datahora tt-titulo.dt_repros   
...
&Scoped-Define ENABLED-OBJECTS BROWSE-6 BROWSE-12 BROWSE-11 l-repro-logs ~
bt-processar bt-buscar bt-refresh fi_cod_fornec_ini fi_cod_fornec_fim ~
fi-dt-atualiza-ini fi-dt-atualiza-fim fi-cod-estabel-ini fi-cod-estabel-fim ~
rd-tipo ed_log bt-deletar rt-button RECT-1 RECT-2 IMAGE-7 IMAGE-8 IMAGE-9 ~
IMAGE-10 IMAGE-11 IMAGE-12 RECT-4 RECT-5 
&Scoped-Define DISPLAYED-OBJECTS dt-log dt-repro l-repro-logs d-valor c-esp ~
c-forn c-par c-ser c-tit c_est c_nom_fornec fi_cod_fornec_ini ~
fi_cod_fornec_fim fi-dt-atualiza-ini fi-dt-atualiza-fim fi-cod-estabel-ini ~
fi-cod-estabel-fim rd-tipo ed_log 
...
DEFINE VARIABLE dt-log AS DATE FORMAT "99/99/99":U 
     LABEL "Data Log" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.

DEFINE VARIABLE dt-repro AS DATE FORMAT "99/99/99":U 
     LABEL "Data Repro." 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 NO-UNDO.
...
DEFINE VARIABLE l-repro-logs AS LOGICAL INITIAL no 
     LABEL "Exibir Somente Reprocessados" 
     VIEW-AS TOGGLE-BOX
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-11 w-livre _FREEFORM
  QUERY BROWSE-11 NO-LOCK DISPLAY
      ...
      tt-titulo.dt_repros       COLUMN-LABEL "Data Reprocessamento" FORMAT "99/99/9999":U
  ENABLE
...
DEFINE FRAME f-relat
     ...
     dt-log AT ROW 20.25 COL 13 COLON-ALIGNED WIDGET-ID 126
     dt-repro AT ROW 21.25 COL 13 COLON-ALIGNED WIDGET-ID 128
     l-repro-logs AT ROW 3 COL 88 WIDGET-ID 124
...
/* SETTINGS FOR FILL-IN d-valor IN FRAME f-relat
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN dt-log IN FRAME f-relat
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN dt-repro IN FRAME f-relat
   NO-ENABLE                                                            */
...
DO:
   OPEN QUERY browse-12 FOR EACH LOG_concur_erro 
    ...
    ASSIGN c_est        :screen-value in frame {&frame-name} = tt-titulo.cod_estab
           ...
           d-valor      :SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(tt-titulo.valor)
           dt-log       :SCREEN-VALUE in frame {&frame-name} = STRING(tt-titulo.datahora)
           dt-repro     :SCREEN-VALUE in frame {&frame-name} = STRING(tt-titulo.dt_repros).
    ...
    ELSE ASSIGN c_est        :screen-value in frame {&frame-name} = "" 
                ...
                d-valor      :SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
                dt-log       :SCREEN-VALUE in frame {&frame-name} = ""
                dt-repro     :SCREEN-VALUE in frame {&frame-name} = "".
...
&Scoped-define SELF-NAME l-repro-logs
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL l-repro-logs w-livre
ON VALUE-CHANGED OF l-repro-logs IN FRAME f-relat /* Exibir Somente Reprocessados */
DO:

   IF (l-repro-logs:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "yes") THEN
       fi-dt-atualiza-ini:LABEL = "Data Reprocessamento".
   ELSE 
       fi-dt-atualiza-ini:LABEL = "Data Log".
...
------------------------------------------------------------------------------*/
  DISPLAY dt-log dt-repro l-repro-logs d-valor c-esp c-forn c-par c-ser c-tit 
          c_est c_nom_fornec fi_cod_fornec_ini fi_cod_fornec_fim 
          fi-dt-atualiza-ini fi-dt-atualiza-fim fi-cod-estabel-ini 
          fi-cod-estabel-fim rd-tipo ed_log 
      WITH FRAME f-relat IN WINDOW w-livre.
  ENABLE BROWSE-6 BROWSE-12 BROWSE-11 l-repro-logs bt-processar bt-buscar 
         bt-refresh fi_cod_fornec_ini fi_cod_fornec_fim fi-dt-atualiza-ini 
 ...
 IF l-repro-logs THEN DO:
      ...
      FOR EACH log_concur_titulo NO-LOCK
       WHERE log_concur_titulo.cdn_fornec     >= fi_cod_fornec_ini
         ...
         AND log_concur_titulo.cod_estab      >= fi-cod-estabel-ini
         AND log_concur_titulo.cod_estab      <= fi-cod-estabel-fim
         AND DATE(log_concur_titulo.dt_repros) >= fi-dt-atualiza-ini
         AND DATE(log_concur_titulo.dt_repros) <= fi-dt-atualiza-fim
         AND LOG_concur_titulo.tipo = rd-tipo
         AND LOG_concur_titulo.LOG_reprocess:
    
         {&forEach} /*pre-processador criado no definitions, cria tabela temporaria tt-titulo e tt-rateio*/
