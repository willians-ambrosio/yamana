...
DEFINE TEMP-TABLE tt-controle-inv-esp-1 NO-UNDO LIKE controle-inv-esp
       field r-rowid as rowid.
DEFINE TEMP-TABLE tt-controle-inv-esp-2 NO-UNDO LIKE controle-inv-esp
       field r-rowid as rowid.
...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "Z01ES001"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    /*Altera‡Æo - 08/09/2006 - tech1007 - Altera‡Æo para exibir o nome do programa que executou o programa que ser  exibido no extrato de versÆo
        ...
        PUT "Z01ES001" AT 1 "2.00.00.000" AT 69 STRING(TODAY,'99/99/99') AT 84 STRING(TIME,'HH:MM:SS':U) AT 94 SKIP.
        /*FO 1329.898 - tech1139 - 01/08/2006 */
...
DEFINE VARIABLE hdboes001-1 AS HANDLE NO-UNDO.
DEFINE VARIABLE hdboes001-2 AS HANDLE NO-UNDO.
...
DEFINE QUERY brTable1 FOR 
      tt-controle-inv-esp-1 SCROLLING.

DEFINE QUERY brTable2 FOR 
      tt-controle-inv-esp-2 SCROLLING.
...
DEFINE BROWSE brTable1
  ...
  QUERY brTable1 NO-LOCK DISPLAY
      tt-controle-inv-esp-1.sequencia FORMAT ">>>>9":U
      tt-controle-inv-esp-1.cod-est-exec FORMAT "X(3)":U
      tt-controle-inv-esp-1.num-ordem FORMAT ">>9":U
      tt-controle-inv-esp-1.num-ord-inv FORMAT ">>>>>,>>>":U
      tt-controle-inv-esp-1.num-projeto FORMAT ">>>>9":U
      tt-controle-inv-esp-1.dt-trans FORMAT "99/99/9999":U
/* _UIB-CODE-BLOCK-END */

    WITH NO-ROW-MARKERS SEPARATORS SIZE 82 BY 6.75
         FONT 2.

DEFINE BROWSE brTable2

  QUERY brTable2 NO-LOCK DISPLAY
      tt-controle-inv-esp-2.dt-trans FORMAT "99/99/9999":U
      tt-controle-inv-esp-2.sequencia FORMAT ">>>>9":U
      tt-controle-inv-esp-2.cod-est-exec FORMAT "X(3)":U
      tt-controle-inv-esp-2.num-ordem FORMAT ">>9":U
      tt-controle-inv-esp-2.num-ord-inv FORMAT ">>>>>,>>>":U
      tt-controle-inv-esp-2.num-projeto FORMAT ">>>>9":U
/* _UIB-CODE-BLOCK-END */
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: tt-controle-inv-esp-1 T "?" NO-UNDO mgesp controle-inv-esp
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
      TABLE: tt-controle-inv-esp-2 T "?" NO-UNDO mgesp controle-inv-esp
      ADDITIONAL-FIELDS:
          field r-rowid as rowid
      END-FIELDS.
   END-TABLES.
...
/*&IF DEFINED(DBOVersion) <> 0 &THEN*/
        ...
        DEFINE TEMP-TABLE ttTable1Aux NO-UNDO LIKE tt-controle-inv-esp-1.
    
    
    
        DEFINE TEMP-TABLE ttTable2Aux NO-UNDO LIKE tt-controle-inv-esp-2.
...
------------------------------------------------------------------------------*/
    ...
    IF cFieldNames = "ROWID":U THEN DO:
            ...
            IF iNumPageAux = 2 THEN DO:
                IF AVAILABLE tt-controle-inv-esp-2 THEN
                    RUN repositionRecord IN WIDGET-HANDLE(cFieldHandles) (INPUT tt-controle-inv-esp-2.r-Rowid).
            ...
            IF iNumPageAux = 1 THEN DO:
                IF AVAILABLE tt-controle-inv-esp-1 THEN
                    RUN repositionRecord IN WIDGET-HANDLE(cFieldHandles) (INPUT tt-controle-inv-esp-1.r-Rowid).
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
            ...
            WHEN 1 THEN
                ...
                ASSIGN rCurrent1 = IF AVAILABLE tt-controle-inv-esp-1
                                       THEN ROWID(tt-controle-inv-esp-1)
                                       ELSE ?.
            ...
            WHEN 2 THEN
                ...
                ASSIGN rCurrent2 = IF AVAILABLE tt-controle-inv-esp-2
                                       THEN ROWID(tt-controle-inv-esp-2)
                                       ELSE ?.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
            ...
            WHEN 1 THEN DO:
                ...
                IF AVAILABLE tt-controle-inv-esp-1 THEN 
                    ASSIGN epc-rowid1 = tt-controle-inv-esp-1.r-Rowid.
                ELSE 
                    ASSIGN epc-rowid1 = ?.
...
/* _UIB-CODE-BLOCK-END */
            ...
            WHEN 2 THEN DO:
                ...
                IF AVAILABLE tt-controle-inv-esp-2 THEN 
                    ASSIGN epc-rowid2 = tt-controle-inv-esp-2.r-Rowid.
                ELSE 
                    ASSIGN epc-rowid2 = ?.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
            ...
            WHEN 1 THEN DO:
                ...
                IF AVAILABLE tt-controle-inv-esp-1 THEN
                    ASSIGN rLast = tt-controle-inv-esp-1.r-Rowid.
                ELSE
                    ASSIGN rLast = ?.
                ...
                /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
                    ...
                    RUN getBatchRecords IN hdboes001-1 (INPUT rLast,
                ...
                IF CAN-FIND(FIRST ttTable1Aux) THEN DO:
                    ...
                    FOR EACH ttTable1Aux:
                        CREATE tt-controle-inv-esp-1.

                        
                            BUFFER-COPY ttTable1Aux TO tt-controle-inv-esp-1.
                        
                        ASSIGN rLastCreated = ROWID(tt-controle-inv-esp-1).
                    END.
                
                    /*--- Abre o browse filho, para atualiza‡Æo dos dados ---*/
                    OPEN QUERY brTable1 FOR EACH tt-controle-inv-esp-1 NO-LOCK.
            ...
            WHEN 2 THEN DO:
                ...
                GET LAST brTable2 NO-LOCK.
                IF AVAILABLE tt-controle-inv-esp-2 THEN
                    ASSIGN rLast = tt-controle-inv-esp-2.r-Rowid.
                ELSE
                    ASSIGN rLast = ?.
                ...
                /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
                    ...
                    RUN getBatchRecords IN hdboes001-2 (INPUT rLast,
                                                        INPUT IF rLast = ? THEN NO ELSE YES,
                                                        INPUT ?,
                                                        OUTPUT iRowsReturned,
                ...
                IF CAN-FIND(FIRST ttTable2Aux) THEN DO:
                    ...
                    FOR EACH ttTable2Aux:
                        CREATE tt-controle-inv-esp-2.

                        
                            BUFFER-COPY ttTable2Aux TO tt-controle-inv-esp-2.
                        
                        ASSIGN rLastCreated = ROWID(tt-controle-inv-esp-2).
                    END.
                
                    /*--- Abre o browse filho, para atualiza‡Æo dos dados ---*/
                    OPEN QUERY brTable2 FOR EACH tt-controle-inv-esp-2 NO-LOCK.
                    
                    /*--- Seta view-port do browse filho para reposicionamento ---*/
                    brTable2:SET-REPOSITIONED-ROW(brTable2:DOWN IN FRAME fPage2) IN FRAME fPage2.
                    
                    /*--- Reposiciona browse filho no £ltimo registro ---*/
                    REPOSITION brTable2 TO ROWID rLastCreated NO-ERROR.
                    GET NEXT brTable2 NO-LOCK.
                    
                    /*--- Posiciona browse filho no pr¢ximo registro ---*/
                    brTable2:SELECT-FOCUSED-ROW().
    ...
    /*TESTE PRE-PROCESSADOR(VERSÇO DO BO)*/
        ...
        CASE pPageNumber:
                ...
                WHEN 1 THEN DO:
                    ...
                    IF AVAILABLE tt-controle-inv-esp-1 THEN DO:
                        ASSIGN rFirst = tt-controle-inv-esp-1.r-Rowid.
                    ...
                    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
                        ...
                        /*--- Retorna todos os registros do DBO filho, na temp-table ttTable1Aux ---*/
                        RUN getBatchRecordsPrev IN hdboes001-1 (INPUT rFirst,
                ...
                WHEN 2 THEN DO:
                    ...
                    GET FIRST brTable2 NO-LOCK.
                    IF AVAILABLE tt-controle-inv-esp-2 THEN DO:
                        ASSIGN rFirst = tt-controle-inv-esp-2.r-Rowid.
                            
                        /*--- Seta ponteiro do browse no registro correto ---*/
                        BROWSE brTable2:SELECT-FOCUSED-ROW().
                    END.
                    ELSE
                        ASSIGN rFirst = ?.
                    ...
                    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
                        ...
                        /*--- Retorna todos os registros do DBO filho, na temp-table ttTable2Aux ---*/
                        RUN getBatchRecordsPrev IN hdboes001-2 (INPUT rFirst,
                                                                INPUT IF rFirst = ? THEN NO ELSE YES,
                                                                INPUT ?,
                                                                OUTPUT iRowsReturned,
                                                                OUTPUT TABLE ttTable2Aux).
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
            ...
            WHEN 1 THEN DO WITH FRAME fPage1:
                ...
                IF AVAILABLE tt-controle-inv-esp-1 THEN
                    ASSIGN rLast = tt-controle-inv-esp-1.r-Rowid
                           rReposition = rowid(tt-controle-inv-esp-1).
                ELSE
                    ASSIGN rLast = ?
                           rReposition = ?.
            ...
            WHEN 2 THEN DO WITH FRAME fPage2:
                ...
                IF AVAILABLE tt-controle-inv-esp-2 THEN
                    ASSIGN rLast       = tt-controle-inv-esp-2.r-Rowid
                           rReposition = rowid(tt-controle-inv-esp-2).
                ELSE
                    ASSIGN rLast       = ?
                           rReposition = ?.
                
                IF  brTable2:MULTIPLE AND rReposition <> ? THEN
                ...
                CASE cEventBrowse:
                    WHEN "PAGE-DOWN":U THEN DO:
                        /*--- Seta view-port do browse para reposicionamento ---*/
                        brTable2:SET-REPOSITIONED-ROW(1).
                        
                        IF  rCurrent2 <> ? THEN DO:
                            /*--- Reposiciona browse no registro atual ---*/
                            REPOSITION brTable2 TO ROWID rCurrent2 NO-ERROR.
                            GET NEXT brTable2 NO-LOCK.
                            
                            /*--- Seleciona o £ltimo registro do view-port do browse ---*/
                            brTable2:SELECT-ROW(brTable2:NUM-ITERATIONS IN FRAME fPage2).
                        END.
                    END.
                    OTHERWISE DO:
                        /*--- Seta view-port do browse para reposicionamento ---*/
                        brTable2:SET-REPOSITIONED-ROW(brTable2:DOWN IN FRAME fPage2 - 1) IN FRAME fPage2.
                        
                        IF  rReposition <> ? THEN DO:
                            /*--- Reposiciona browse no registro atual ---*/
                            REPOSITION brTable2 TO ROWID rReposition NO-ERROR.
                            GET NEXT brTable2 NO-LOCK.
                            
                            /*--- Posiciona browse no pr¢ximo registro ---*/
                            brTable2:SELECT-FOCUSED-ROW().
                            brTable2:SELECT-NEXT-ROW().
                        END.
                    END.
                END CASE.
            END.
    ...
    /*TESTE PRE-PROCESSADOR(VERSÇO DO BO)*/
        ...
        CASE pPageNumber:
                ...
                WHEN 1 THEN DO WITH FRAME fPage1:
                    ...
                    IF  CAN-FIND(FIRST tt-controle-inv-esp-1) THEN DO:
                        /*--- Transfere dados da temp-table ttTable1Aux para a temp-table {&ttTable1} ---*/
                ...
                WHEN 2 THEN DO WITH FRAME fPage2:
                    ...
                    IF  CAN-FIND(FIRST tt-controle-inv-esp-2) THEN DO:
                        /*--- Transfere dados da temp-table ttTable2Aux para a temp-table {&ttTable2} ---*/
                    ...
                    CASE cEventBrowse:
                        WHEN "PAGE-UP":U THEN DO:
                            /*--- Seta view-port do browse para reposicionamento ---*/
                            brTable2:SET-REPOSITIONED-ROW(brTable2:DOWN IN FRAME fPage2).
                            ...
                            IF  rCurrent2 <> ? THEN DO:
                                ...
                                /*--- Seleciona o £ltimo registro do view-port do browse ---*/
                                brTable2:SELECT-ROW(1).
                            END.
                        END.
                        OTHERWISE DO:
                            /*--- Seta view-port do browse para reposicionamento ---*/
                            brTable2:SET-REPOSITIONED-ROW(1) IN FRAME fPage2.
...
run btb/btb918zb.p (input c-programa-mg97,
    ...
    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
        ...
        /*--- Destr¢i DBO Table1 ---*/
            ...
            IF VALID-HANDLE(hdboes001-1) THEN
                RUN destroy IN hdboes001-1.
    ...
    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
        ...
        /*--- Destr¢i DBO Table2 ---*/
            ...
            IF VALID-HANDLE(hdboes001-2) THEN
                RUN destroy IN hdboes001-2.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN c-programa-mg97 = CAPS("z01es001":U)
...
/* _UIB-CODE-BLOCK-END */
    ...
    /*--- Executa programa de folder ---*/
        ...
        RUN initializeFolders IN hFolder (INPUT FRAME fPage0:HANDLE,
                                          INPUT 
                                                ...
                                                ,
                                          INPUT "Chave, Data":U,
                                          INPUT NUM-ENTRIES("Chave, Data":U),
                                          INPUT ?,
                                          INPUT ?,
                                          INPUT ?,
    ...
    /*Colocado esta l¢gica para caso o desenvolvedor passar o handle da bo j  
     ...
     do programa Feita por Anderson tech485 28/08/2202*/
             ...
             IF VALID-HANDLE(hdboes001-1) THEN
                 RUN emptyrowerrors IN hdboes001-1 NO-ERROR.
        
        
             IF VALID-HANDLE(hdboes001-2) THEN
                 RUN emptyrowerrors IN hdboes001-2 NO-ERROR.
    ...
    IF NO
            ...
            OR NOT VALID-HANDLE(hdboes001-1) 
        
            
        
            OR NOT VALID-HANDLE(hdboes001-2)
    ...
    THEN DO:
        ...
        RUN utp/ut-msgs.p (INPUT "SHOW":U, 
                           INPUT 18881, 
                           INPUT CAPS("z01es001":U) + "~~":U + CAPS("2.00.00.000":U)).
    ...
    ELSE DO:
        /*Foi inclu¡da esta l¢gica para verifica‡Æo de erros relacionados com 
         a permissÆo de execu‡Æo do programa de dbo Altera‡Æo em 17/06/2002 por Anderson(tech540)*/
              ...
              RUN getrowerrors IN hdboes001-1 (OUTPUT TABLE rowerrors).
...
/* _UIB-CODE-BLOCK-END */
              ...
              RUN getrowerrors IN hdboes001-2 (OUTPUT TABLE rowerrors).
              IF CAN-FIND(FIRST RowErrors WHERE RowErrors.ErrorNumber <> 3 AND
                                                RowErrors.ErrorNumber <> 8 AND
                                                RowErrors.ErrorNumber <> 10 AND
                                                RowErrors.ErrorSubType = "ERROR":U) THEN DO:
    ...
    RUN openQueries IN THIS-PROCEDURE.
        ...
        IF VALID-HANDLE(hRange) THEN 
            ...
            RUN initializeMultipleRange IN hRange (INPUT THIS-PROCEDURE:HANDLE,
                                                   ...
                                                   INPUT "mgesp.controle-inv-esp.cod-est-exec,mgesp.controle-inv-esp.num-ord-inv,mgesp.controle-inv-esp.num-ordem,mgesp.controle-inv-esp.num-projeto,mgesp.controle-inv-esp.sequencia":U,
            ...
            RUN initializeMultipleRange IN hRange (INPUT THIS-PROCEDURE:HANDLE,
                                                   ...
                                                   INPUT "mgesp.controle-inv-esp.dt-trans":U,
                                                   INPUT "no",
                                                   OUTPUT hFirstField).
...
/* Query rebuild information for BROWSE brTable1
     _TblList          = "Temp-Tables.tt-controle-inv-esp-1"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.tt-controle-inv-esp-1.sequencia
     _FldNameList[2]   = Temp-Tables.tt-controle-inv-esp-1.cod-est-exec
     _FldNameList[3]   = Temp-Tables.tt-controle-inv-esp-1.num-ordem
     _FldNameList[4]   = Temp-Tables.tt-controle-inv-esp-1.num-ord-inv
     _FldNameList[5]   = Temp-Tables.tt-controle-inv-esp-1.num-projeto
     _FldNameList[6]   = Temp-Tables.tt-controle-inv-esp-1.dt-trans
     _Query            is OPENED
*/  /* BROWSE brTable1 */
...
/* Query rebuild information for BROWSE brTable2
     _TblList          = "Temp-Tables.tt-controle-inv-esp-2"
     _Options          = "NO-LOCK"
     _FldNameList[1]   = Temp-Tables.tt-controle-inv-esp-2.dt-trans
     _FldNameList[2]   = Temp-Tables.tt-controle-inv-esp-2.sequencia
     _FldNameList[3]   = Temp-Tables.tt-controle-inv-esp-2.cod-est-exec
     _FldNameList[4]   = Temp-Tables.tt-controle-inv-esp-2.num-ordem
     _FldNameList[5]   = Temp-Tables.tt-controle-inv-esp-2.num-ord-inv
     _FldNameList[6]   = Temp-Tables.tt-controle-inv-esp-2.num-projeto
     _Query            is OPENED
*/  /* BROWSE brTable2 */
...
RUN esp\esin001.w PERSISTENT SET hProgramImplant.
...
IF VALID-HANDLE(hProgramImplant)             AND
   ...
   hProgramImplant:FILE-NAME = "esp\esin001.w":U THEN
    /*--- Inicializa programa de implanta‡Æo ---*/
    RUN initializeInterface IN hProgramImplant.

IF VALID-HANDLE(hProgramImplant)             AND
   hProgramImplant:TYPE      = "PROCEDURE":U AND
   hProgramImplant:FILE-NAME = "esp\esin001.w":U THEN DO:
    ...
    IF gr-controle-inv-esp <> ? THEN DO:
        ASSIGN iRepositionPageNumber = 1
               rRepositionTable      = gr-controle-inv-esp.
...
IF VALID-HANDLE(hProgramImplant)             AND
   ...
   hProgramImplant:FILE-NAME = "esp\esin001.w":U THEN DO:
    ...
    /*--- Posiciona no £ltimo registro utilizado no programa de implanta‡Æo ---*/
    IF gr-controle-inv-esp <> ? THEN DO:
        ASSIGN iRepositionPageNumber = 2
               rRepositionTable      = gr-controle-inv-esp.
...
/*--- Evento de CURSOR-DOWN, PAGE-DOWN, OFF-END e END padrÆo para o browse da p gina 2 ---*/
    ...
    ON RETURN  OF BROWSE brTable2  DO:
         RUN returnValues IN THIS-PROCEDURE.
         APPLY "CLOSE":U TO THIS-PROCEDURE.
    END.
    
    ON CURSOR-DOWN OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyCursorDown para disparar o evento de CURSOR-DOWN ---*/
RUN applyCursorDown IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON END OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyEnd para disparar o evento de END ---*/
RUN applyEnd IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON OFF-END OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyOffEnd para disparar o evento de OFF-END ---*/
RUN applyOffEnd IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON PAGE-DOWN OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyPageDown para disparar o evento de PAGE-DOWN ---*/
RUN applyPageDown IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON VALUE-CHANGED OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyValueChanged para disparar o evento de VALUE-CHANGED ---*/
RUN applyValueChanged IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON MOUSE-SELECT-DBLCLICK OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyDblClick para disparar o evento de DBL-CLICK ---*/
RUN applyDblClick IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON CURSOR-UP OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyCursorUp para disparar o evento de CURSOR-UP ---*/
RUN applyCursorUp IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON HOME OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyHome para disparar o evento de HOME ---*/
RUN applyHome IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON OFF-HOME OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyPageDown para disparar o evento de OFF-HOME ---*/
RUN applyOffHome IN THIS-PROCEDURE (INPUT 2).
...
/* _UIB-CODE-BLOCK-END */
    ...
    ON PAGE-UP OF brTable2 IN FRAME fPage2 DO:
...
/*--- Executa procedure applyPageUp para disparar o evento de PAGE-UP ---*/
RUN applyPageUp IN THIS-PROCEDURE (INPUT 2).
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE(hdboes001-1) OR
       hdboes001-1:TYPE <> "PROCEDURE":U OR
       hdboes001-1:FILE-NAME <> "esbo\boes001.p":U THEN DO:
...
run pi_connect in h-servid-rpc ("esbo\boes001.p", yes, "").
...
repeat while rpc_exec("esbo\boes001.p") on stop undo rpc_block, retry rpc_block:
    ...
    then do:
        ...
        then do:
            if rpc_program("esbo\boes001.p") <> ? then do:
                ...
                then do:
                        ...
                        run value(rpc_program("esbo\boes001.p")) persistent set hdboes001-1 on rpc_server("esbo\boes001.p") transaction distinct no-error.
        ...
        if rpc_program("esbo\boes001.p") <> ? then do:
            ...
            then do:
                    ...
                    run value(rpc_program("esbo\boes001.p")) persistent set hdboes001-1 no-error.
    ...
    RUN setConstraintChave IN hdboes001-1 ( input ""      ,
                                              input "ZZZ"   ,
                                              input 0       ,
                                              input 99999999,
                                              input 0       ,
                                              ...
                                              input 99999   ,
                                              input 0       ,
                                              input 99999   ) NO-ERROR.
    
    /*:T--- Verifica se o DBO j  est  inicializado ---*/
    IF NOT VALID-HANDLE(hdboes001-2) OR
       hdboes001-2:TYPE <> "PROCEDURE":U OR
       hdboes001-2:FILE-NAME <> "esbo\boes001.p":U THEN DO:
...
repeat while rpc_exec("esbo\boes001.p") on stop undo rpc_block, retry rpc_block:
    ...
    then do:
        ...
        then do:
            if rpc_program("esbo\boes001.p") <> ? then do:
                ...
                then do:
                        ...
                        run value(rpc_program("esbo\boes001.p")) persistent set hdboes001-2 on rpc_server("esbo\boes001.p") transaction distinct no-error.
        ...
        if rpc_program("esbo\boes001.p") <> ? then do:
            ...
            then do:
                    ...
                    run value(rpc_program("esbo\boes001.p")) persistent set hdboes001-2 no-error.
   ...
   /* RUN setConstraintData IN {&hDBOTable2} ( input 01/01/0001,
                                             input 12/31/9999) NO-ERROR.*/
...
/* _UIB-CODE-BLOCK-END */
    ...
    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
        ...
        /*--- Abre query do DBO ---*/
        RUN openQueryStatic IN hdboes001-1 (INPUT "Chave":U).
        
        /*--- Retorna faixa de registro do DBO ---*/
        RUN getBatchRecords IN hdboes001-1 (INPUT IF iRepositionPageNumber = 1
                                                                  THEN rRepositionTable
                                                                  ELSE ?,
                                                          INPUT NO,
                                                          INPUT  40 ,
                                                          OUTPUT iRowsReturned,
                                                          OUTPUT TABLE tt-controle-inv-esp-1).
...
IF ((iConstraintPageNumber = 0 OR iConstraintPageNumber = 2) AND iRepositionPageNumber = 0) OR
   ((iRepositionPageNumber = 0 OR iRepositionPageNumber = 2) AND iConstraintPageNumber = 0) THEN DO:
...
/* _UIB-CODE-BLOCK-END */
    ...
    /*--- Manter compatibilidade dos thinTemplates com BO 1.1 e DBO 2.0 ---*/
        ...
        /*--- Abre query do DBO ---*/
        RUN openQueryStatic IN hdboes001-2 (INPUT "Data":U).
        
        /*--- Retorna faixa de registro do DBO ---*/
        RUN getBatchRecords IN hdboes001-2 (INPUT IF iRepositionPageNumber = 2
                                                                  THEN rRepositionTable
                                                                  ELSE ?,
                                                          INPUT NO,
                                                          INPUT  40 ,
                                                          OUTPUT iRowsReturned,
                                                          OUTPUT TABLE tt-controle-inv-esp-2).
    
    
    /*--- Abre a query associada ao browse da p gina {&PageNumber} ---*/
    OPEN QUERY brTable2 FOR EACH tt-controle-inv-esp-2 NO-LOCK.
    APPLY "VALUE-CHANGED":U TO BROWSE brTable2.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE tt-controle-inv-esp-1 THEN DO:
        CASE pcField:
            WHEN "cod-desp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-desp).
            WHEN "cod-emitente":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-emitente).
            WHEN "cod-emitente-desp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-emitente-desp).
            WHEN "cod-est-exec":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-est-exec).
            WHEN "cod-itiner":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-itiner).
            WHEN "cod-pto-contr":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.cod-pto-contr).
            WHEN "dt-trans":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.dt-trans).
            WHEN "embarque":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.embarque).
            WHEN "ent-comp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.ent-comp).
            WHEN "ent-real":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.ent-real).
            WHEN "ep-codigo":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.ep-codigo).
            WHEN "item":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.item).
            WHEN "narrativa":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.narrativa).
            WHEN "nat-operacao":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.nat-operacao).
            WHEN "nr-contrato":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.nr-contrato).
            WHEN "nro-docto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.nro-docto).
            WHEN "num-ord-inv":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.num-ord-inv).
            WHEN "num-ordem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.num-ordem).
            WHEN "num-pedido":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.num-pedido).
            WHEN "num-projeto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.num-projeto).
            WHEN "num-seq-item":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.num-seq-item).
            WHEN "numero-ordem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.numero-ordem).
            WHEN "sai-comp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.sai-comp).
            WHEN "sai-real":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.sai-real).
            WHEN "sequencia":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.sequencia).
            WHEN "serie-docto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.serie-docto).
            WHEN "tipo-doc":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.tipo-doc).
            WHEN "valor-origem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-1.valor-origem).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE tt-controle-inv-esp-2 THEN DO:
        CASE pcField:
            WHEN "cod-desp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-desp).
            WHEN "cod-emitente":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-emitente).
            WHEN "cod-emitente-desp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-emitente-desp).
            WHEN "cod-est-exec":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-est-exec).
            WHEN "cod-itiner":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-itiner).
            WHEN "cod-pto-contr":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.cod-pto-contr).
            WHEN "dt-trans":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.dt-trans).
            WHEN "embarque":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.embarque).
            WHEN "ent-comp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.ent-comp).
            WHEN "ent-real":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.ent-real).
            WHEN "ep-codigo":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.ep-codigo).
            WHEN "item":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.item).
            WHEN "narrativa":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.narrativa).
            WHEN "nat-operacao":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.nat-operacao).
            WHEN "nr-contrato":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.nr-contrato).
            WHEN "nro-docto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.nro-docto).
            WHEN "num-ord-inv":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.num-ord-inv).
            WHEN "num-ordem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.num-ordem).
            WHEN "num-pedido":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.num-pedido).
            WHEN "num-projeto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.num-projeto).
            WHEN "num-seq-item":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.num-seq-item).
            WHEN "numero-ordem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.numero-ordem).
            WHEN "sai-comp":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.sai-comp).
            WHEN "sai-real":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.sai-real).
            WHEN "sequencia":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.sequencia).
            WHEN "serie-docto":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.serie-docto).
            WHEN "tipo-doc":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.tipo-doc).
            WHEN "valor-origem":U THEN
                ASSIGN pcFieldValue = STRING(tt-controle-inv-esp-2.valor-origem).
        END CASE.
    END.
...
------------------------------------------------------------------------------*/
    ...
    CASE pPageNumber:
        WHEN 1 THEN
            ...
            RUN setConstraintChave IN hdboes001-1 (input fnIniRangeCharPage(INPUT 1, INPUT 1),
                                                     ...
                                                     input fnIniRangeIntPage (INPUT 1, INPUT 4),
                                                     input fnEndRangeIntPage (INPUT 1, INPUT 4),
                                                     input fnIniRangeIntPage (INPUT 1, INPUT 5),
                                                     input fnEndRangeIntPage (INPUT 1, INPUT 5)).
        
        WHEN 2 THEN
            /*:T--- Seta Constraints para o DBO Table2 ---*/
            RUN setConstraintData  IN hdboes001-2 (input fnIniRangeDatePage(INPUT 2, INPUT 1),
                                                     input fnEndRangeDatePage(INPUT 2, INPUT 1)).
    END CASE.
