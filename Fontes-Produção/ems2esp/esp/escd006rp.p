...
{include/i-prgvrs.i ESCD006RP 2.06.00.001}
/********************************************************************************************/
...
procedure pi-impressao:
   ...
   ASSIGN ch-excel:Range( "A" + STRING(i-cont-linha,'99999')):VALUE = "Nro. Lote Ctbl"     
          ...
          ch-excel:Range( "L" + STRING(i-cont-linha,'99999')):VALUE = "Usuario".

   ASSIGN i-cont-linha = i-cont-linha + 1.

   FOR EACH item_lancto_ctbl 
            WHERE item_lancto_ctbl.dat_lancto_ctbl >= tt-param.dat_lancto_ctbl_ini AND /* DATA DE INICIO DOS LANCAMENTOS */
                  item_lancto_ctbl.dat_lancto_ctbl <= tt-param.dat_lancto_ctbl_fim AND /* DATA FINAL DOS LANCAMENTOS */
                  /*item_lancto_ctbl.cod_indic_econ  = "Real"                        AND */
                  (item_lancto_ctbl.cod_cenar_ctbl = " " OR   item_lancto_ctbl.cod_cenar_ctbl     = "CONTSOC") /* AND
                  item_lancto_ctbl.cod_empresa = "CGO" */
            NO-LOCK,
            FIRST lote_ctbl OF item_lancto_ctbl
                  WHERE lote_ctbl.cod_modul_dtsul >= tt-param.cod_modul_dtsul_ini AND
                        lote_ctbl.cod_modul_dtsul <= tt-param.cod_modul_dtsul_fim
                  NO-LOCK
                  BY item_lancto_ctbl.dat_lancto_ctbl:

      RUN pi-acompanhar IN h-acomp (INPUT "Carregando Lote - " + STRING(lote_ctbl.num_lote_ctbl) + " - " + STRING(item_lancto_ctbl.dat_lancto_ctbl,"99/99/9999")).
      ...
      IF item_lancto_ctbl.cod_indic_econ  <> "Real" THEN
        DO:
          FIND FIRST aprop_lancto_ctbl
            ...
            /* AND   aprop_lancto_ctbl.cod_finalid_ecom    = item_lancto_ctbl.num_seq_lancto_ctbl */
            NO-ERROR.
      ...
      ASSIGN ch-excel:Range( "A" + STRING(i-cont-linha,'99999')):VALUE = lote_ctbl.num_lote_ctbl                                                                                                             
             ...
             ch-excel:Range( "I" + STRING(i-cont-linha,'99999')):VALUE = replace(replace(REPLACE(REPLACE(REPLACE(item_lancto_ctbl.des_histor_lancto_ctbl,CHR(10),""),CHR(9),""),CHR(13),""),";"," "),","," ")
             ch-excel:Range( "J" + STRING(i-cont-linha,'99999')):VALUE = item_lancto_ctbl.cod_cta_ctbl                                                                                                       
             ch-excel:Range( "K" + STRING(i-cont-linha,'99999')):VALUE = IF AVAIL aprop_lancto_ctbl AND item_lancto_ctbl.cod_indic_econ  <> "Real" THEN aprop_lancto_ctbl.val_lancto_ctbl ELSE item_lancto_ctbl.val_lancto_ctbl                                                                                                    
             ch-excel:Range( "L" + STRING(i-cont-linha,'99999')):VALUE = lote_ctbl.cod_usuar_ult_atualiz.
   ...
   ch-excel:Range("A1:L" + string(i-cont-linha - 1)):autofilter(,,,).

   ch-excel:Range("A1:L" + string(i-cont-linha - 1)):select.
...
procedure pi-impressao-csv:
   ...
   ASSIGN c-arquivo = SESSION:TEMP-DIRECTORY + "escd006.csv".
   ...
   OUTPUT STREAM st-arq TO VALUE(c-arquivo) NO-CONVERT NO-MAP.
   ...
   FOR EACH item_lancto_ctbl 
            WHERE item_lancto_ctbl.dat_lancto_ctbl >= tt-param.dat_lancto_ctbl_ini AND /* DATA DE INICIO DOS LANCAMENTOS */
                  item_lancto_ctbl.dat_lancto_ctbl <= tt-param.dat_lancto_ctbl_fim AND /* DATA FINAL DOS LANCAMENTOS */
                  item_lancto_ctbl.cod_indic_econ  = "Real"                        AND
                  (item_lancto_ctbl.cod_cenar_ctbl = " " OR   item_lancto_ctbl.cod_cenar_ctbl     = "CONTSOC")
            NO-LOCK,
            FIRST lote_ctbl OF item_lancto_ctbl
                  WHERE lote_ctbl.cod_modul_dtsul >= tt-param.cod_modul_dtsul_ini AND
                        lote_ctbl.cod_modul_dtsul <= tt-param.cod_modul_dtsul_fim
      ...
      PUT STREAM st-arq                                                                                                                          
          ...
          replace(replace(REPLACE(REPLACE(REPLACE(item_lancto_ctbl.des_histor_lancto_ctbl,CHR(10),""),CHR(9),""),CHR(13),""),";"," "),","," ")   ";"
          item_lancto_ctbl.cod_cta_ctbl                                                                                                          ";"
          item_lancto_ctbl.val_lancto_ctbl                                                                                                       ";"
          lote_ctbl.cod_usuar_ult_atualiz                                                                                                        ";" SKIP.
   END.

   OUTPUT STREAM st-arq CLOSE.
END PROCEDURE.
/********************************************************************************************/
/************************************** FIM DO PROGRAMA *************************************/
/********************************************************************************************/
