...
/****************************************************************************************** 
**         Programa: cc003-w01.w
**            Autor: Vando Ribeiro
**       Fornecedor: Grupo DKP
**       Data: 16/11/2018
** Change/Chamado: 
**    Objetivo: Relaciona usuario do financeiro para receber e-mail de pd com cond pagto
                inferior a 21 dias.
**
...
&Scoped-define INTERNAL-TABLES usuar_mestre esp_usu_email_pd
...
&Scoped-define FIELDS-IN-QUERY-brw-usu-sel esp_usu_email_pd.cod-usuario ~
usuar_mestre.nom_usuario 
&Scoped-define ENABLED-FIELDS-IN-QUERY-brw-usu-sel 
&Scoped-define QUERY-STRING-brw-usu-sel FOR EACH esp_usu_email_pd NO-LOCK, ~
      EACH usuar_mestre WHERE TRUE /* Join to esp_usu_email_pd incomplete */ ~
      AND usuar_mestre.cod_usuario = esp_usu_email_pd.cod-usuario NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-brw-usu-sel OPEN QUERY brw-usu-sel FOR EACH esp_usu_email_pd NO-LOCK, ~
      EACH usuar_mestre WHERE TRUE /* Join to esp_usu_email_pd incomplete */ ~
      AND usuar_mestre.cod_usuario = esp_usu_email_pd.cod-usuario NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-brw-usu-sel esp_usu_email_pd usuar_mestre
&Scoped-define FIRST-TABLE-IN-QUERY-brw-usu-sel esp_usu_email_pd
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brw-usu-sel w-livre _STRUCTURED
  QUERY brw-usu-sel NO-LOCK DISPLAY
      esp_usu_email_pd.cod-usuario FORMAT "x(12)":U
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-livre ASSIGN
         HIDDEN             = YES
         TITLE              = "Usu rios E-mail Compras com Condi‡Æo Pagto < 21 dias"
         HEIGHT             = 18.25
         WIDTH              = 83.57
         MAX-HEIGHT         = 18.96
         MAX-WIDTH          = 103.43
...
"usuar_mestre.cod_usuario" "Cod. Usu rio" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > emsfnd.usuar_mestre.nom_usuario
"usuar_mestre.nom_usuario" "Nome Usu rio" ? "character" ? ? ? ? ? ? no ? no no "50" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brw-usu-disp */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE brw-usu-sel
     _TblList          = "mgesp.esp_usu_email_pd,emsfnd.usuar_mestre WHERE mgesp.esp_usu_email_pd ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _Where[2]         = "usuar_mestre.cod_usuario = esp_usu_email_pd.cod-usuario"
     _FldNameList[1]   = mgesp.esp_usu_email_pd.cod-usuario
     _FldNameList[2]   = emsfnd.usuar_mestre.nom_usuario
     _Query            is OPENED
*/  /* BROWSE brw-usu-sel */
&ANALYZE-RESUME
...
ON END-ERROR OF w-livre /* Usu rios E-mail Compras com Condi‡Æo Pagto < 21 dias */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-livre w-livre
ON WINDOW-CLOSE OF w-livre /* Usu rios E-mail Compras com Condi‡Æo Pagto < 21 dias */
DO:
...
DO:
    ...
    FIND esp_usu_email_pd WHERE
         esp_usu_email_pd.cod-usuario = usuar_mestre.cod_usuario NO-LOCK NO-ERROR.
    IF NOT AVAIL esp_usu_email_pd THEN
    DO:
        CREATE esp_usu_email_pd.
        ASSIGN esp_usu_email_pd.cod-usuario = usuar_mestre.cod_usuario.
       {&OPEN-QUERY-brw-usu-sel}
    END.
    ELSE
    DO:
        RUN utp/ut-msgs.p ("show",17006,"Usu rio Inv lido~~ Usuario  " + esp_usu_email_pd.cod-usuario + "-" + usuar_mestre.nom_usuario + "  j  selecionado.").
...
DO:
    FIND esp_usu_email_pd WHERE
         esp_usu_email_pd.cod-usuario = usuar_mestre.cod_usuario EXCLUSIVE NO-ERROR.
    IF AVAIL esp_usu_email_pd THEN
    DO:
        DELETE esp_usu_email_pd.
       {&OPEN-QUERY-brw-usu-sel}
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "cc003-w01" "1.00.00.001"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "esp_usu_email_pd"}
