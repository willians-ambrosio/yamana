...
{include/i-prgvrs.i YMAC0002RP 1.02.00.003 } /*** 010003 ***/
/*******************************************************************************
**
**       Programa: prghur/esp/ymfp0006rp.p
**
**       Data....: agosto/2012.
**
**       Autor...: Totvs S.A.
**
**       Objetivo: Liquido Banco Bradesco Meio Magnetico
**
*******************************************************************************/

/****************************************************************************************** 
** 	   Programa: ymac0002rp.p - copia do programa ymfp0006rp.p
...
def var c-tipo-folha      as char format "x(20)"               no-undo.
...
/***************************** Definicao de Frames ****************************/
form
    "SELE€ÇO"                                                                 colon 20 skip
    tt-param.i-es-ini       label "Estabelecimento"                           colon 50 space(1) 
    "|<   >|"                                                                          space(1)
    tt-param.i-es-fim       no-label                                                   skip 
    tt-param.i-bco-liq-ini  label "Banco"                                     colon 50 space(1) 
    "|<   >|"                                                                          space(1)
    tt-param.i-bco-liq-fim  no-label
...
form 
    ...
    c-tipo-folha             label "Tipo Folha"                               colon 50 
    tt-param.i-parc          label "Parcela"                                  colon 50 
    tt-param.i-mes-ref       label "Mˆs Referˆncia"                           colon 50 
    tt-param.i-ano-ref       label "Ano Referˆncia"                           colon 50 
...
{utp/ut-liter.i Liquido_Folha_Bradesco_Van MFP C}
assign c-titulo-relat = return-value.
{utp/ut-liter.i "Folha de Pagamento" *}
   ...
   if avail rh_agenc_bcia and 
      ...
      do v_num_index = 1 to 6:
         if (v_num_index = 1 and
            ...
            for each funcionario of param_empres_rh no-lock where
                   ...
                   ((funcionario.cdn_categ_sal = 1 and tt-param.l-mensal) OR 
                    ...
                    (funcionario.cdn_categ_sal = 6 and tt-param.l-diarista)) AND 
                     ...
                     funcionario.cdn_forma_pagto_bco = v_cdn_forma_pagto and
                    (not v_log_folha_educnal or
                    (v_log_folha_educnal and funcionario.cdn_tip_contrat_func = 0))
                break by funcionario.cdn_estab
                      ...
                      by if tt-param.classifica = 1
                         then string(funcionario.cdn_funcionario,"99999999")
                              ...
                              else string(funcionario.cdn_cta_corren,"999999999")
                ...
                for each movto_calcul_func of funcionario no-lock where
                         ...
                         movto_calcul_func.idi_tip_fp                  = tt-param.i-tp-fl   and
                         movto_calcul_func.qti_parc_habilit_calc_fp    = tt-param.i-parc:
                    do i-inx = 1 to movto_calcul_func.qti_efp:
                       if movto_calcul_func.cdn_idx_efp_funcao_espcif[i-inx] = 52 then do:
                          assign d-valor-lanc  = movto_calcul_func.val_calcul_efp[i-inx].
                          leave.
                       end.
                    end.
                end.
                ...
                if d-valor-lanc > 0 then do:
                   if v_num_seq_lot = 0 then do:
                      ...
                      put stream ger-liq                                           
                          ...
                          tt-param.cod_dv_agencia                                  format "x(01)"                        /* DIg Ag 58  */                                               
...
if tt-param.parametro then do:
    ...
    assign l-mensal-2   :format in frame f-selec = trim(return-value)
           l-horista-2  :format in frame f-selec = trim(return-value)
           l-semanal-2  :format in frame f-selec = trim(return-value)
           l-quinzenal-2:format in frame f-selec = trim(return-value)
           l-tarefa-2   :format in frame f-selec = trim(return-value)
           l-diarista-2 :format in frame f-selec = trim(return-value)
           ...
           c-tipo-folha                          = {database/inpy/i01py127.i 04 tt-param.i-tp-fl}. 
