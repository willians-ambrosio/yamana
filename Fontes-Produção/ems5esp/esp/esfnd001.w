...
*****************************************************************************/
{include/i-prgvrs.i esfnd001 5.06.00.000}
...
DEFINE VARIABLE h_esfnd001-b01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esfnd001-q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_esfnd001-v01 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo de Parƒmetros"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 38.21
         MAX-WIDTH          = 182.86
         VIRTUAL-HEIGHT     = 38.21
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manuten‡Æo de Parƒmetros */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manuten‡Æo de Parƒmetros */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esfnd001-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_esfnd001-v01 ).
       RUN set-position IN h_esfnd001-v01 ( 2.83 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 89.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Parƒmetros' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.25 , 1.43 ) NO-ERROR.
       RUN set-size IN h_folder ( 12.88 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esfnd001-b01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp\esfnd001b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_esfnd001-b01 ).
       RUN set-position IN h_esfnd001-b01 ( 7.00 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.75 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esp/esfnd001-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = esp\esfnd001-z01.w,
                     ProgVaPara = esp\esfnd001-g01.w,
                     ProgIncMod = esp\esfnd001a.w,
                     Implantar = no':U ,
             OUTPUT h_esfnd001-q01 ).
       RUN set-position IN h_esfnd001-q01 ( 1.00 , 56.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_esfnd001-v01. */
       RUN add-link IN adm-broker-hdl ( h_esfnd001-q01 , 'Record':U , h_esfnd001-v01 ).
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_esfnd001-v01 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_esfnd001-b01. */
       RUN add-link IN adm-broker-hdl ( h_esfnd001-q01 , 'Record':U , h_esfnd001-b01 ).

       /* Links to SmartQuery h_esfnd001-q01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_esfnd001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_esfnd001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_esfnd001-q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_esfnd001-q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esfnd001-v01 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_esfnd001-v01 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_esfnd001-b01 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esfnd001" "5.06.00.000"}
