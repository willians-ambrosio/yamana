...
*******************************************************************************/
{include/i-prgvrs.i esap006-g01 2.06.00.001}
...
&Scoped-Define DISPLAYED-FIELDS emitente.cod-emitente 
...
DEFINE FRAME d-vapara
     emitente.cod-emitente AT ROW 2 COL 7.72 COLON-ALIGNED WIDGET-ID 2
          VIEW-AS FILL-IN 
          SIZE 10 BY .88
...
DO:
    find es-fornec-ap no-lock 
        where es-fornec-ap.cod-emitente = input frame {&frame-name} emitente.cod-emitente no-error.
    if not avail es-fornec-ap then do:
        {utp/ut-table.i mgesp es-fornec-ap 1}
        run utp/ut-msgs.p (input "show",
                           input 2,
                           input return-value).
        return no-apply.
    end.
    assign p-row-tabela = rowid(es-fornec-ap).
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE emitente THEN 
    DISPLAY emitente.cod-emitente 
      WITH FRAME d-vapara.
  ENABLE rt-button emitente.cod-emitente bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esap006-g01" "2.06.00.001"}
