...
/*------------------------------------------------------------------------
File.............: ESP001A.w
Description......: Relat¢rio espec¡fico investimentos
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 25/08/05 - 10:47 - adm
OBS..............: Este fonte foi gerado pelo Data Viewer
------------------------------------------------------------------------*/

define variable c-prog-gerado as character no-undo initial "ESP001A".
...
DEFINE FRAME f-pg-sel
   ...
   i-ep-codigo-fim no-label
     at row 1 col 38 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   c-cod-est-exec-ini label "Estabel Exe"
     at row 2 col 22 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   c-cod-est-exec-fim no-label
     at row 2 col 38 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   i-num-projeto-ini label "Num Projeto"
   ...
   i-num-ordem-ini label "N£mero Ordem"
     at row 4 col 22 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   i-num-ordem-fim no-label
     at row 4 col 38 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   i-cod-especialidade-ini label "Especialidade"
     at row 5 col 22 colon-aligned
     view-as fill-in 
     size 3 by .88
     font 1

   i-cod-especialidade-fim no-label
     at row 5 col 38 colon-aligned
     view-as fill-in 
     size 3 by .88
     font 1

   i-cod-sub-espec-ini label "Sub Espec"
     at row 6 col 22 colon-aligned
     view-as fill-in 
     size 3 by .88
     font 1

   i-cod-sub-espec-fim no-label
     at row 6 col 38 colon-aligned
     view-as fill-in 
     size 3 by .88
     font 1

   i-num-secao-ini label "Se‡Æo"
     at row 7 col 22 colon-aligned
     view-as fill-in 
     size 2 by .88
     font 1

   i-num-secao-fim no-label
     at row 7 col 38 colon-aligned
     view-as fill-in 
     size 2 by .88
     font 1

   IMAGE-1 AT ROW 01.00 COL 31
   IMAGE-2 AT ROW 01.00 COL 36
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
DEFINE FRAME f-pg-par
   ...
   data-corte
     at row 1.35 col 12 colon-aligned
     view-as fill-in
     size 11 by .88
     font 1

   l-moeda
     at row 2.35 col 12 colon-aligned     font 1
...
assign h-label = l-moeda:side-label-handle in frame f-pg-par
       h-label:row = l-moeda:row in frame f-pg-par - 0.1.

/* *************************  Create Window  ************************** */

CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Relat¢rio espec¡fico investimentos - ESP001A"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
ASSIGN data-corte = today.
ASSIGN l-moeda = 0.
assign v-cod-prog-gerado = "ESP001A".
...
&ENDIF
  ...
  assign wh-label-imp:screen-value = "Exporta‡Æo".
PAUSE 0 BEFORE-HIDE.
...
assign text-parametro:visible in frame f-pg-imp = no
       rect-10:visible in frame f-pg-imp = no
       tb-parametro:visible in frame f-pg-imp = no
       rs-formato:visible in frame f-pg-imp = no.
...
PROCEDURE enable_UI :
   ...
   DISPLAY  
   i-ep-codigo-ini i-ep-codigo-fim
   c-cod-est-exec-ini c-cod-est-exec-fim
   i-num-projeto-ini i-num-projeto-fim
   i-num-ordem-ini i-num-ordem-fim
   i-cod-especialidade-ini i-cod-especialidade-fim
   i-cod-sub-espec-ini i-cod-sub-espec-fim
   i-num-secao-ini i-num-secao-fim
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "ESP001A" + ".tmp".

    assign tt-param.data-corte = input frame f-pg-par data-corte
           tt-param.l-moeda = input frame f-pg-par l-moeda
           tt-param.i-ep-codigo-ini = input frame f-pg-sel i-ep-codigo-ini
           tt-param.i-ep-codigo-fim = input frame f-pg-sel i-ep-codigo-fim
           tt-param.c-cod-est-exec-ini = input frame f-pg-sel c-cod-est-exec-ini
           tt-param.c-cod-est-exec-fim = input frame f-pg-sel c-cod-est-exec-fim
           ...
           tt-param.i-num-ordem-ini = input frame f-pg-sel i-num-ordem-ini
           tt-param.i-num-ordem-fim = input frame f-pg-sel i-num-ordem-fim
           tt-param.i-cod-especialidade-ini = input frame f-pg-sel i-cod-especialidade-ini
           tt-param.i-cod-especialidade-fim = input frame f-pg-sel i-cod-especialidade-fim
           tt-param.i-cod-sub-espec-ini = input frame f-pg-sel i-cod-sub-espec-ini
           tt-param.i-cod-sub-espec-fim = input frame f-pg-sel i-cod-sub-espec-fim
           tt-param.i-num-secao-ini = input frame f-pg-sel i-num-secao-ini
           tt-param.i-num-secao-fim = input frame f-pg-sel i-num-secao-fim
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "inp/ESP0011.p".
