...
*******************************************************************************/
{include/i-prgvrs.i YMSA0001 1.00.00.000}
...
DEFINE VARIABLE h_q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 82 BY 12.96 WIDGET-ID 100.
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 65.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prghur/esp/YMSA0001-v01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01 ).
       RUN set-position IN h_v01 ( 2.79 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 81.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'prghur/esp/YMSA0001-q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = prghur/esp/YMSA0001-z01.w,
                     ProgVaPara = prghur/esp/YMSA0001-g01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01 ).
       RUN set-position IN h_q01 ( 1.00 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01 ).
       RUN add-link IN adm-broker-hdl ( h_q01 , 'Record':U , h_v01 ).

       /* Links to SmartQuery h_q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMSA0001" "1.00.00.000"}
