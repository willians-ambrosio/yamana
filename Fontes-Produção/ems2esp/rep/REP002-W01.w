...
/****************************************************************************************** 
**         Programa: rep002-w01.w
**            Autor: Felipe Vieira
**       Fornecedor: DKP
**       Data: 25/09/2018
** Change/Chamado: 
**    Objetivo: Cadastro de parƒmetros de bloqueio
**
...
******************************************************************************************/
{include/i-prgvrs.i REP002-W01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i REP002-W01 REP}
&ENDIF
...
DEFINE VARIABLE h_REP002-Q01 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_REP002-V01 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 78.57 BY 13.33 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadcom ASSIGN
         HIDDEN             = YES
         TITLE              = "Cadastro de Parƒmetros de Bloqueio"
         HEIGHT             = 13.46
         WIDTH              = 78.14
         MAX-HEIGHT         = 40.54
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF w-cadcom /* Cadastro de Parƒmetros de Bloqueio */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadcom w-cadcom
ON WINDOW-CLOSE OF w-cadcom /* Cadastro de Parƒmetros de Bloqueio */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 62.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'REP/REP002-V01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_REP002-V01 ).
       RUN set-position IN h_REP002-V01 ( 2.96 , 1.57 ) NO-ERROR.
       /* Size in UIB:  ( 11.25 , 77.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'REP/REP002-Q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = REP\REP002-Z01.w,
                     ProgVaPara = REP\REP002-G01.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_REP002-Q01 ).
       RUN set-position IN h_REP002-Q01 ( 2.25 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_REP002-V01. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_REP002-V01 ).
       RUN add-link IN adm-broker-hdl ( h_REP002-Q01 , 'Record':U , h_REP002-V01 ).

       /* Links to SmartQuery h_REP002-Q01. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_REP002-Q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_REP002-Q01 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_REP002-Q01 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_REP002-V01 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "rep002-w01" "1.00.00.000"}
