...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "esce0402b"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    /*Alteraá∆o - 08/09/2006 - tech1007 - Alteraá∆o para exibir o nome do programa que executou o programa que ser† exibido no extrato de vers∆o
        ...
        PUT "esce0402b" AT 1 "2.00.00.001" AT 69 STRING(TODAY,'99/99/99') AT 84 STRING(TIME,'HH:MM:SS':U) AT 94 SKIP.
        /*FO 1329.898 - tech1139 - 01/08/2006 */
...
/********************************************************************************
***
*** esce0402b - Gerador de Arquivo Excel XML para XLS
***
********************************************************************************/
{utp/ut-glob.i}
...
def temp-table tt-sald-terc      no-undo
    field tipo         as integer format "9"
    field cod-depos    as char format "x(3)"
    field it-codigo    like item.it-codigo
    field ge-codigo    as char format "x(14)"
    field class-fiscal as char format "x(14)"
    field vl-mat-m     like ped-curva.dec-1
    field vl-mob-m     like ped-curva.dec-1
    field vl-ggf-m     like ped-curva.dec-1
    field vl-mat-o     like ped-curva.dec-1
    field vl-mob-o     like ped-curva.dec-1
    field vl-ggf-o     like ped-curva.dec-1
    field quantidade   like ped-curva.quantidade
    index tt-st-tipo tipo
    index tt-st-tpit tipo
                     it-codigo
    index tt-st-imp  tipo
                     ge-codigo
                     class-fiscal
                     it-codigo.
...
procedure piExplodeEstrutura :
    ...
    for each estrutura where estrutura.it-codigo = i-item no-lock :  
        ...
        FIND FIRST tt-estrutura EXCLUSIVE-LOCK
            WHERE tt-estrutura.it-codigo    = estrutura.es-codigo
            AND   tt-estrutura.cod-estabel  = item.cod-estabel
            AND   tt-estrutura.codigo-refer = item.codigo-refer
            NO-ERROR.
        IF NOT AVAIL tt-estrutura THEN DO :
            create tt-estrutura.
            assign i-num-sequen              = i-num-sequen + 2
                   tt-estrutura.it-codigo    = estrutura.es-codigo
                   tt-estrutura.niv-mais-bai = item.niv-mais-bai
            ...
            IF tt-estrutura.codigo-refer = 'cpv' THEN DO :
                ASSIGN tt-estrutura.codigo-refer = "orig".

                create tt-estrutura.
                assign tt-estrutura.it-codigo    = estrutura.es-codigo
                       tt-estrutura.niv-mais-bai = item.niv-mais-bai
                       tt-estrutura.num-sequen   = i-num-sequen - 1
                       tt-estrutura.cod-estabel  = item.cod-estabel
                       tt-estrutura.inform-compl = item.inform-compl 
                       tt-estrutura.codigo-refer = item.codigo-refer
                       tt-estrutura.un           = item.un.
...
procedure piSaldoIni :
    define input  parameter  r-estrutura AS rowid                        no-undo.
    ...
    find first b-tt-estrutura exclusive-lock
        where rowid(b-tt-estrutura) = r-estrutura
        no-error.
    if avail b-tt-estrutura then do :
        /* Busca Saldo Atual em Quantidade */
        for each saldo-estoq fields (saldo-estoq.qtidade-atu)
            where saldo-estoq.it-codigo    = b-tt-estrutura.it-codigo
              and saldo-estoq.cod-estabel  = b-tt-estrutura.cod-estabel
              no-lock :       
        ...
        if not avail saldo-estoq then do:
           assign b-tt-estrutura.qtidade-ini        = 0 
            ...
            for each item-estab fields (item-estab.cod-estabel
                                        item-estab.it-codigo
                                        item-estab.sald-ini-ggf-m
                                        item-estab.sald-atu-ggf-m
                                        item-estab.sald-ini-mob-m
                                        item-estab.sald-atu-mob-m)
                where item-estab.it-codigo   = b-tt-estrutura.it-codigo
                  and item-estab.cod-estabel = b-tt-estrutura.cod-estabel
                  no-lock with 1 down:
        ...
        /** CALCULA SALDO TERC */
        if b-tt-estrutura.qtidade-ini = 0 then do :
            empty temp-table tt-sald-terc.
            run rep/reapi317.p (input tt-estrutura.cod-estabel,
                                input tt-estrutura.it-codigo,
                                input 0,
                                input yes,
                                input yes,
                                input yes,
                                input yes,
                                input yes,
                                input tt-param.data-ini - 1,
                                input yes,
                                output table tt-sald-terc). 
    
            for each  tt-sald-terc no-lock
                where tt-sald-terc.quantidade  > 0:

                assign b-tt-estrutura.qtidade-ini        = b-tt-estrutura.qtidade-ini        + tt-sald-terc.quantidade
                       b-tt-estrutura.sald-cash-ini[1]   = b-tt-estrutura.sald-cash-ini[1]   + tt-sald-terc.vl-ggf-m  
                       b-tt-estrutura.sald-nocash-ini[1] = b-tt-estrutura.sald-nocash-ini[1] + tt-sald-terc.vl-mob-m  .
            end.
            ...
            for each  tt-sald-terc no-lock
                ...
                assign b-tt-estrutura.sald-cash-ini[2]   = b-tt-estrutura.sald-cash-ini[2]   + tt-sald-terc.vl-ggf-m  
                       b-tt-estrutura.sald-nocash-ini[2] = b-tt-estrutura.sald-nocash-ini[2] + tt-sald-terc.vl-mob-m  .
            end.
        end.
...
procedure piMovtoItem :
    ...
    if avail b-tt-estrutura then do :
        ...
        for each ord-prod 
            ...
            for each movto-mat fields(movto-mat.esp-docto
               ...
               case movto-mat.esp-docto :
                   WHEN  1 THEN /* ACA - Acabado               */
                       assign b-tt-estrutura.qtidade-movto = b-tt-estrutura.qtidade-movto + movto-mat.quantidade.
                   WHEN  8 THEN /* EAC - estorno de acabado    */
                       assign b-tt-estrutura.qtidade-movto = b-tt-estrutura.qtidade-movto - movto-mat.quantidade.
        ...
        if b-tt-estrutura.qtidade-from > 0 then
            assign b-tt-estrutura.qtidade-movto = 0.
...
PROCEDURE piMovtoCPV :
    ...
    IF AVAIL b-tt-estrutura THEN DO :
        ...
        for each movto-estoq fields (movto-estoq.tipo-trans
                                     ...
                                     movto-estoq.conta-contabil) use-index item-data
            where movto-estoq.cod-estabel    = b-tt-estrutura.cod-estabel
            and   movto-estoq.it-codigo      = b-tt-estrutura.it-codigo
            and   movto-estoq.dt-trans      >= da-data-ini
            and   movto-estoq.dt-trans      <= da-data-fin
            and   movto-estoq.conta-contabil = "5110100400000000"
            and  (movto-estoq.esp-docto      = 20  or   
                  movto-estoq.esp-docto      = 21  or   
                  movto-estoq.esp-docto      = 22) no-lock :
            /*
            if b-tt-estrutura.codigo-refer <> "cpv" then do :
        ...
        IF b-tt-estrutura.codigo-refer = "cpv" THEN
            ASSIGN b-tt-estrutura.qtidade-from        = b-tt-estrutura.qtidade-to        * -1
                   b-tt-estrutura.sald-cash-from[1]   = b-tt-estrutura.sald-cash-to[1]   * -1
                   b-tt-estrutura.sald-cash-from[2]   = b-tt-estrutura.sald-cash-to[2]   * -1 
                   b-tt-estrutura.sald-nocash-from[1] = b-tt-estrutura.sald-nocash-to[1] * -1
                   b-tt-estrutura.sald-nocash-from[2] = b-tt-estrutura.sald-nocash-to[2] * -1.
...
procedure piExecute :
    ...
    for each tt-estrutura
        by tt-estrutura.niv-mais-bai descending
        by tt-estrutura.num-sequen descending.

        IF tt-estrutura.codigo-refer <> "cpv" THEN

        run piSaldoIni(input ROWID(tt-estrutura),
        ...
        run piMovtoItem(input ROWID(tt-estrutura),
                        input tt-param.data-ini,
                        input tt-param.data-fim).                                               
        
        run piMovtoCPV(input ROWID(tt-estrutura),
                       input tt-param.data-ini,
                       input tt-param.data-fim).
        ...
        assign tt-estrutura.qtidade-fin        = tt-estrutura.qtidade-ini        + tt-estrutura.qtidade-from        + tt-estrutura.qtidade-movto        + tt-estrutura.qtidade-to        + tt-estrutura.qtidade-ajust 
