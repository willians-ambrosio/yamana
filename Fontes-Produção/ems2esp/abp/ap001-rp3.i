        ...
        REPEAT:
             ...
             IF TRIM(ENTRY(1,c-dados,"|")) = "DETAIL" THEN DO:
                ...
                ASSIGN tt-dados.BATCH_id    = TRIM(ENTRY(2,c-dados,"|"))
                  ...
                  ELSE ASSIGN tt-dados.portador   = fornec_financ.cod_portador
...
PROCEDURE pi_tabela_log:
    ...
    FOR EACH tt_log BREAK BY tt_log.cod_estab
        ...
        IF FIRST-OF(tt_log.titulo) THEN DO:
            ...
            FOR EACH LOG_concur_rateio 
                    WHERE LOG_concur_rateio.id = log_concur_titulo.id:
...
PROCEDURE pi-report-excel:
    ...
    FOR EACH LOG_concur_titulo NO-LOCK WHERE
        ...
        BREAK BY log_concur_titulo.cod_empresa     
              BY LOG_concur_titulo.tipo        
              BY log_concur_titulo.cod_estab:

        IF FIRST-OF(LOG_concur_titulo.cod_empresa) THEN DO:
        ...
        IF FIRST-OF(log_concur_titulo.cod_estab) 
            THEN d-vl-tot-est = 0.

        IF FIRST-OF(LOG_concur_titulo.tipo)
             THEN d-vl-tot-tipo = 0.

        ASSIGN d-vl-tot-emp  = d-vl-tot-emp  + LOG_concur_titulo.valor
               d-vl-tot-est  = d-vl-tot-est  + LOG_concur_titulo.valor
               d-vl-tot-tipo = d-vl-tot-tipo + LOG_concur_titulo.valor
               d-tot-geral   = d-tot-geral   + LOG_concur_titulo.valor.

        RUN pi-dados-excel.

        IF LAST-OF(LOG_concur_titulo.cod_estab) 
            THEN RUN pi-excel_total (INPUT "Total Estabelecimento " + LOG_concur_titulo.cod_estab,
                                     INPUT  d-vl-tot-est).

        IF LAST-OF(LOG_concur_titulo.cod_empresa) THEN
            RUN pi-excel_total (INPUT "Total Empresa " + LOG_concur_titulo.cod_empresa,
                                INPUT d-vl-tot-emp).      

        IF LAST-OF(LOG_concur_titulo.tipo) 
            THEN RUN pi-excel_total (INPUT "Total processamento " + (IF LOG_concur_titulo.tipo = 1 THEN "ERRO" ELSE "SUCESSO"),
                                     INPUT d-vl-tot-tipo).

        IF LAST-OF(LOG_concur_titulo.cod_empresa) 
            THEN DO: 
...
/*         ch-excel:Range(cRange):Merge.           */
           ...
           ch-excel:Range("A" + STRING(i-cont-linha) + ":L" + STRING(i-cont-linha)):Interior:ColorIndex = 40.
