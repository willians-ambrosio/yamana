...
*******************************************************************************/
{include/i-prgvrs.i YMGO0003 1.02.10.000 } /*** 01010001 ***/
...
l_log_corporativo bt-ok bt-cancela bt-ajuda 
...
DEFINE VARIABLE l_log_corporativo AS LOGICAL INITIAL no 
     LABEL "Corporativo" 
     VIEW-AS TOGGLE-BOX
     SIZE 14 BY 1.25 NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME d-vapara
     c_cdn_empresa AT ROW 1.21 COL 30 COLON-ALIGNED WIDGET-ID 2
     c_cdn_estab AT ROW 2.21 COL 30 COLON-ALIGNED WIDGET-ID 4
     l_log_corporativo AT ROW 3 COL 32 WIDGET-ID 6
...
ON GO OF FRAME d-vapara /* V  Para M‚trica PLR */
DO:
FIND tip_metricas_plr NO-LOCK
    WHERE tip_metricas_plr.cdn_empresa EQ INPUT FRAME {&FRAME-NAME} c_cdn_empresa
      AND tip_metricas_plr.cdn_estab EQ INPUT FRAME {&FRAME-NAME} c_cdn_estab
      AND tip_metricas_plr.log_corporativo EQ INPUT FRAME {&FRAME-NAME} l_log_corporativo NO-ERROR.

IF NOT AVAILABLE tip_metricas_plr THEN DO:
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT "M‚tricas PLR").
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(tip_metricas_plr).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para M‚trica PLR */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_cdn_empresa c_cdn_estab l_log_corporativo 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cdn_empresa c_cdn_estab l_log_corporativo bt-ok bt-cancela 
         bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMGO0003" "1.02.00.000"}
