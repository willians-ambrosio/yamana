...
RUN findWidget (INPUT "i-ordem",        INPUT "FILL-IN",    INPUT p-wgh-frame, OUTPUT wh-cc0301a-i-ordem     ).
...
IF  p-ind-event  = "BEFORE-INITIALIZE":U  AND
    ...
    IF VALID-HANDLE(wh-cc0301a-bt-ok-ant) THEN DO:
        ...
        CREATE BUTTON wh-cc0301a-bt-ok-new
        ASSIGN FRAME     = wh-cc0301a-bt-ok-ant:FRAME
               WIDTH     = wh-cc0301a-bt-ok-ant:WIDTH
               HEIGHT    = wh-cc0301a-bt-ok-ant:HEIGHT
               LABEL     = wh-cc0301a-bt-ok-ant:LABEL + '*'
               COLUMN    = wh-cc0301a-bt-ok-ant:COLUMN
               ROW       = wh-cc0301a-bt-ok-ant:ROW
               SENSITIVE = YES
               VISIBLE   = YES
               TRIGGERS:
                    ON CHOOSE PERSISTENT RUN upc/cc0301a-u02.p(INPUT p-wgh-object).
