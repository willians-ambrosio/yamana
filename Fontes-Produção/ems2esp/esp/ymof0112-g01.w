...
&Scoped-Define ENABLED-OBJECTS rt-button c_estado bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_estado 
...
DEFINE VARIABLE c_estado AS CHARACTER FORMAT "x(4)":U 
     LABEL "UF" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_estado AT ROW 1.5 COL 25 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND es-uf-estorno-cred NO-LOCK
    WHERE es-uf-estorno-cred.estado EQ INPUT FRAME {&FRAME-NAME} c_estado NO-ERROR.

IF NOT AVAILABLE es-uf-estorno-cred THEN DO:
    {utp/ut-table.i mgesp es-uf-estorno-cred 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-uf-estorno-cred).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_estado 
      WITH FRAME d-vapara.
  ENABLE rt-button c_estado bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMOF0112-G01" "1.00.00.000"}
