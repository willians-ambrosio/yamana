...
PROCEDURE pi_processa_padrao:
    ...
    do:
        ...
        if avail ext_bord_ap and ext_bord_ap.log_imposto then do:
    ...
    if avail tt_param_program_formul then do:
            ...
            ASSIGN data-faturamento-aux = DATE(int(SUBSTRING(v_dt_periodo,3,2)) - 1 ,DAY(DATE(int(SUBSTRING(v_dt_periodo,3,2)) - 1 MOD 12 + 1, 1, int(SUBSTRING(v_dt_periodo,5,4)) - 1 ) - 1),int(SUBSTRING(v_dt_periodo,5,4))).
    ...
    if avail compl_impto_retid_ap then
        ...
        if avail ext_item_bord_ap then 
           ASSIGN v_cod_receita      = STRING(ext_item_bord_ap.cod_receita,"999999")
                  v_cod_referencia   = fill('0',17 - length(trim(ext_item_bord_ap.referencia))) + trim(ext_item_lote_impl_ap.referencia) 
                  ...
                  v_cod_id_contrib   = ext_item_bord_ap.id_fgts
                  v_cod_id_contrib   = replace(replace(replace(v_cod_id_contrib,"-",""),".",""),"/","")
