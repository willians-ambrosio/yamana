...
**       Data....: 30/03/12
**       Autor...: DATASUL S.A.
**       Objetivo: Relat¢rio Materiais Alocados
**       VersÆo..: 1.00.000 - adm
**       OBS.....: Este fonte foi gerado pelo Data Viewer 3.00
*******************************************************************************/

define variable c-prog-gerado as character no-undo initial "ESP03927".
...
define temp-table tt-param
    ...
    field c-it-codigo-ini like reservas.it-codigo
    field c-it-codigo-fim like reservas.it-codigo
    field da-dt-reserva-ini like reservas.dt-reserva
    field da-dt-reserva-fim like reservas.dt-reserva
.
...
def new shared var c-it-codigo-ini like reservas.it-codigo format "x(16)" initial "" no-undo.
def new shared var c-it-codigo-fim like reservas.it-codigo format "x(16)" initial "ZZZZZZZZZZZZZZZZ" no-undo.
def new shared var da-dt-reserva-ini like reservas.dt-reserva format "99/99/9999" initial "01/01/1800" no-undo.
def new shared var da-dt-reserva-fim like reservas.dt-reserva format "99/99/9999" initial "12/31/9999" no-undo.
...
def var de-quant-aloc-tt-001 like reservas.quant-aloc no-undo.
def var de-quant-orig-tt-002 like reservas.quant-orig no-undo.
...
form reservas.it-codigo column-label "Item Reserva" format "x(16)" at 001
     reservas.cod-depos column-label "Dep" format "x(3)" at 018
     reservas.quant-aloc column-label "Qtde Aloc" format "->>>>>,>>9.9999" at 022
     reservas.nr-ord-produ column-label "Ord Prod" format ">>>,>>>,>>9" at 038
     reservas.quant-orig column-label "Qtde" format "->>>>>,>>9.9999" at 051
...
assign c-programa     = "ESP03927"
       c-versao       = "2.06"
       c-revisao      = ".00.000"
       c-titulo-relat = "Relat¢rio Materiais Alocados"
       c-sistema      = "Relat¢rio".
...
assign i-ep-codigo-usuario = tt-param.ep-codigo
       ...
       da-dt-reserva-ini = tt-param.da-dt-reserva-ini
       da-dt-reserva-fim = tt-param.da-dt-reserva-fim
.

def var l-imprime as logical no-undo.


        assign de-quant-aloc-tt-001 = 0
               de-quant-orig-tt-002 = 0.
assign l-imprime = no.
...
for each reservas no-lock
         where reservas.dt-reserva >= da-dt-reserva-ini and 
               reservas.dt-reserva <= da-dt-reserva-fim and
               reservas.it-codigo >= c-it-codigo-ini and 
               reservas.it-codigo <= c-it-codigo-fim,
    each item no-lock
         where item.it-codigo = reservas.it-codigo and
               reservas.quant-aloc  > 0:

    assign v-num-reg-lidos = v-num-reg-lidos + 1.
    run pi-acompanhar in h-acomp(input string(v-num-reg-lidos)).

    assign de-quant-aloc-tt-001 = de-quant-aloc-tt-001 + 
                                          reservas.quant-aloc
           de-quant-orig-tt-002 = de-quant-orig-tt-002 + 
                                          reservas.quant-orig.
    ...
    if  tt-param.formato = 1 then do:
        ...
        display stream str-rp reservas.it-codigo
            reservas.cod-depos
            reservas.quant-aloc
            reservas.nr-ord-produ
            reservas.quant-orig
...
if  l-imprime = no then do:
    ...
    display stream str-rp "---------------" @ 
            ...
            with stream-io frame f-relat-09-80.
    down stream str-rp with frame f-relat-09-80.
        put stream str-rp de-quant-aloc-tt-001 format "->>>>>,>>9.9999" to 036.
        put stream str-rp de-quant-orig-tt-002 format "->>>>>,>>9.9999" to 065.
...
if  tt-param.parametro then do:
   ...
   disp stream str-rp 
      c-it-codigo-ini colon 17 "|< >|"   at 41 c-it-codigo-fim no-label
      da-dt-reserva-ini colon 17 "|< >|"   at 41 da-dt-reserva-fim no-label
   ...
   disp stream str-rp "   Relat¢rio de Materiais Alocados"
