...
**       Data....: 18/10/10
**       Autor...: DATASUL S.A.
**       Objetivo: Relat¢rio Grupo x Programa x Empresa x Usu rio
**       VersÆo..: 1.00.000 - 30100979
**       OBS.....: Este fonte foi gerado pelo Data Viewer 3.00
*******************************************************************************/

define variable c-prog-gerado as character no-undo initial "RELAT227".
...
form usuar_mestre.cod_usuario column-label "Usu rio" format "x(12)" at 001
     ...
     segur_empres_usuar.cod_empresa column-label "Empresa" format "x(3)" at 047
     usuar_grp_usuar.cod_grp_usuar column-label "Grupo" format "x(3)" at 055
     grp_usuar.des_grp_usuar column-label "Descri‡Æo" format "x(32)" at 061 skip
     prog_dtsul_segur.cod_prog_dtsul column-label "Programa" format "x(50)" at 001
...
assign c-programa     = "relat227"
       c-versao       = "2.00"
       c-revisao      = ".00.000"
       c-titulo-relat = "Relat¢rio Grupo x Programa x Empresa x Usu rio"
       c-sistema      = "".
...
run pi-inicializar in h-acomp(input "Descri‡Æo do Grupo").
...
for each usuar_mestre no-lock
    ...
    each segur_empres_usuar no-lock
         where segur_empres_usuar.cod_usuario = usuar_mestre.cod_usuario,
    each usuar_grp_usuar no-lock
         where usuar_grp_usuar.cod_usuario = segur_empres_usuar.cod_usuario,
    each grp_usuar no-lock
         where grp_usuar.cod_grp_usuar = usuar_grp_usuar.cod_grp_usuar and
               grp_usuar.cod_grp_usuar >= c-cod_grp_usuar-ini and 
               grp_usuar.cod_grp_usuar <= c-cod_grp_usuar-fim,
    ...
    run pi-acompanhar in h-acomp(input string(grp_usuar.des_grp_usua)).
    ...
    put stream str-rp unformatted usuar_mestre.cod_usuario ";"
            ...
            prog_dtsul.des_prog_dtsul skip.
