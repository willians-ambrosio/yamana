...
/*------------------------------------------------------------------------
File.............: ESPCT001.w
Description......: Relatorio Contratos e Saldos
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 25/09/09 - 17:44 - 30100979
OBS..............: Este fonte foi gerado pelo Data Viewer
------------------------------------------------------------------------*/

define variable c-prog-gerado as character no-undo initial "ESPCT001".
...
def var ind-sit-contrat-tmp as char no-undo.
def new shared var ind-sit-contrat as integer format ">9" label "Contratos Emitidos" view-as radio-set  horizontal radio-buttons "Emitido",1.
...
DEFINE FRAME f-pg-sel
   ...
   c-cod-estabel-fim no-label
     at row 1 col 43 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   i-nr-contrato-ini label "Nr Contrato"
     at row 2 col 22 colon-aligned
     view-as fill-in 
     size 10 by .88
     font 1

   i-nr-contrato-fim no-label
     at row 2 col 43 colon-aligned
     view-as fill-in 
     size 10 by .88
     font 1

   da-dt-ini-validade-ini label "In¡cio"
     at row 3 col 22 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dt-ini-validade-fim no-label
     at row 3 col 43 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dt-ter-validade-ini label "Data T‚rmino"
     at row 4 col 22 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dt-ter-validade-fim no-label
     at row 4 col 43 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   IMAGE-1 AT ROW 01.00 COL 36
   IMAGE-2 AT ROW 01.00 COL 41
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
DEFINE FRAME f-pg-par
   ...
   v-cod-tipo-grafico-dv203
     at row 1.35 col 20 colon-aligned     view-as combo-box
     size 31 by .88
     font 1

   ind-sit-contrat
     at row 2.35 col 20 colon-aligned     font 1
...
assign h-label = ind-sit-contrat:side-label-handle in frame f-pg-par
       h-label:row = ind-sit-contrat:row in frame f-pg-par - 0.1.

/* *************************  Create Window  ************************** */

CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Relatorio Contratos e Saldos - ESPCT001"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
DO:
   do  on error undo, return no-apply:
        ...
        IF  NUM-ENTRIES(INPUT FRAME f-pg-imp c-arquivo,".") <>  2 and
            input frame f-pg-imp rs-destino = 2
        THEN DO:
            MESSAGE "NÆo foi informada a extensÆo do arquivo destino." VIEW-AS ALERT-BOX ERROR.
            RETURN NO-APPLY.
        END.

        IF input frame f-pg-imp rs-destino = 2 and
           ENTRY(2,INPUT FRAME f-pg-imp c-arquivo,".") <> "XLS"
        THEN DO:
            ASSIGN c-arquivo:SCREEN-VALUE IN FRAME f-pg-imp = ENTRY(1,INPUT FRAME f-pg-imp c-arquivo,".") + ".xls".
            MESSAGE "A extensÆo do arquivo destino deve ser [XLS]. Portanto, foi alterada automaticamente"VIEW-AS ALERT-BOX INFORMATION.
        END.
...
ASSIGN ind-sit-contrat = 1.
assign v-cod-prog-gerado = "ESPCT001".
...
&ENDIF
  ...
  assign wh-label-imp:screen-value = "Gera‡Æo".
PAUSE 0 BEFORE-HIDE.
...
assign rs-destino:screen-value in frame f-pg-imp = "3".
    ...
    assign da-dt-ini-validade-ini:screen-value in frame f-pg-sel = string(01/01/2003).
    assign da-dt-ini-validade-fim:screen-value in frame f-pg-sel = string(12/31/9999).
    assign da-dt-ter-validade-ini:screen-value in frame f-pg-sel = string(12/31/2008).
    assign da-dt-ter-validade-fim:screen-value in frame f-pg-sel = string(12/31/9999).
...
PROCEDURE enable_UI :
   ...
   i-nr-contrato-ini i-nr-contrato-fim
   da-dt-ini-validade-ini da-dt-ini-validade-fim
   da-dt-ter-validade-ini da-dt-ter-validade-fim
   ...
   ENABLE RECT-7 rs-destino bt-arquivo c-arquivo RECT-9 rs-execucao
   WITH FRAME f-pg-imp IN WINDOW C-Win.
  ...
  return error.
   ...
   assign tt-param.usuario              = c-seg-usuario
      ...
      if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "ESPCT001" + "." + v-cod-extens-arq.

    assign tt-param.ind-sit-contrat = input frame f-pg-par ind-sit-contrat
           ...
           tt-param.i-nr-contrato-ini = input frame f-pg-sel i-nr-contrato-ini
           tt-param.i-nr-contrato-fim = input frame f-pg-sel i-nr-contrato-fim
           tt-param.da-dt-ini-validade-ini = input frame f-pg-sel da-dt-ini-validade-ini
           tt-param.da-dt-ini-validade-fim = input frame f-pg-sel da-dt-ini-validade-fim
           tt-param.da-dt-ter-validade-ini = input frame f-pg-sel da-dt-ter-validade-ini
           tt-param.da-dt-ter-validade-fim = input frame f-pg-sel da-dt-ter-validade-fim
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "esp/ESPCT001rp.p".
