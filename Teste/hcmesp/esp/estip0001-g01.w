...
*******************************************************************************/
{include/i-prgvrs.i ESTIP0001G 2.12.00.004}
...
&Scoped-Define ENABLED-OBJECTS rt-button v_cdn_empresa v_cdn_estab ~
v_num_anotip bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS v_cdn_empresa v_cdn_estab v_num_anotip 
...
DEFINE VARIABLE v_cdn_estab AS CHARACTER FORMAT "x(5)" 
...
DEFINE FRAME d-vapara
     v_cdn_empresa AT ROW 1.17 COL 24 COLON-ALIGNED WIDGET-ID 2
     v_cdn_estab AT ROW 2.17 COL 24 COLON-ALIGNED WIDGET-ID 4
     v_num_anotip AT ROW 3.17 COL 24 COLON-ALIGNED WIDGET-ID 6
...
DO:
  find es_controle no-lock 
      where es_controle.cdn_empresa = input frame {&frame-name} v_cdn_empresa and
            es_controle.cdn_estab   = input frame {&FRAME-NAME} v_cdn_estab   and
            es_controle.num_anotip  = input frame {&FRAME-NAME} v_num_anotip no-error.
  if  not avail es_controle then do:
      {utp/ut-table.i hcm es_controle 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(es_controle).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY v_cdn_empresa v_cdn_estab v_num_anotip 
      WITH FRAME d-vapara.
  ENABLE rt-button v_cdn_empresa v_cdn_estab v_num_anotip bt-ok bt-cancela 
         bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESTIP0001G" "2.12.00.004"}
