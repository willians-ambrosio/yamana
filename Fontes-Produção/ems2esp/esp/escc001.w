...
/*------------------------------------------------------------------------
  ...
  Revis∆o: 002 - Inclus∆o do estabelecimento - 18/02/08
...
DEF TEMP-TABLE  tt-movto
    ...
    FIELD  tt-fm-codigo         AS   CHAR FORMAT "X(8)"
    FIELD  tt-nr-ord-produc     LIKE req-ord-produc.nr-ord-produ    
    FIELD  tt-it-codigo         AS   CHAR FORMAT "x(16)"
...
DEFINE VARIABLE dt-ord-fim AS DATE FORMAT "99/99/9999":U INITIAL 12/31/9999 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.

DEFINE VARIABLE dt-ord-ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/1900 
     LABEL "Data Ordem" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-sel
          ...
          "Equipamento" NO-LABEL
     dt-ord-ini AT ROW 6.25 COL 11 COLON-ALIGNED
     dt-ord-fim AT ROW 6.25 COL 44 COLON-ALIGNED NO-LABEL
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "<Historico de Itens -  ESCC001.002>"
         HEIGHT             = 15.13
         WIDTH              = 72.29
         MAX-HEIGHT         = 31.29
         MAX-WIDTH          = 182.86
...
ON END-ERROR OF C-Win /* <Historico de Itens -  ESCC001.002> */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* <Historico de Itens -  ESCC001.002> */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-est-ini fi-est-fim fi-fam-ini fi-fam-fim fi-item-ini fi-item-fim 
          dt-ord-ini dt-ord-fim fl-texto 
  ...
  ENABLE IMAGE-11 IMAGE-12 IMAGE-21 IMAGE-22 IMAGE-25 IMAGE-26 IMAGE-27 
         ...
         fi-item-fim dt-ord-ini dt-ord-fim fl-texto 
      WITH FRAME f-pg-sel IN WINDOW C-Win.
...
FOR EACH ITEM NO-LOCK 
    ...
    FIND LAST ordem-compra 
         WHERE ordem-compra.dat-ordem    >= dt-ord-ini          and 
               ordem-compra.dat-ordem    <= dt-ord-fim          AND /*  
...
/*                item.fm-codigo >= fi-fam-ini                      AND */
/*                item.fm-codigo <= fi-fam-fim                     ,    */
   /* FOR each req-ord-produc no-lock
         where req-ord-produc.nr-requisicao = ordem-compra.nr-requisicao and
               req-ord-produc.sequencia     = ordem-compra.sequencia AND
               req-ord-produc.nr-ord-produ >= i-nr-ord-produ-ini and 
               req-ord-produc.nr-ord-produ <= i-nr-ord-produ-fim and  
               req-ord-produc.nr-requisicao >= fi-req-ini and 
               req-ord-produc.nr-requisicao <= fi-req-fim , */
    /*each mmv-ord-manut no-lock
         where mmv-ord-manut.nr-ord-produ = req-ord-produc.nr-ord-produ and
               mmv-ord-manut.cc-codigo   >= fi-cc-ini and 
               mmv-ord-manut.cc-codigo   <= fi-cc-fim and
               mmv-ord-manut.cod-eqpto   >= cd-equipto-ini and 
               mmv-ord-manut.cod-eqpto   <= cd-equipto-fim and
               mmv-ord-manut.ct-codigo   >= fi-ct-ini and 
               mmv-ord-manut.ct-codigo   <= fi-ct-fim , */
   ...
   ASSIGN tt-cod-estabel       = ordem-compra.cod-estabel
          ...
          tt-data-pedido       = ordem-compra.data-pedido
          tt-dt-emissao        = ?
          tt-dt-trans          = ?
          tt-dat-trans         = ?
   ...
   ASSIGN fl-texto:SCREEN-VALUE = "Pesquisando Item......... "
                                  + STRING(item.it-codigo ).


    /* dt ultima req */ 
    FIND LAST  movto-estoq NO-LOCK /* USE-INDEX data */
        WHERE  movto-estoq.it-codigo     = ITEM.it-codigo
          AND  movto-estoq.cod-estabel   = ITEM.cod-estabel
          AND (movto-estoq.esp-docto     = 28
          OR   movto-estoq.esp-docto     = 30)  NO-ERROR. 

    IF AVAIL movto-estoq THEN
       ASSIGN tt-movto.tt-dat-trans = movto-estoq.dt-trans.
    ELSE 
       ASSIGN tt-movto.tt-dat-trans = ?.
      


    /* pesquisa saldo do item */
     FOR EACH saldo-estoq
       WHERE /*saldo-estoq.cod-depos = ITEM.cod-depos AND*/
             saldo-estoq.cod-estabel = ITEM.cod-estabel 
         /*AND saldo-estoq.cod-localiz = ITEM.cod-localiz*/ 
             /* saldo-estoq.lote     = ITEM.lote AND*/
         AND saldo-estoq.it-codigo   = ITEM.it-codigo 
        /* AND saldo-estoq.cod-refer   = ITEM.cod-refer */ NO-LOCK:

        ASSIGN tt-qtidade-atu = tt-qtidade-atu + saldo-estoq.qtidade-atu.
     END.
        
    /* data do recebimento fisico */
    FIND it-doc-fisico WHERE 
         it-doc-fisico.it-codigo    = ordem-compra.it-codigo    AND
         it-doc-fisico.num-pedido   = ordem-compra.num-pedido   AND
         it-doc-fisico.numero-ordem = ordem-compra.numero-ordem NO-LOCK NO-ERROR.

    IF AVAIL it-doc-fisico  THEN DO:
    
       FIND doc-fisico OF it-doc-fisico NO-LOCK NO-ERROR.

       IF AVAIL doc-fisico THEN
          ASSIGN tt-dt-emissao = doc-fisico.dt-emissao 
                 tt-dt-trans   = doc-fisico.dt-trans.

    END.

    /* prazo de entrega */
    FIND cotacao-item NO-LOCK use-index cotacao
       where cotacao-item.numero-ordem = ordem-compra.numero-ordem
         and   cotacao-item.it-codigo    = ordem-compra.it-codigo
         and   cotacao-item.preco-unit   > 0 NO-ERROR.

    IF AVAIL cotacao-item  THEN
       ASSIGN tt-prazo-entreg = cotacao-item.prazo-entreg.
    ELSE
       ASSIGN tt-prazo-entreg = 0.
    ...
    FIND req-ord-produc 
         where req-ord-produc.nr-requisicao = ordem-compra.nr-requisicao 
           AND req-ord-produc.sequencia     = ordem-compra.sequencia NO-LOCK NO-ERROR.

    IF AVAIL req-ord-produc  THEN
       ASSIGN tt-dat-trans = req-ord-produc.dat-trans.
    */
...
/************************************************************/
    ...
    /*************** Imprime alteraá‰es **************************/
    /*************** Cabeªalho ***********************************/ 
    chWorkSheet:Range("a1"):Value = "Hist¢rico de Itens  -  "    + string(TODAY) .
                    /*Value  " + string(fi-mes-ini,"99") + " / " + string(fi-ano-ini)*/ .
    chWorkSheet:Range("a1"):Font:Size = 12.
    chWorkSheet:Range("a1"):Font:Bold = TRUE.
    chWorkSheet:Range("a3"):Value = "Fam°lia". 
    chWorkSheet:Range("b3"):Value = "Item ".
    chWorkSheet:Range("c3"):Value = "Descriá∆o ".
    chWorkSheet:Range("d3"):Value = "Pedido  ".
    chWorkSheet:Range("e3"):Value = "Pr Ult.Compra ". 
    chWorkSheet:Range("f3"):Value = "% Descto ".
    chWorkSheet:Range("g3"):Value = "IPI  ". 
    chWorkSheet:Range("h3"):Value = "ICMS ".     
    chWorkSheet:Range("i3"):Value = "ISS  ".  
    chWorkSheet:Range("j3"):Value = "Frete ".
    chWorkSheet:Range("k3"):Value = "Pr Total ".     
    chWorkSheet:Range("l3"):Value = "Dt Ordem ".  
    chWorkSheet:Range("m3"):Value = "Dt Pedido ".     
    chWorkSheet:Range("n3"):Value = "Tempo Compra".
    chWorkSheet:Range("o3"):Value = "Prazo Entrega".
    chWorkSheet:Range("p3"):Value = "Dt Nota Fiscal".
    chWorkSheet:Range("q3"):Value = "Tempo Entr/Forn".
    chWorkSheet:Range("r3"):Value = "Dt Rec. F°sico  ".
    chWorkSheet:Range("s3"):Value = "Tempo Transp  ".
    chWorkSheet:Range("t3"):Value = "Ult.Req.Estoque ".
    chWorkSheet:Range("u3"):Value = "Saldo Estoque   ".
    chWorkSheet:Range("v3"):Value = "Fornecedor   "   .
    chWorkSheet:Range("w3"):Value = "Comprador   "   .
    chWorkSheet:Range("x3"):Value = "Estabel   "   .

    chWorkSheet:Range("a3:x3"):Font:Bold = TRUE.
    ...
    ASSIGN i-linha = 5.
     ...
     FOR EACH tt-movto   :
        ...
        ASSIGN chExcelApplication:range( "A" + STRING(i-linha) ):value = tt-fm-codigo   
               chExcelApplication:Range( "a" + STRING(i-linha)):NumberFormat="0000" 
               chExcelApplication:range( "B" + STRING(i-linha) ):value = tt-it-codigo  
               chExcelApplication:Range( "b" + STRING(i-linha)):NumberFormat="00000000" 
               chExcelApplication:range( "C" + STRING(i-linha) ):value = tt-desc-item   
               chExcelApplication:range( "d" + STRING(i-linha) ):value = tt-num-pedido
               chExcelApplication:range( "e" + STRING(i-linha) ):value = tt-preco-unit
               chExcelApplication:range( "f" + STRING(i-linha) ):value = tt-perc-descto
               chExcelApplication:range( "g" + STRING(i-linha) ):value = tt-aliquota-ipi
               chExcelApplication:range( "h" + STRING(i-linha) ):value = tt-aliquota-icm              
               chExcelApplication:range( "i" + STRING(i-linha) ):value = tt-aliquota-iss                
               chExcelApplication:range( "j" + STRING(i-linha) ):value = tt-valor-frete               
               chExcelApplication:range( "k" + STRING(i-linha) ):value = tt-preco-orig                 
               chExcelApplication:range( "l" + STRING(i-linha) ):value = tt-data-emis               
               chExcelApplication:range( "m" + STRING(i-linha) ):value = tt-data-pedido        
               chExcelApplication:range( "n" + STRING(i-linha) ):value = tt-data-pedido - tt-data-emis                
               chExcelApplication:range( "o" + STRING(i-linha) ):value = tt-prazo-entreg
               chExcelApplication:range( "p" + STRING(i-linha) ):value = tt-dt-emissao
               chExcelApplication:range( "q" + STRING(i-linha) ):value = tt-dt-emissao - tt-data-pedido
               chExcelApplication:range( "r" + STRING(i-linha) ):value = tt-dt-trans
               chExcelApplication:range( "s" + STRING(i-linha) ):value = tt-dt-trans - tt-dt-emissao
               chExcelApplication:range( "t" + STRING(i-linha) ):value = tt-dat-trans /* requisicao */
               chExcelApplication:range( "u" + STRING(i-linha) ):value = tt-qtidade-atu
               chExcelApplication:range( "v" + STRING(i-linha) ):value = tt-cod-emit
               chExcelApplication:range( "w" + STRING(i-linha) ):value = tt-cod-comprado
               chExcelApplication:range( "x" + STRING(i-linha) ):value = tt-cod-estabel.
