...
FOR EACH tt-es-mov-ext-cfa BREAK 
    ...
    run pi-acompanhar in h-acomp (input "Item EXCEL => " +  tt-es-mov-ext-cfa.it-codigo).


  
    IF FIRST-OF(tt-es-mov-ext-cfa.it-codigo) THEN DO:
   
        chWorksheet:range("a" + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.ep-codigo.
/*         chWorksheet:range("b" + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.cod-estabel. */
        ...
        ELSE DO:
            c-desc-item = "".
        END.
        chWorksheet:range("d" + STRING(i-linha)):VALUE = c-desc-item.
        chWorksheet:range("e" + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.classe.
        FIND es-cfa WHERE es-cfa.classe = tt-es-mov-ext-cfa.classe NO-LOCK NO-ERROR.
   
   
        chWorksheet:range("f" + STRING(i-linha)):VALUE = IF AVAIL es-cfa THEN es-cfa.descricao  ELSE "".
   
        i-coluna = i-coluna-ini.
        i-coluna = i-coluna + 1.
        c-coluna = ENTRY(i-coluna,c-coluna1,",").
        chWorksheet:range( c-coluna + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.pergunta-literal.
        ASSIGN i-coluna = i-coluna + 1.
        c-coluna = ENTRY(i-coluna,c-coluna1,",").
        chWorksheet:range( c-coluna + STRING(i-linha)):VALUE = tt-es-mov-ext-cfa.resposta-literal.
        ASSIGN i-coluna = i-coluna + 1.
   
    END.
    ELSE DO:
/*         c-coluna = ENTRY(i-coluna,c-coluna1,","). */
        chWorksheet:cells( i-linha,i-coluna):VALUE = tt-es-mov-ext-cfa.pergunta-literal.
        ASSIGN i-coluna = i-coluna + 1.
/*         c-coluna = ENTRY(i-coluna,c-coluna1,","). */
        chWorksheet:cells(i-linha,i-coluna):VALUE = tt-es-mov-ext-cfa.resposta-literal.
        ASSIGN i-coluna = i-coluna + 1.
    END.
