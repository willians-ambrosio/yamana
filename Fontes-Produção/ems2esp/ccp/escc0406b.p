...
********************************************************************************/
{include/i-prgvrs.i ESCC0406B 2.00.00.043 } /*** 010043 ***/

&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ESCC0406b MCC}
&ENDIF

/***************************************************************************
**
**   ESCC0406B.P - relatorio de pedidos emitidos ordem de estabelecimento.
**
***************************************************************************/
{cdp/cdcfgmat.i}
...
def var c-quebra like estabelec.cod-estabel.
...
form c-lb-estab            format "x(15)" ": "
     estabelec.cod-estabel " - " estabelec.nome skip(1)
...
for each pedido-compr
    ...
    each ordem-compra
        ...
        and ((tt-param.l-receb and ordem-compra.situacao <> 4)
    ...
    for each prazo-compra
        ...
        if  ordem-compra.cod-estabel <> c-quebra
        or  line-counter > 62 then do:
            page.
            find estabelec
                 where estabelec.cod-estabel = ordem-compra.cod-estabel
                 no-lock no-error.
            if  avail estabelec then
                disp c-lb-estab
                     estabelec.cod-estabel
                     estabelec.nome with stream-io frame f-quebra.
            assign c-quebra = ordem-compra.cod-estabel.
...
/*                                          tt-param.i-moeda).                                                  */
            ...
            if  l-imp-ord then do:
                ...
                /*Unidade de Neg¢cio*/
                &IF '{&bf_mat_versao_ems}' >= '2.062' &THEN
                    ordem-compra.cod-unid-negoc at 49
                &ENDIF
                skip.
                if  tt-param.l-narrativa = yes then do:
                    /* Imprime a Narrativa */
    ...
    if  l-imprimiu then do:
        ...
        if  last-of(ordem-compra.num-pedido) then do:
            ...
            assign de-tot-liq-est = de-tot-liq-est + de-tot-liq
                   de-tot-ipi-est = de-tot-ipi-est + de-tot-ipi
                   de-tot-ped-est = de-tot-ped-est + de-tot-ped
                   de-ger-des-est = de-ger-des-est + de-tot-descto
                   l-imprimiu     = no
                   de-tot-ped     = 0
                   de-tot-descto  = 0
                   de-tot-liq     = 0
    ...
    if  last-of(ordem-compra.cod-estabel) then do:
        put c-lb-tot-liq    to 49 format "x(13)" ": " de-tot-liq-est at 52 
            c-lb-tot-ipi    to 93 format "x(12)" ": " de-tot-ipi-est at 96 skip
            c-lb-tot-descto to 49 format "x(14)" ": " de-ger-des-est at 54 
            c-lb-tot-est    to 93 format "x(21)" ": " de-tot-ped-est at 96 skip(1).
        assign de-ger-liq     = de-ger-liq + de-tot-liq-est
               de-ger-ipi     = de-ger-ipi + de-tot-ipi-est
               de-tot-ger     = de-tot-ger + de-tot-ped-est
               de-ger-descto  = de-ger-descto + de-ger-des-est
               de-tot-ped-est = 0
               de-tot-liq-est = 0
               de-ger-des-est = 0
               de-tot-ipi-est = 0.
