...
{include/i-prgvrs.i escd0401-u01 2.06.00.001}
...
def new global shared var wh-escd0401-cod-cond-pag  as widget-handle   no-undo.
...
/*        wh-escd0401-cod-cond-pag = widget-handle(entry(1, c-handle-obj)) no-error. */
/* if valid-handle(wh-escd0401-cod-cond-pag) then                                    */
...
if p-ind-event  = "BEFORE-INITIALIZE"        and
   ...
   p-wgh-object:file-name matches "*cd0401*" then do:

    assign c-handle-obj             = fc-handle-obj("cod-cond-pag", p-wgh-frame)
           wh-escd0401-cod-cond-pag = widget-handle(entry(1, c-handle-obj))
           no-error.
...
if p-ind-event  = "VALIDATE"                     and
   ...
   p-wgh-object:file-name matches "*cd0401-v04*" then do:
    ...
    if avail emitente then do:
        if string(emitente.cod-cond-pag) <> wh-escd0401-cod-cond-pag:screen-value then do:
            find es-cond-pagto no-lock
                where es-cond-pagto.cod-cond-pag = int(wh-escd0401-cod-cond-pag:screen-value) no-error.
