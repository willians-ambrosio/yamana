...
*******************************************************************************/
{include/i-prgvrs.i escd0008-G01 2.06.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod_prog_dtsul c_cod_referencia ~
d_dat_valid_ini c_cod_empresa bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod_prog_dtsul c_cod_referencia ~
d_dat_valid_ini c_cod_empresa 
...
DEFINE VARIABLE c_cod_empresa AS CHARACTER FORMAT "x(4)":U 
     LABEL "Empresa" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.

DEFINE VARIABLE c_cod_prog_dtsul AS CHARACTER FORMAT "x(50)":U 
     LABEL "Programa" 
     VIEW-AS FILL-IN 
     SIZE 47.29 BY .88 NO-UNDO.

DEFINE VARIABLE c_cod_referencia AS CHARACTER FORMAT "x(70)":U 
     LABEL "Referˆncia" 
     VIEW-AS FILL-IN 
     SIZE 47.29 BY .88 NO-UNDO.

DEFINE VARIABLE d_dat_valid_ini AS DATE FORMAT "99/99/9999":U 
...
DEFINE FRAME d-vapara
     c_cod_prog_dtsul AT ROW 1 COL 19.72 COLON-ALIGNED
     c_cod_referencia AT ROW 2 COL 19.72 COLON-ALIGNED
     d_dat_valid_ini AT ROW 3 COL 19.72 COLON-ALIGNED
     c_cod_empresa AT ROW 4.04 COL 19.72 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 5.29 COL 2.14
     bt-cancela AT ROW 5.29 COL 13
     bt-ajuda AT ROW 5.29 COL 58
     rt-button AT ROW 5.04 COL 1
...
DO:
FIND es_param_emp NO-LOCK
    WHERE es_param_emp.cod_prog_dtsul EQ INPUT FRAME {&FRAME-NAME} c_cod_prog_dtsul
      AND es_param_emp.cod_referencia EQ INPUT FRAME {&FRAME-NAME} c_cod_referencia
      AND es_param_emp.dat_valid_ini EQ INPUT FRAME {&FRAME-NAME}  d_dat_valid_ini 
      AND es_param_emp.cod_empresa   EQ INPUT FRAME {&FRAME-NAME}  c_cod_empresa NO-ERROR.

IF NOT AVAILABLE es_param_emp THEN DO:
    {utp/ut-table.i mgesp es_param_emp 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es_param_emp).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod_prog_dtsul c_cod_referencia d_dat_valid_ini c_cod_empresa 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod_prog_dtsul c_cod_referencia d_dat_valid_ini 
         c_cod_empresa bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "escd0008-G01" "2.06.00.000"}
