...
&Scoped-Define ENABLED-FIELDS es-cfa.classe 
...
&Scoped-Define DISPLAYED-FIELDS es-cfa.classe 
...
DEFINE FRAME d-vapara
     es-cfa.classe AT ROW 2 COL 22 COLON-ALIGNED WIDGET-ID 4
          LABEL "CFA"
          VIEW-AS FILL-IN 
          SIZE 8.14 BY .88
...
/* SETTINGS FOR FILL-IN es-cfa.classe IN FRAME d-vapara
   EXP-LABEL                                                            */
...
DO:
FIND es-cfa NO-LOCK
    WHERE es-cfa.classe EQ INPUT FRAME {&FRAME-NAME} es-cfa.classe NO-ERROR.

IF NOT AVAILABLE es-cfa THEN DO:
    {utp/ut-table.i mgesp es-cfa 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-cfa).
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE es-cfa THEN 
    DISPLAY es-cfa.classe 
      WITH FRAME d-vapara.
  ENABLE rt-button es-cfa.classe bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
