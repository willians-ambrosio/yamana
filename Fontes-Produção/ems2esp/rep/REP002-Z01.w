...
/****************************************************************************************** 
**         Programa: rep002-Z01.w
**            Autor: Felipe Vieira
**       Fornecedor: DKP
**       Data: 25/09/2018
** Change/Chamado: 
**    Objetivo: Cadastro de parƒmetros de bloqueio (Zoom)
**
...
******************************************************************************************/
{include/i-prgvrs.i REP002-Z01 1.00.00.000}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i REP002-Z01 REP}
&ENDIF
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-implantar bt-ajuda bt-ok ~
bt-cancela 
...
DEFINE VARIABLE h_b-table-customer-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     ...
     bt-ajuda AT ROW 16.5 COL 42.43
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Parƒmetros de bloqueio"
         HEIGHT             = 17.17
         WIDTH              = 53.14
         MAX-HEIGHT         = 17.21
         MAX-WIDTH          = 108.29
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Parƒmetros de bloqueio */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Parƒmetros de bloqueio */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Estabelecimento' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 13.67 , 51.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'rep/REP002-B01.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-table-customer-2 ).
       RUN set-position IN h_b-table-customer-2 ( 3.25 , 3.57 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 47.00 ) */

       /* Links to SmartBrowser h_b-table-customer-2. */
       RUN add-link IN adm-broker-hdl ( h_b-table-customer-2 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-table-customer-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ENABLE rt-button bt-implantar bt-ajuda bt-ok bt-cancela 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "REP002-Z01" "1.00.00.000"}
