/****************************************************************************************** 
** 	   Programa: rep005-rp.p
**   	      Autor: Vando Ribeiro
**   	 Fornecedor: Grupo DKP
**    	 Data: 29/10/2018
** Change/Chamado: 
**    Objetivo: Criar pedido de execu‡Æo para Enviar e-mail
**
...
{btb/btb912zc.i3}
{include/i_dbinst.i}
{include/i-prgvrs.i REP005-RP 0.12.00.000}

/* defini‡Æo das temp-tables para recebimento de parametros */
...
ASSIGN 
    tt-param2.destino          = 3
    tt-param2.arquivo          = "rep004-w01.lst"
...
assign
    c-programa 	= "REP005-RP"
    c-versao	= "12.01"
    c-revisao	= ".00.001"
    c-empresa 	= "YAMANA"
    c-sistema	= "MATERIAIS"
    c-titulo-relat = "RELATàRIO DE CRIA€ÇO DE PEDIDOS"
    /*
...
ASSIGN
    dt-ini = TODAY /*tt-param.dt-exec-ped-i*/
    /*dt-fim = tt-param.dt-exec-ped-f
    hr-exec = SUBSTRING(TODAY,1,2) + SUBSTRING(TODAY,3,2) + "00"
    */
    hr-exec = SUBSTRING(STRING(TIME, "HH:MM:SS"),1,2) + SUBSTRING(STRING(TIME, "HH:MM:SS"),4,2) + SUBSTRING(STRING(TIME, "HH:MM:SS"),7,2)
    hr-cria = hr-exec.

FOR EACH  esp_param_bloq NO-LOCK
    WHERE esp_param_bloq.env-relat:

    CASE esp_param_bloq.freq-envio:
        WHEN 1 THEN DO: /*frequencia diaria*/

            FIND FIRST ped_exec NO-LOCK USE-INDEX pedexec_01
                 WHERE ped_exec.dat_exec_ped_exec      = dt-ini
                   AND ped_exec.cod_prog_dtsul         = "rep004-w01"
                   AND ped_exec.nom_prog_ext           = "rep\rep004-rp.r"
                   AND ped_exec.cod_release_prog_dtsul = "12.00.01" NO-ERROR.

            IF NOT AVAIL ped_exec THEN
            DO:
                RUN pi-ped-exec (INPUT "D").
            END.
        END.
        WHEN 2 THEN DO: /*frequencia semanal*/

            RUN pi-semana-ini (OUTPUT dt-ini-semana).

            FIND FIRST ped_exec NO-LOCK USE-INDEX pedexec_06
                 WHERE ped_exec.cod_prog_dtsul         = "rep004-w01"
                   AND ped_exec.nom_prog_ext           = "rep\rep004-rp.r"
                   AND ped_exec.cod_release_prog_dtsul = "12.00.01"
                   AND ped_exec.dat_livre_2            = dt-ini-semana NO-ERROR.
            IF NOT AVAIL ped_exec THEN
            DO:
                RUN pi-ped-exec (INPUT "S").
            END.
        END.
            ...
            DO:
                FIND FIRST ped_exec NO-LOCK USE-INDEX pedexec_06
                     WHERE ped_exec.cod_prog_dtsul         = "rep004-w01"
                       AND ped_exec.nom_prog_ext           = "rep\rep004-rp.r"
                       AND ped_exec.cod_release_prog_dtsul = "12.00.01"
                       AND ped_exec.dat_livre_2            =  DATE("01/" + SUBSTRING(STRING(TODAY, "99/99/9999"),4,7)) NO-ERROR.
                IF NOT AVAIL ped_exec THEN
                DO:
                    RUN pi-ped-exec (INPUT "M").
                END.
            END.
...
FOR EACH tt-log:
   PUT STREAM str-rp
       tt-log.num_ped_exec AT 01 FORMAT ">>>>>>9"
       tt-log.dt-exec-ped  AT 10 FORMAT "99/99/9999"
       tt-log.hr-exec      AT 22 FORMAT "99:99:99" 
       tt-log.nom_prog_ext AT 32 FORMAT "X(20)" SKIP.
END.
RETURN.


PROCEDURE pi-ped-exec:
    DEFINE INPUT PARAMETER p-frequencia AS CHAR NO-UNDO.
    ...
    CASE p-frequencia:
        WHEN "D" THEN
            ASSIGN tt-ped_exec.des_inf_aplic = "Di rio".
        WHEN "S" THEN
            ASSIGN tt-ped_exec.dat_livre_2   = dt-ini-semana
                   tt-ped_exec.des_inf_aplic = "Semanal".
        WHEN "M" THEN
            ASSIGN tt-ped_exec.dat_livre_2   = DATE("01/" + SUBSTRING(STRING(TODAY, "99/99/9999"),4,7))
                   tt-ped_exec.des_inf_aplic = "Mensal".
    END CASE.
    ...
    CREATE tt-log.
    ASSIGN
        tt-log.num_ped_exec = i-num_ped_exec
        tt-log.dt-exec-ped  = dt-ini
        tt-log.hr-exec      = hr-exec
        tt-log.nom_prog_ext = tt-ped_exec.nom_prog_ext.

    EMPTY TEMP-TABLE tt-ped_exec.
    EMPTY TEMP-TABLE tt-ped_exec_param.
    EMPTY TEMP-TABLE tt-ped_exec_param_aux.
    EMPTY TEMP-TABLE tt-ped_exec_sel.

END PROCEDURE.


PROCEDURE pi-semana-ini:
