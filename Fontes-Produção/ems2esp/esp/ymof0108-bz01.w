...
DEFINE VARIABLE c-final AS CHARACTER FORMAT "X(4)" INITIAL "ZZZZ" 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-table B-table-Win _STRUCTURED
  QUERY br-table NO-LOCK DISPLAY
      es-cfa.classe COLUMN-LABEL "CFA" FORMAT "x(4)":U WIDTH 7
...
DEFINE FRAME F-Main
     ...
     c-final AT ROW 1 COL 21 COLON-ALIGNED NO-LABEL
     bt-confirma AT ROW 1 COL 46
...
/* Query rebuild information for BROWSE br-table
     _TblList          = "mgesp.es-cfa"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "mgesp.es-cfa.classe >= c-inicial
 AND mgesp.es-cfa.classe <= c-final"
     _FldNameList[1]   > mgesp.es-cfa.classe
"classe" "CFA" ? "character" ? ? ? ? ? ? no ? no no "7" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > mgesp.es-cfa.descricao
"descricao" "Descricao" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
