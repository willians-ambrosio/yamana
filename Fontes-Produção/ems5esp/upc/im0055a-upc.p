...
IF p-ind-event = "AFTER-INITIALIZE" THEN DO:
    ...
    IF VALID-HANDLE(wgh-embarque-simples-IM0055A) THEN DO:
        ...
        FIND FIRST ext-historico-embarque
             WHERE ext-historico-embarque.cod-estabel    = wgh-cod-estabel-IM0055A:screen-value
               and ext-historico-embarque.cod-itiner     = int(wgh-cod-itiner-IM0055A:screen-value)
               and ext-historico-embarque.cod-pto-contr  = int(wgh-cod-pto-contr-IM0055A:screen-value)
               and ext-historico-embarque.embarque       = wgh-embarque-IM0055A:screen-value
               and ext-historico-embarque.sequencia      = int(wgh-seq-IM0055A:screen-value) NO-ERROR.
...
if p-ind-event  = "BEFORE-ENABLE" THEN DO:
    ...
    FIND FIRST itinerario 
         WHERE itinerario.cod-itiner      = int(wgh-cod-itiner-IM0055A:SCREEN-VALUE)
           AND itinerario.pto-desembarque = int(wgh-cod-pto-contr-IM0055A:SCREEN-VALUE) NO-LOCK NO-ERROR.
...
if p-ind-event  = "AFTER-SAVE-FIELDS" THEN DO:
    ...
    IF VALID-HANDLE(wgh-embarque-simples-IM0055A) THEN DO:
        ...
        IF AVAIL ext-historico-embarque THEN 
            ASSIGN ext-historico-embarque.log-simples-remessa = LOGICAL(wgh-embarque-simples-IM0055A:SCREEN-VALUE).

        ELSE DO:
            CREATE ext-historico-embarque.
            ASSIGN ext-historico-embarque.cod-estabel         = wgh-cod-estabel-IM0055A:screen-value       
                   ext-historico-embarque.cod-itiner          = int(wgh-cod-itiner-IM0055A:screen-value)   
                   ext-historico-embarque.cod-pto-contr       = int(wgh-cod-pto-contr-IM0055A:screen-value)
                   ext-historico-embarque.embarque            = wgh-embarque-IM0055A:screen-value          
                   ext-historico-embarque.sequencia           = int(wgh-seq-IM0055A:screen-value)
                   ext-historico-embarque.log-simples-remessa = LOGICAL(wgh-embarque-simples-IM0055A:SCREEN-VALUE).
        END.
    END.
END.
