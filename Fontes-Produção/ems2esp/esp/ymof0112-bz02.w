...
&Scoped-define FIELDS-IN-QUERY-br-table ccusto.cod_empresa ~
ccusto.cod_ccusto ccusto.des_tit_ctbl 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table 
&Scoped-define QUERY-STRING-br-table FOR EACH ccusto WHERE ~{&KEY-PHRASE} ~
      AND ccusto.cod_ccusto >= c-inicial ~
 AND ccusto.cod_ccusto <= c-final ~
 AND ccusto.cod_empresa = v_cod_empres_usuar NO-LOCK ~
    BY ccusto.cod_ccusto INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-table OPEN QUERY br-table FOR EACH ccusto WHERE ~{&KEY-PHRASE} ~
      AND ccusto.cod_ccusto >= c-inicial ~
 AND ccusto.cod_ccusto <= c-final ~
 AND ccusto.cod_empresa = v_cod_empres_usuar NO-LOCK ~
    BY ccusto.cod_ccusto INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-table ccusto
&Scoped-define FIRST-TABLE-IN-QUERY-br-table ccusto
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-table B-table-Win _STRUCTURED
  QUERY br-table NO-LOCK DISPLAY
      ccusto.cod_empresa FORMAT "x(3)":U
      ccusto.cod_ccusto FORMAT "x(11)":U
      ccusto.des_tit_ctbl COLUMN-LABEL "T¡tulo" FORMAT "x(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE br-table
     _TblList          = "ems5.ccusto"
     _Options          = "NO-LOCK INDEXED-REPOSITION KEY-PHRASE"
     _OrdList          = "ems5.ccusto.cod_ccusto|yes"
     _Where[1]         = "ems5.ccusto.cod_ccusto >= c-inicial
 AND ems5.ccusto.cod_ccusto <= c-final
 AND ems5.ccusto.cod_empresa = v_cod_empres_usuar"
     _FldNameList[1]   = ems5.ccusto.cod_empresa
     _FldNameList[2]   = ems5.ccusto.cod_ccusto
     _FldNameList[3]   > ems5.ccusto.des_tit_ctbl
"ccusto.des_tit_ctbl" "T¡tulo" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    if  avail ems5.ccusto then do:
        case p-campo:
            when "cod_ccusto" then
                assign p-valor = string(ccusto.cod_ccusto).
            when "descricao" then
                assign p-valor = string(ccusto.des_tit_ctbl).
        end.
    end.
    return p-valor.
END PROCEDURE.
