...
ASSIGN c_arquivo_destino = SESSION:TEMP-DIRECTORY + "esbs001.xls".
...
PROCEDURE pi_valida:
    FOR EACH rh_estab
        ...
        FIND FIRST estab_benef_prev_brad
            ...
            AND   estab_benef_prev_brad.cdn_estab     = rh_estab.cdn_estab
            AND   estab_benef_prev_brad.cdn_beneficio = 1 NO-LOCK NO-ERROR.
        IF NOT AVAIL estab_benef_prev_brad THEN DO:
            MESSAGE "A Empresa " + STRING(rh_estab.cdn_empresa) + " Estabelecimento " + STRING(rh_estab.cdn_estab) + " nÆo possui Parƒmetro de PGBL Empresa Cadastrado! Favor cadastrar atrav‚s do Programa [ESBS002]."
        ...
        FIND FIRST estab_benef_prev_brad
            WHERE estab_benef_prev_brad.cdn_empresa   = rh_estab.cdn_empresa
            AND   estab_benef_prev_brad.cdn_estab     = rh_estab.cdn_estab
            AND   estab_benef_prev_brad.cdn_beneficio = 2 NO-LOCK NO-ERROR.
        IF NOT AVAIL estab_benef_prev_brad THEN DO:
            MESSAGE "A Empresa " + STRING(rh_estab.cdn_empresa) + " Estabelecimento " + STRING(rh_estab.cdn_estab) + " nÆo possui Parƒmetro de PGBL Funcionario Cadastrado! Favor cadastrar atrav‚s do Programa [ESBS002]."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
            ASSIGN i-liberado = NO.
        END.
        FIND FIRST estab_benef_prev_brad
            WHERE estab_benef_prev_brad.cdn_empresa   = rh_estab.cdn_empresa
            AND   estab_benef_prev_brad.cdn_estab     = rh_estab.cdn_estab
            AND   estab_benef_prev_brad.cdn_beneficio = 3 NO-LOCK NO-ERROR.
        IF NOT AVAIL estab_benef_prev_brad THEN DO:
            MESSAGE "A Empresa " + STRING(rh_estab.cdn_empresa) + " Estabelecimento " + STRING(rh_estab.cdn_estab) + " nÆo possui Parƒmetro de VGBL Funcionario Cadastrado! Favor cadastrar atrav‚s do Programa [ESBS002]."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
            ASSIGN i-liberado = NO.
        END.
    END.
...
PROCEDURE pi_abre_planilha.
    ...
    ASSIGN ch_excel_book  = ch_excel_application:WORKBOOKS:ADD()
           ...
           ch_excel_sheet:name = "PGBL Empresa".
...
PROCEDURE pi_escreve_planilha:
    ...
    FOR EACH funcionario NO-LOCK
        ...
        FIND FIRST estab_benef_prev_brad
            ...
            AND   estab_benef_prev_brad.cdn_estab     = funcionario.cdn_estab
            AND   estab_benef_prev_brad.cdn_beneficio = 1 NO-LOCK NO-ERROR.
        ...
        ASSIGN ch_excel_sheet:RANGE("O" + STRING(numero-linha)):VALUE = (IF STRING(INTEGER(MONTH(rh_pessoa_fisic.dat_nascimento)) + 1) = '13' THEN '01' ELSE STRING(INTEGER(MONTH(rh_pessoa_fisic.dat_nascimento)) + 1)) + '/01/' + STRING(INTEGER(YEAR(rh_pessoa_fisic.dat_nascimento)) + 60).
        ASSIGN ch_excel_sheet:RANGE("P" + STRING(numero-linha)):VALUE = STRING(MONTH(TODAY)) + '/01/' + STRING(YEAR(TODAY)) /*Verificar*/.
    ...
    ch_excel_sheet:name = "PGBL Funcionario".
    ...
    ch_excel_sheet:name = "VGBL Funcionario".
