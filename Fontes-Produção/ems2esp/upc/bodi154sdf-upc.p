...
/************************************************************************
**
{include/i-prgvrs.i BODI154SDF-UPC-SUG-NATUR 2.00.00.004 } /*** 010004 ***/
...
assign c-prg-vrs = "2.00.00.001"
       c-prg-obj = "BODI154SDF-UPC-SUG-NATUR".
...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "BODI154SDF-UPC-SUG-NATUR"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    /*Altera‡Æo - 08/09/2006 - tech1007 - Altera‡Æo para exibir o nome do programa que executou o programa que ser  exibido no extrato de versÆo
        ...
        PUT "BODI154SDF-UPC-SUG-NATUR" AT 1 "2.00.00.001" AT 39 c-prg-obj-pai AT 54 STRING(TODAY,'99/99/99') AT 84 STRING(TIME,'HH:MM:SS':U) AT 94 SKIP.
...
**  BODI154.i
**
**  ped-item - Arquivo de Itens do Pedido de Venda
**
...
IF  p-ind-event = "AfterSetDefaultItem" THEN DO:
    ...
    FOR FIRST tt-epc
        ...
        RUN outputTable IN h-bo (OUTPUT TABLE tt-ped-item).

        FOR FIRST tt-ped-item:

            RUN esbo/boes001.p PERSISTENT SET h-boesp.

            RUN getNaturezaItem IN h-boesp (INPUT  tt-ped-item.nome-abrev,
            ...
            IF c-natur-oper-pedido <> "" THEN
                ASSIGN tt-ped-item.nat-operacao = c-natur-oper-pedido.

            RUN inputTable IN h-bo (INPUT TABLE tt-ped-item).
