...
&Scoped-define FIELDS-IN-QUERY-BROWSE-3 tt-dados.cod_estab tt-dados.nom_fav tt-dados.cod_func tt-dados.vlr_bco tt-dados.vlr_rh tt-dados.cod_ocorr   
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 86 BY 9.29 ROW-HEIGHT-CHARS .67 FIT-LAST-COLUMN.
...
/*                                                                                                                                                   */
   ...
   /* Gera a baixa do t°tulo no Contas a Pagar */
    ...
    RUN pi-consomeWs IN h-van006 (INPUT "",
                                  INPUT 2 /* Retorno */).
...
FOR EACH es_param_dir_van NO-LOCK WHERE
            es_param_dir_van.cdn_empresa = v_cod_empres_usuar AND 
            es_param_dir_van.tipo        = NO /* Recebe */  AND 
            es_param_dir_van.log_ativo   = YES:
 
    RUN van\van001a.p (INPUT es_param_dir_van.dir_retorno,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-decript w-livre 
PROCEDURE pi-decript :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
