...
*******************************************************************************/
{include/i-prgvrs.i ESCN0206 12.1.13.001 } /*** 010004 ***/

&IF "{&EMSFND_VERSION}" >= "1.00"
&THEN
{include/i-license-manager.i ESCN0206 MCN}
&ENDIF
...
DEFINE VARIABLE c-arquivo-entrada AS CHARACTER FORMAT "x(255)"
     VIEW-AS FILL-IN
     SIZE 40 BY .88
     BGCOLOR 15  NO-UNDO.
...
DEFINE FRAME f-pg-par
     da-transacao AT ROW 3 COL 25 COLON-ALIGNED
     c-arquivo-entrada AT ROW 5.92 COL 6 HELP
          "Nome do arquivo de destino do relat¢rio" NO-LABEL
     bt-arquivo-entrada AT ROW 5.92 COL 45.86 HELP
          "Escolha do nome do arquivo"
     text-entrada AT ROW 4.92 COL 8.86 NO-LABEL
     RECT-12 AT ROW 2.88 COL 4.43
     RECT-8 AT ROW 5.21 COL 4.72
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 75 BY 10.38.
...
ON END-ERROR OF C-Win /* Importaá∆o de Contrato */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Importaá∆o de Contrato */
DO:
...
DO:
    ...
    assign c-arq-conv = replace(input frame f-pg-par c-arquivo-entrada, "/", "~\").
    ...
    if  l-ok = yes then do:
        ...
        display c-arquivo-entrada with frame f-pg-par.
    end.
END.
...
{utp/ut9000.i "ESCN0206" "12.1.13.001"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   /*{include/i-imvrf.i &programa=escn0206 &versao-layout=001}*/
    def new global shared var v_cod_dialet_corren as character no-undo.
    &global-define DialLayout Yes
    
    assign c-arq-layout = "cnp~/esp~/layout~/loescn0206.001":U + "pt":U.
   ...
   assign da-transacao:screen-value in frame f-pg-par = string(today).
...
------------------------------------------------------------------------------*/
  ...
  ENABLE /*RECT-11 rs-todos*/ RECT-7 rs-destino bt-arquivo-destino 
         bt-config-impr-destino c-arquivo-destino RECT-9 rs-execucao 
      WITH FRAME f-pg-log IN WINDOW C-Win.
  {&OPEN-BROWSERS-IN-QUERY-f-pg-log}
  DISPLAY da-transacao c-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
  ENABLE RECT-12 da-transacao RECT-8 c-arquivo-entrada bt-arquivo-entrada 
      WITH FRAME f-pg-par IN WINDOW C-Win.
...
do  on error undo, return error
    ...
    assign c-arquivo-entrada   = input frame f-pg-par c-arquivo-entrada.
           file-info:file-name   = c-arquivo-entrada.
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.todos           = 1 /*input frame f-pg-log rs-todos*/
           tt-param.arq-entrada     = input frame f-pg-par c-arquivo-entrada
           tt-param.data-exec       = today
           tt-param.hora-exec       = time
           tt-param.da-transacao    = input frame f-pg-par da-transacao
           tt-param.c-todos         = "Todos"/*entry((tt-param.todos - 1) * 2 + 1,
                                        rs-todos:radio-buttons in frame f-pg-log)*/
           tt-param.c-destino       = entry((tt-param.destino - 1) * 2 + 1,
                                        rs-destino:radio-buttons in frame f-pg-log).
    ...
    {include/i-imrun.i cnp/esp/escn0206rp.p}
