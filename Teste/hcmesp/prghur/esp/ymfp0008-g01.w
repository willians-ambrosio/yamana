...
&Scoped-Define ENABLED-OBJECTS rt-button i_cdn_gestor bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cdn_gestor 
...
DEFINE VARIABLE i_cdn_gestor AS INTEGER FORMAT "99999999":U INITIAL 0 
     LABEL "Cod.Gestor" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cdn_gestor AT ROW 1.96 COL 30 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Gestor */
DO:
FIND es_Gestor NO-LOCK
    WHERE es_Gestor.cdn_gestor EQ INPUT FRAME {&FRAME-NAME} i_cdn_gestor NO-ERROR.

IF NOT AVAILABLE es_Gestor THEN DO:
    {utp/ut-table.i hresp es_Gestor 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es_Gestor).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Gestor */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_cdn_gestor 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cdn_gestor bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
