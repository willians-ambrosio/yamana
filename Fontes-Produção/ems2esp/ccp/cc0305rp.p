...
{include/i-prgvrs.i CC0305RP 2.00.00.072}  /*** 010072 ***/
...
run utp/ut-trfrrp.p (input frame f-pedido-compr-head:handle).
run utp/ut-trfrrp.p (input frame f-assinatura:handle).
run utp/ut-trfrrp.p (input frame f-b2b:handle).
run utp/ut-trfrrp.p (input frame f-narrativa:handle).
run utp/ut-trfrrp.p (input frame f-descricao:handle).
run utp/ut-trfrrp.p (input frame f-labels:handle).
run utp/ut-trfrrp.p (input frame f-contrato:handle).
run utp/ut-trfrrp.p (input frame f-pedido:handle).
run utp/ut-trfrrp.p (input frame f-total:handle).

{include~/i-rpcab.i}
{include~/i-rpout.i}

if l-eprocurement then do:
    {utp/ut-liter.i "Pedidos_de_Compra_Enviados_Via_Portal-Datasul" *}
    assign c-titulo-relat = return-value.
    {utp/ut-liter.i "COMPRAS" *}
    assign c-sistema      = return-value.
    view frame f-cabec.
    view frame f-rodape.
    /*view frame f-pedido-compr-head.*/
...
run pi-inicializar in h-acomp (input return-value).
    ...
    &IF defined(bf_mat_bloqueio_fornec) &THEN
        ...
        /*--- N∆o imprime pedido quando a api retornar "NOK" ---*/
    ...
    then next.
        ...
        **/
            ...
            for each  b-ord
                ...
                if  l-pendente then
                    ...
                    or  tt-param.l-eprocurement = yes then do:
                           {utp/ut-liter.i Pedido_n∆o_enviado_por_estar_pendente_de_Aprovaá∆o_Eletrìnica! *}
                           put unformatted pedido-compr.num-pedido   at 10
                                           "|"                       at 27
                                           pedido-compr.cod-emitente at 29
                                           "|"                       at 38
                                           return-value format "X(65)"      at 40 skip.
                        leave.
                    end.
                    else
                        leave.
        ...
        /* Vari†veis alimentadas para serem utilizadas nas integraá‰es B2B */
...
*/
            ...
            for each  prazo-compra
                ...
                disp prazo-compra.parcela       at 2 space(0)
                ...
                run utp/ut-trfrrp.p (input frame f-parcela:handle). 
                assign l-ordem = yes.
        ...
        if  l-imprimiu  then do:
            if  pedido-compr.cod-cond-pag = 0 then do:
                find first cond-especif where pedido-compr.num-pedido = cond-especif.num-pedido no-lock no-error.
                if  avail cond-especif THEN DO:
                    disp skip(1)
                         "----------------------------- "
                       + c-lb-cond-pag
                       + " -------------------------------" skip
                         cond-especif.data-pagto[1] at 6 "-"
                         cond-especif.perc-pagto[1] no-label "%" at 41
                         cond-especif.data-pagto[2] "-"
                         cond-especif.perc-pagto[2] no-label "%" at 78 skip
                         cond-especif.data-pagto[3] at 6 "-"
                         cond-especif.perc-pagto[3] no-label "%" at 41
                         cond-especif.data-pagto[4] "-"
                         cond-especif.perc-pagto[4] no-label "%" at 78 skip
                         cond-especif.data-pagto[5] at 6 "-"
                         cond-especif.perc-pagto[5] no-label "%" at 41
                         cond-especif.data-pagto[6] "-"
                         cond-especif.perc-pagto[6] no-label "%" at 78 skip
                         cond-especif.data-pagto[7] at 6 "-"
                         cond-especif.perc-pagto[7] no-label "%" at 41
                         cond-especif.data-pagto[8] "-"
                         cond-especif.perc-pagto[8] no-label "%" at 78 skip
                         cond-especif.data-pagto[9] at 6 "-"
                         cond-especif.perc-pagto[9]  no-label "%" at 41
                         cond-especif.data-pagto[10] "-"
                         cond-especif.perc-pagto[10] no-label "%" at 78 skip
                         cond-especif.data-pagto[11] at 6 "-"
                         cond-especif.perc-pagto[11] no-label "%" at 41
                         cond-especif.data-pagto[12] "-"
                         cond-especif.perc-pagto[12] no-label "%" at 78 skip (1)
                    ...
                    run utp/ut-trfrrp.p (input frame f-cond-pagto-esp:handle).
                END.
            end.
            find mensagem where pedido-compr.cod-mensagem = mensagem.cod-mensagem no-lock no-error.
            assign l-com1 = no.
...
if tt-param.l-impressao then do:
    ...
    If avail tt-b2b THEN do:
      assign l-param = tt-param.l-bus-to-bus.
          put c-lb-b2b at 5 ": " l-param format "Sim/N∆o" skip(1).
    END.  
    ...
    and tt-param.l-eprocurement then do:  /* parceiro de neg¢cios no Portal */
        ...
        if  i-status <> 2 then do:
            run ccp/cc0305d.p (input rowid(pedido-compr),
                               input c-changecode,
                               input-output table tt-b2b).
            for each tt-b2b: /* N∆o imprimir nada que retornou do cc0305d */
                delete tt-b2b.
            end.
        ...
        put pedido-compr.num-pedido   at 10.
        put "|"                       at 27.
        put pedido-compr.cod-emitente at 29.
        put "|"                       at 38.
        put c-status format "X(40)"   at 40 skip.
