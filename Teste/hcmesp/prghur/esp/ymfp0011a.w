...
*******************************************************************************/
{include/i-prgvrs.i ymfp0011A 1.02.00.003 } /*** 010003 ***/
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i ymfp0011a mfp}
&ENDIF
...
DEFINE VARIABLE h_ymqry003 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_ymvwr003a AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 6.5 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 6.5 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 6.5 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 6.5 COL 60.43
     rt-button AT ROW 6.33 COL 1.57
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 71.72 BY 9.96 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Individual Factor"
         HEIGHT             = 6.88
         WIDTH              = 71.72
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Individual Factor */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Individual Factor */
DO:
...
DO:
{include/salvafil.i h_ymvwr003a}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'object/sopesp/vwr/ymvwr003a.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_ymvwr003a ).
       RUN set-position IN h_ymvwr003a ( 1.21 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 71.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'object/sopesp/qry/ymqry003.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_ymqry003 ).
       RUN set-position IN h_ymqry003 ( 1.75 , 57.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_ymvwr003a. */
       RUN add-link IN adm-broker-hdl ( h_ymqry003 , 'Record':U , h_ymvwr003a ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_ymvwr003a ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_ymvwr003a ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ymfp0011A" "1.02.00.002"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_ymvwr003a (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_ymqry003 (input v-row-table).
