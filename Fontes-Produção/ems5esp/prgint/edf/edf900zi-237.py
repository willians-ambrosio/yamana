...
def var c-versao-prg as char initial " 12.01.19.001":U no-undo.
def var c-versao-rcode as char initial "[[[12.01.19.001[[[":U no-undo. /* Controle de Versao R-CODE - Nao retirar do Fonte */
...
PROCEDURE pi_extracao_banco_brasil_detalhe:
    ...
    /*
    output to c:\temp\dados_bradesco.txt append.
    ...
    */
    ...
    then do: /* --- DETALHE ---*/
        ...
        assign v_val_titulo = dec(substring(tt_param_program_formul.ttv_des_contdo, 1,17)).   /* int para dec */
        ...
        if avail item_bord_ap then do:
            find portador no-lock where 
                 portador.cod_portador = item_bord_ap.cod_portador no-error.

            assign v_cdn_forma_pagto = int(item_bord_ap.cod_forma_pagto) when avail item_bord_ap
                   v_cod_banco       = portador.cod_banco when avail portador.
        ...
        if v_cod_segto = '' then do:
            ...
            /* --- 2- DOC / 1-CRDITO CONTA CORRENTE / 7-CHEQUE ADMINISTRATIVO / 8-ORDEM DE PAGAMENTO ---*/
            ...
            then do:
                ...
                if v_cdn_forma_pagto = 2 or v_cdn_forma_pagto = 3 then /* DOC */ 
                    assign v_compl_tp_servico    = '01' /* Cr‚dito em conta */
                           v_cod_finalid_ted     = ''
                           v_compl_finalid_pagto = 'CC'.


                if v_cdn_forma_pagto = 1 then /* Cr‚dito em conta*/ 
                    assign v_compl_tp_servico    = ' ' /* Cr‚dito em conta */
                           v_cod_finalid_ted     = ''
                           v_compl_finalid_pagto = ' '.

                if v_cdn_forma_pagto = 4 then /* Cheque Administrativo  */
                      assign v_compl_tp_servico    = ' ' /* Cr‚dito em conta */  
                             v_cod_finalid_ted     = ''                         
                             v_compl_finalid_pagto = 'CC'.   

                if v_cdn_forma_pagto = 6 then /* ORDEM DE PAGAMENTO  */
                       assign v_compl_tp_servico    = ' ' /* Cr‚dito em conta */ 
                              v_cod_finalid_ted     = ''                         
                              v_compl_finalid_pagto = ''.                      
        
                if v_cdn_forma_pagto = 7 then /* TED CIP - MESMA TITULARIDADE */
                     assign v_cod_finalid_ted     = '00010'  
                            v_compl_finalid_pagto = 'CC'
                            v_compl_tp_servico    = ''.
        
                if v_cdn_forma_pagto = 8 then /* TED STR - OUTRA TITULARIDADE */
                    assign v_cod_finalid_ted     = '00010'  
                           v_compl_finalid_pagto = 'CC'
                           v_compl_tp_servico    = ''.
                ...
                assign v_cod_det = caps('A' 
                                        ...
                                        + string(substring(v_cod_digito_agenc_bcia_fav,1,1),'x')   /* D¡gito Verificador da Agˆncia 29 29 1 */
                                        + string(dec(v_cod_cta_corren_fav),'999999999999')         /* N£mero da Conta Corrente 30 41 12 */
                                        + string(substring(v_cod_digito_cta_corren_fav,1,1),'x')   /* D¡gito Verificador da Conta 42 42 1 */
                                        + fill(' ',1)                                              /* D¡gito Verificador da AG/Conta 43 43 1 */
                                        + string(v_nom_favorec,'x(30)')                            /* Nome do Favorecido 44 73 30 */
                                        + string(v_cod_tit_ap,'x(20)')                             /* Nr Docto 20 */
                                        ...
                                        + string(v_compl_tp_servico,'x(02)')                       /* Compl tipo servi‡o - 218 a 219 - Apenas para DOC*/
                                        + string(v_cod_finalid_ted,'x(05)')                        /* Cod. Finalidade TED -5 - 220 a 224*/
                                        + string(v_compl_finalid_pagto,'x(02)')                    /* Compl Finalid Pagto - 2 - 225 a 226 */
                                        + fill(' ', 3)                                             /* Uso Febraban - 3 */  
            ...
            if (v_cdn_tip_forma_pagto = 1 or v_cdn_tip_forma_pagto = 5 or
                v_cdn_forma_pagto = 6 or v_cdn_forma_pagto = 5 or v_cdn_forma_pagto = 13)
            then do:
                ...
                IF int(v_val_pagto) > 250000000 THEN DO:
                   ...
                   find first tt_segment_tot
                    ...
                    assign v_cod_det = v_cod_det + chr(10)  /* Soma a linha do J + o J 52 */ 
                                       ...
                                       + ' '                                      /* Tipo de inscri‡Æo sacador - 132 a 132               */
                        ...
                        then assign  tt_segment_tot.ttv_qtd_proces_edi  = tt_segment_tot.ttv_qtd_proces_edi  + 1
                            ...
                            assign  v_cod_det = v_cod_det 
                                                ...
                                                + caps(string(v_cod_banco,'x(03)')              /* Cod Banco        1 a 3 - v_cod_banco                */       
                                                        ...
                                                        + "1"                                      /* Identifica uso info complementar */ 
                                                        + fill(' ',80)                  
                                                        + fill(' ',80)
...
PROCEDURE pi_processa_padrao:
   ...
   if avail ext_item_lote_impl_ap then
       ASSIGN v_cod_receita      = STRING(ext_item_lote_impl_ap.cod_receita,"999999")
              ...
              v_per_apuracao     = replace(replace(string(ext_item_lote_impl_ap.perc_impto),".",""),",","")              
              v_dt_periodo       = replace(string(ext_item_lote_impl_ap.competencia,"99/99/9999"),"/","")
   ...
   if avail ext_item_bord_ap then 
       ASSIGN v_cod_receita      = STRING(ext_item_bord_ap.cod_receita,"999999")                                                            
              ...
              v_per_apuracao     = replace(replace(string(ext_item_bord_ap.per_apur),".",""),",","")                                                                     
              v_dt_periodo       = replace(string(ext_item_bord_ap.competencia,"99/99/9999"),"/","")
    ...
    find first tt_param_program_formul where
         tt_param_program_formul.tta_cdn_segment_edi = 289  and
         tt_param_program_formul.tta_cdn_element_edi = 3709 no-lock no-error.
    if avail tt_param_program_formul and v_dt_periodo = "" then 
    ...
    if avail ext_item_lote_impl_ap then
       assign c-aux = replace(string(ext_item_lote_impl_ap.competencia,"99/99/9999"),"/","")
              c-aux =  substring(c-aux ,3,6).

    if avail ext_item_bord_ap then
       assign c-aux = replace(string(ext_item_bord_ap.competencia,"99/99/9999"),"/","")
              c-aux =  substring(c-aux ,3,6). 
