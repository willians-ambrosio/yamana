...
{include/i-prgvrs.i YMFP0002C 1.00.00.000}
...
DEFINE TEMP-TABLE tt-origem NO-UNDO
       ...
       FIELD cdn_ocor_mpe  LIKE ocor_nao_autoriz.cdn_ocor_mpe
       FIELD des_ocor_mpe  LIKE ocor_nao_autoriz.des_ocor_mpe
       INDEX id_tt_origem  IS UNIQUE PRIMARY cod_grp_usuar cdn_ocor_mpe ASCENDING.
...
DEFINE TEMP-TABLE tt-destino NO-UNDO
       ...
       FIELD cdn_ocor_mpe  LIKE ocor_nao_autoriz.cdn_ocor_mpe
       FIELD des_ocor_mpe  LIKE ocor_nao_autoriz.des_ocor_mpe
       INDEX id_tt_destino IS UNIQUE PRIMARY cod_grp_usuar cdn_ocor_mpe ASCENDING.

DEFINE TEMP-TABLE tt_ocor_orig NO-UNDO
       FIELD cdn_ocor_mpe LIKE ocor_nao_autoriz.cdn_ocor_mpe
       FIELD des_ocor_mpe LIKE ocor_nao_autoriz.des_ocor_mpe.
...
&Scoped-define FIELDS-IN-QUERY-br-source-browse tt-origem.cdn_ocor_mpe tt-origem.des_ocor_mpe   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-source-browse   
&Scoped-define SELF-NAME br-source-browse
&Scoped-define QUERY-STRING-br-source-browse FOR EACH tt-origem                               BY tt-origem.cdn_ocor_mpe
&Scoped-define OPEN-QUERY-br-source-browse OPEN QUERY {&SELF-NAME} FOR EACH tt-origem                               BY tt-origem.cdn_ocor_mpe.
...
&Scoped-define FIELDS-IN-QUERY-br-target-browse tt-destino.cdn_ocor_mpe tt-destino.des_ocor_mpe   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-target-browse   
&Scoped-define SELF-NAME br-target-browse
&Scoped-define QUERY-STRING-br-target-browse FOR EACH tt-destino                               BY tt-destino.cdn_ocor_mpe
&Scoped-define OPEN-QUERY-br-target-browse OPEN QUERY {&SELF-NAME} FOR EACH tt-destino                               BY tt-destino.cdn_ocor_mpe.
...
rt-source-browse-2 br-source-browse br-target-browse bt-add bt-del ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-source-browse w-window _FREEFORM
  QUERY br-source-browse DISPLAY
      tt-origem.cdn_ocor_mpe
      tt-origem.des_ocor_mpe
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-target-browse w-window _FREEFORM
  QUERY br-target-browse DISPLAY
      tt-destino.cdn_ocor_mpe
      tt-destino.des_ocor_mpe
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 36.43 BY 8.75.
...
DEFINE FRAME f-formation
     ...
     br-source-browse AT ROW 4.75 COL 2.86
     br-target-browse AT ROW 4.83 COL 52
     bt-add AT ROW 7 COL 42
     bt-del AT ROW 8.63 COL 42
     bt-marca-todos-source AT ROW 13.88 COL 8.57
     bt-desmarca-todos-source AT ROW 13.88 COL 25.43
     bt-marca-todos-target AT ROW 13.88 COL 58.29
     bt-desmarca-todos-target AT ROW 13.88 COL 74.29
     bt-ok AT ROW 15.71 COL 2.86
     bt-cancela AT ROW 15.71 COL 13.86
     bt-ajuda AT ROW 15.71 COL 78
     "Ocorrˆncias" VIEW-AS TEXT
          SIZE 8.86 BY .67 AT ROW 4 COL 4.14
     "Ocorrˆncias NÇO Autorizadas" VIEW-AS TEXT
          SIZE 20.5 BY .67 AT ROW 4 COL 53.43
     RECT-1 AT ROW 15.5 COL 1.86
     rt-key-parent AT ROW 1.33 COL 1.86
     rt-source-browse AT ROW 4.33 COL 1.86
     rt-source-browse-2 AT ROW 4.38 COL 51
...
ON CHOOSE OF bt-desmarca-todos-source IN FRAME f-formation
DO:
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "YMFP0002C" "1.00.00.000"}
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL tt-origem THEN DO:
      ...
      ASSIGN tt-destino.cdn_empresa   = tt-origem.cdn_empresa
             ...
             tt-destino.cdn_ocor_mpe  = tt-origem.cdn_ocor_mpe 
             tt-destino.des_ocor_mpe  = tt-origem.des_ocor_mpe.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tt-destino WHERE
      NOT CAN-FIND(FIRST ocor_nao_autoriz NO-LOCK WHERE
                         ocor_nao_autoriz.cdn_empresa   = tt-destino.cdn_empresa AND
                         ocor_nao_autoriz.cod_grp_usuar = tt-destino.cod_grp_usuar AND
                         ocor_nao_autoriz.cdn_ocor_mpe  = tt-destino.cdn_ocor_mpe):

      CREATE ocor_nao_autoriz. 
      ASSIGN ocor_nao_autoriz.cdn_empresa   = tt-destino.cdn_empresa
             ocor_nao_autoriz.cod_grp_usuar = tt-destino.cod_grp_usuar
             ocor_nao_autoriz.cdn_ocor_mpe  = tt-destino.cdn_ocor_mpe
             ocor_nao_autoriz.des_ocor_mpe  = tt-destino.des_ocor_mpe.
  END.

  /* Elimina o registro da tabela espec¡fica, caso o mesmo seja retirado do browse de destino */
  FOR EACH tt-origem,
      FIRST ocor_nao_autoriz EXCLUSIVE-LOCK WHERE
            ocor_nao_autoriz.cdn_empresa   = tt-origem.cdn_empresa AND    
            ocor_nao_autoriz.cod_grp_usuar = tt-origem.cod_grp_usuar AND
            ocor_nao_autoriz.cdn_ocor_mpe  = tt-origem.cdn_ocor_mpe:
...
------------------------------------------------------------------------------*/
  ...
  if avail tt-destino THEN DO:
      ...
      ASSIGN tt-origem.cdn_empresa   = tt-destino.cdn_empresa
             ...
             tt-origem.cdn_ocor_mpe  = tt-destino.cdn_ocor_mpe
             tt-origem.des_ocor_mpe  = tt-destino.des_ocor_mpe.
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  IF AVAIL grp_usuar_esp THEN DO:
      ...
      FOR EACH ocor_nao_autoriz NO-LOCK WHERE
               ocor_nao_autoriz.cdn_empresa   = grp_usuar_esp.cdn_empresa AND
               ocor_nao_autoriz.cod_grp_usuar = grp_usuar_esp.cod_grp_usuar:

          CREATE tt-destino.
          ASSIGN tt-destino.cdn_empresa   = ocor_nao_autoriz.cdn_empresa     
                 tt-destino.cod_grp_usuar = ocor_nao_autoriz.cod_grp_usuar     
                 tt-destino.cdn_ocor_mpe  = ocor_nao_autoriz.cdn_ocor_mpe 
                 tt-destino.des_ocor_mpe  = ocor_nao_autoriz.des_ocor_mpe.
      END.

      /* Carregar uma temp-table para armazenar os tipos de ocorrˆncias */
      do v-linhas = 1 to num-entries({database\intm\i02tm049.i 03}):
          create tt_ocor_orig.
          assign tt_ocor_orig.cdn_ocor_mpe  = v-linhas
                 tt_ocor_orig.des_ocor_mpe  = string(entry(v-linhas,{database\intm\i02tm049.i 03})).
      end.

      /* Carregar o browse de origem com os registros que nÆo estÆo no browse de destino */
      FOR EACH tt_ocor_orig NO-LOCK WHERE
          NOT can-find(FIRST tt-destino WHERE
                             tt-destino.cdn_empresa   = grp_usuar_esp.cdn_empresa AND
                             tt-destino.cod_grp_usuar = grp_usuar_esp.cod_grp_usuar AND
                             tt-destino.cdn_ocor_mpe  = tt_ocor_orig.cdn_ocor_mpe):
          CREATE tt-origem.
          ASSIGN tt-origem.cdn_empresa   = grp_usuar_esp.cdn_empresa
                 tt-origem.cod_grp_usuar = grp_usuar_esp.cod_grp_usuar
                 tt-origem.cdn_ocor_mpe  = tt_ocor_orig.cdn_ocor_mpe 
                 tt-origem.des_ocor_mpe  = tt_ocor_orig.des_ocor_mpe.
      END.
